...
class InterviewService {
  ...
  _ensureStoragePaths() {
    try {
      ...
      if (!fs.existsSync(this._historyFile)) {
        fs.writeFileSync(
          this._historyFile,
          JSON.stringify({ items: [], version: 1 }, null, 2),
          'utf8'
        )
      }
    } catch (e) {
  ...
  setupIpcHandlers() {
    ...
    ipcMain.handle(
      'interview-complete',
      this.handleCompleteInterview.bind(this)
    )
  ...
  async handleStartInterview(_event, { config, studioId, userContext }) {
    try {
      ...
      const interviewerPersona = this.createInterviewerPersona(
        studioContext,
        config
      )
      ...
      logger.info(
        `Interview session started: ${sessionId} for studio ${studioId} with persona: ${interviewerPersona.name}`
      )
  ...
  async handleAnalyzeResponse(
    _event,
    { sessionId, questionId: _questionId, response, roleType, studioContext }
  ) {
    try {
      if (!this.aiService) {
  ...
  generateQuestionsFromDatabase(config, studioId) {
    ...
    if (config.includeBehavioral) {
      const behavioralQuestions = this.questionDatabase.behavioral
        .filter(
          q => q.roleSpecific?.includes(config.roleType) || !q.roleSpecific
        )
    ...
    if (config.includeStudioSpecific && studio) {
      const studioQuestions = this.questionDatabase.studioSpecific
        .filter(
          q =>
            q.studioId === studioId || q.applicableStudios?.includes(studioId)
        )
  ...
  createPersonalizedAnalysisPrompt(
    response,
    roleType,
    studioContext,
    interviewerPersona,
...
- Encouraging growth while being honest about areas needing work`
  ...
  createAnalysisPrompt(response, roleType, studioContext) {
    return this.createPersonalizedAnalysisPrompt(
      response,
      roleType,
      studioContext,
      null,
      ...
      behavioral: [
        {
          ...
          type: 'behavioral',
          question:
            'Tell me about a time when you had to solve a complex problem under tight deadline pressure.',
          followUps: [
      ...
      technical: [
        {
          ...
          type: 'technical',
          question:
            'How would you optimize the performance of a game running at 30 FPS to achieve 60 FPS?',
          followUps: [
      ...
      studioSpecific: [
        {
          ...
          studioId: 'riot-games',
          question:
            'How do you think Riot\'s "Player Focus" value should influence feature development decisions?',
          followUps: [
            'Give an example',
            'How would you measure player impact?',
          ],
          expectedDuration: 120,
          difficulty: 'medium',
  ...
  createInterviewerPersona(studio, config) {
    const personas = {
      'riot-games': {
        'Software Engineer': {
          ...
          background:
            "Former competitive gamer turned engineering leader. Led development of VALORANT's anti-cheat system.",
          style:
            'Direct but encouraging, focuses on problem-solving mindset and player empathy',
          values: [
        ...
        'Game Designer': {
          ...
          background:
            'Designer on League of Legends champions and TFT systems. Avid gamer across multiple genres.',
          style:
            'Curious and analytical, loves diving deep into game mechanics and player psychology',
          values: [
            'Player experience',
            'Data-driven design',
            'Creative innovation',
      ...
      blizzard: {
        'Software Engineer': {
          ...
          background:
            "Veteran engineer who worked on WoW's server architecture. Deep MMO gaming experience.",
          style:
            'Methodical and thorough, values craftsmanship and attention to detail',
    ...
    const persona = studioPersonas[config.roleType] ||
      studioPersonas['Software Engineer'] || {
        name: 'Sam Wilson',
        role: 'Senior Interviewer',
        background:
          'Experienced gaming industry professional with passion for finding great talent.',
        style:
          'Professional and thorough, focuses on both technical skills and cultural fit',
        values: [
  ...
  extractUserProfileSummary(userContext) {
    ...
    return {
      name: profile.name || '',
      gamingExperience: this.summarizeGamingExperience(
        profile.gamingExperience || []
      ),
      technicalSkills: this.extractTechnicalSkills(
        profile.skills || [],
        resume.skills || {}
      ),
      softSkills: this.extractSoftSkills(
        profile.skills || [],
        resume.skills || {}
      ),
    ...
    return [
      {
        ...
        type: 'intro',
        question:
          'Tell me about yourself and your interest in the gaming industry.',
  ...
  async saveSessionToHistory(session) {
    try {
      ...
      const summary = {
        ...
        score:
          session.finalAnalysis?.overallScore ||
          session.responses?.reduce(
            (a, r) => a + (r.analysis?.overallScore || 0),
            0
...
4. Specific recommendations for gaming industry interviews`
  ...
  async handleGetStats(_event) {
    try {
      ...
      const avg = total
        ? items.reduce((a, i) => a + (Number(i.score) || 0), 0) / total
        : 0
      ...
      return {
        ...
        data: {
          ...
          metadata: {
            ...
            estimatedDuration: questions.reduce(
