...
function createWindow() {
  ...
  const loadDistOrShowHelp = (devUrl, lastError) => {
    ...
    mainWindow.loadURL('data:text/html;base64,' + Buffer.from(helpHtml).toString('base64'));
  };
  ...
  mainWindow.webContents.on('will-navigate', (event, url) => {
    try {
      ...
      if (target.origin !== 'null' && !/^https?:\/\/127\.0\.0\.1(?::\d+)?$/.test(target.origin)) {
      ...
      interviewService = new InterviewService({
        model: {
          ...
          generateContent: async (request) => {
            const prompt = typeof request === 'string' ? request : (request?.contents?.[0]?.parts?.[0]?.text || '')
        ...
        generateContent: async (prompt, options = {}) => {
          const result = await aiService.generateText(prompt, { systemInstructions: options.systemPrompt, options });
          return result.text;
        }
      });
    } else {
...
ipcMain.handle('ai-generate-text', async (_event, { prompt, systemInstructions, options = {} }) => {
  try {
    if (!aiService) throw new Error('AI service not initialized. Please configure your API key first.');
    const result = await aiService.generateText(prompt, { systemInstructions, options });
    return result.text;
  } catch (error) {
...
ipcMain.handle('ai-generate-content', async (_event, { prompt, systemInstructions, options = {} }) => {
  try {
    if (!aiService) throw new Error('AI service not initialized. Please configure your API key first.');
    const { text, usageMetadata, latencyMs } = await aiService.generateText(prompt, { systemInstructions, options });
...
ipcMain.on('ai-stream-generate-start', async (event, { requestId, prompt, systemInstructions, options = {} }) => {
  if (!aiService) {
    event.sender.send('ai-stream-chunk', { requestId, error: 'AI not initialized' });
    return;
  }
  try {
    const iterator = aiService.streamText(prompt, { systemInstructions, options });
    for await (const result of iterator) {
      if (streamCancelSet.has(requestId)) {
        event.sender.send('ai-stream-complete', { requestId, cancelled: true });
        streamCancelSet.delete(requestId);
        return;
      }
      
      if (result.chunk) {
        event.sender.send('ai-stream-chunk', { requestId, chunk: result.chunk });
      }
    ...
    event.sender.send('ai-stream-complete', { requestId, error: error.message });
  }
});

// Enhanced cancellation support
const streamCancelSet = new Set();
ipcMain.on('ai-stream-cancel', ignoreEvent((requestId) => {
...
[{"skill": "original skill/activity", "professionalSkill": "professional equivalent", "explanation": "how it transfers with examples", "industries": ["relevant industries"], "demandLevel": "high|medium|low", "examples": ["specific scenarios"]}]`;
    ...
    const system = 'You are a career transition specialist expert at mapping diverse experiences to professional skills. Return valid JSON array only, no markdown.';
...
ipcMain.handle('ai-resume-score', async (_event, { resumeData, jobTitle, options = {} }) => {
  try {
    if (!aiService) throw new Error('AI service not initialized');
...
{"score": number (0-100), "strengths": [string array], "improvements": [string array], "keywords": [string array of missing important keywords]}`;
    ...
    const system = 'You are an expert resume reviewer and ATS specialist with 15+ years experience. Focus on actionable feedback and ATS optimization. Only output valid JSON, no markdown.';
    const { text } = await aiService.generateText(prompt, { 
      systemInstructions: system, 
...
ipcMain.handle('media-get-devices', async () => {
  ...
  return { success: true, message: 'Device enumeration should be handled in renderer' };
});
...
ipcMain.handle('audio-stt-transcribe', ignoreEvent(async (request) => {
  try {
    if (!aiService) throw new Error('AI service not initialized');
    ...
    const { text } = await aiService.transcribeAudio({ base64: audioData, mimeType, language });
...
ipcMain.handle('audio-tts-speak', ignoreEvent(async (request) => {
  try {
    if (!aiService) {
      throw new Error('AI service not initialized');
    }
    ...
    if (request.provider === 'gemini') {
      ...
      const audioContent = await aiService.generateSpeech({ text, voice, rate });
...
ipcMain.handle('audio-process-speech', ignoreEvent(async ({ audioBlob, language = 'en-US', mimeType }) => {
  try {
    ...
    if (aiService && audioBlob) {
      ...
      const norm = (() => {
        ...
        if (audioBlob && typeof audioBlob === 'object') {
          if (typeof audioBlob.base64 === 'string') return { base64: audioBlob.base64, mime: audioBlob.mimeType || mimeType || 'audio/wav' };
          if (typeof audioBlob.data === 'string') return { base64: audioBlob.data, mime: audioBlob.mimeType || mimeType || 'audio/wav' };
          if (Array.isArray(audioBlob.data)) {
            ...
            return { base64: buf.toString('base64'), mime: audioBlob.mimeType || mimeType || 'application/octet-stream' };
          }
        }
        return null;
      })();
      if (norm && norm.base64) {
        try {
          const { text } = await aiService.transcribeAudio({ base64: norm.base64, mimeType: norm.mime, language });
          if (text && text.trim()) {
            return { success: true, transcript: text.trim(), confidence: 0.9 };
          }
        } catch (e) {
          console.warn('[audio] AI transcription failed, falling back to offline analysis:', e?.message || e);
        }
      }
    }
    ...
    try {
      ...
      } else if (audioBlob && typeof audioBlob === 'object' && typeof audioBlob.data === 'string') {
        buf = Buffer.from(audioBlob.data, 'base64');
      } else if (audioBlob && typeof audioBlob === 'object' && Array.isArray(audioBlob.data)) {
          ...
          while (offset + 8 <= buf.length) {
            ...
            if (chunkId === 'fmt ') {
              ...
              while (scan + 8 <= buf.length) {
                ...
                if (id2 === 'data') { dataSize = sz2; break; }
                scan += 8 + sz2;
              }
    ...
    const durStr = durationSec ? `${durationSec.toFixed(2)}s` : 'unknown duration';
...
ipcMain.handle('secure-store-get', ignoreEvent(async (key) => {
  try {
    if (keytar) {
...
ipcMain.handle('show-save-dialog', ignoreEvent(async (options) => {
  const result = await dialog.showSaveDialog(mainWindow, options);
  return result;
}));

ipcMain.handle('show-open-dialog', ignoreEvent(async (options) => {
  const result = await dialog.showOpenDialog(mainWindow, options);
  return result;
}));

ipcMain.handle('open-external', ignoreEvent(async (url) => {
...
ipcMain.handle('app-report-error', ignoreEvent(async (errorData) => {
  try {
...
ipcMain.handle('audio-start-recording', ignoreEvent(async ({ deviceId }) => {
  try {
    const streamId = `stream_${Date.now()}`;
    activeRecordings.add(streamId);
    mainWindow.webContents.send('renderer-start-recording', { streamId, deviceId });
...
ipcMain.handle('audio-stop-recording', ignoreEvent(async ({ streamId }) => {
  try {
...
ipcMain.handle('audio-play', ignoreEvent(async ({ audioData }) => {
  try {
    if (!mainWindow) throw new Error('No active window');
...
ipcMain.handle('fs-show-open-dialog', ignoreEvent(async (options) => {
  const result = await dialog.showOpenDialog(mainWindow, options);
  return result;
}));

ipcMain.handle('fs-show-save-dialog', ignoreEvent(async (options) => {
  const result = await dialog.showSaveDialog(mainWindow, options);
  return result;
}));

ipcMain.handle('fs-read-file', ignoreEvent(async ({ filePath }) => {
  try {
...
ipcMain.handle('fs-write-file', ignoreEvent(async ({ filePath, data }) => {
  try {
...
ipcMain.handle('db-create-backup', ignoreEvent(async ({ defaultPath }) => {
  try {
...
async function ensurePrefStore() {
  ...
  try {
    ...
    console.warn('[prefs] electron-store not available, falling back to memory:', error.message);
    prefStore = {
      _data: {},
...
const interviewSuggestionsFile = path.join(__dirname, 'cache', 'interview-suggestions.json');
...
try {
  ...
  if (fs.existsSync(studioDataFile)) {
    ...
    if (raw && raw.version && Array.isArray(raw.data)) {
      ...
      normalizedStudiosCache = raw.data.reduce((acc, s) => { if (s?.id) acc[s.id] = s; return acc; }, {});
  ...
  if (fs.existsSync(interviewSuggestionsFile)) {
    try { interviewStepSuggestions = JSON.parse(fs.readFileSync(interviewSuggestionsFile,'utf8')) || {}; } catch (e) { console.warn('[suggestions] load failed', e.message); }
  }
} catch (e) { console.warn('[studio-stats] failed to load cache', e.message); }

function persistStudioStatsCache() {
  try { fs.writeFileSync(statsCacheFile, JSON.stringify(studioStatsCache, null, 2), 'utf8'); }
  catch (e) { console.warn('[studio-stats] persist failed', e.message); }
}

function persistNormalizedStudios() {
  try { fs.writeFileSync(studioDataFile, JSON.stringify({ version: normalizedStudiosVersion, data: Object.values(normalizedStudiosCache) }, null, 2), 'utf8'); }
  catch (e) { console.warn('[studios-normalized] persist failed', e.message); }
}

function persistInterviewSuggestions() {
  try { fs.writeFileSync(interviewSuggestionsFile, JSON.stringify(interviewStepSuggestions,null,2),'utf8'); }
  catch (e) { console.warn('[suggestions] persist failed', e.message); }
}
...
ipcMain.handle('studio-stats-set', async (_event, { studioId, stats }) => {
  try {
    if (!studioId || typeof stats !== 'object') throw new Error('Invalid params');
    studioStatsCache[studioId] = { ...stats, lastUpdated: stats.lastUpdated || new Date().toISOString() };
...
ipcMain.handle('studios-list-normalized', async () => {
  try {
  return { success: true, version: normalizedStudiosVersion, data: Object.values(normalizedStudiosCache) };
  } catch (e) { return { success: false, error: e.message }; }
});
...
ipcMain.handle('studios-get-normalized', async (_event, { id }) => {
  ...
  return { success: true, version: normalizedStudiosVersion, data: normalizedStudiosCache[id] || null };
  } catch (e) { return { success: false, error: e.message }; }
});

// Interview step suggestions persistence
ipcMain.handle('interview-suggestions-get-all', async () => {
  try { return { success: true, data: interviewStepSuggestions }; }
  catch (e) { return { success: false, error: e.message }; }
});
ipcMain.handle('interview-suggestions-set', async (_event, { stepKey, record, suggestions, ts, model, rationale }) => {
  try {
    ...
    if (record && typeof record === 'object') {
      ...
      finalRecord = { questions: suggestions.slice(0,10), ts: ts || new Date().toISOString(), model: model || 'unknown' };
    } else {
    ...
    const newTs = finalRecord.ts ? new Date(finalRecord.ts).getTime() : Date.now();
    if (newTs >= existingTs) {
...
ipcMain.handle('app-update-settings', async (_event, settings) => {
  try {
    ...
    const allowed = [
      'theme','selectedModel','compactView','portfolioLayout','selectedMicId','selectedSpeakerId'
    ];
...
ipcMain.handle('portfolio-export', async (_event, request) => {
  try {
    const { format = 'json', includeFeaturedOnly = false, includeAnalytics = false } = request || {};
    ...
    if (includeAnalytics) {
      exportData.analytics = {
        ...
        featuredProjects: exportData.projects?.filter(p => p.featured)?.length || 0,
    ...
    console.log(`[Portfolio] Exported ${exportData.projects?.length || 0} projects in ${format} format`);
    return result;
...
ipcMain.handle('portfolio-import', async (_event, { filePath, format = 'auto' }) => {
  try {
    ...
    console.log(`[Portfolio] Imported ${portfolioData.projects?.length || 0} projects from ${filePath}`);
...
ipcMain.handle('jobs-search', async (_event, filters = {}) => {
  try {
    ...
    try {
      ...
      if (mod && typeof mod.searchJobs === 'function') {
        const result = await mod.searchJobs({
          ...
          sources: Array.isArray(sources) && sources.length ? sources : ['arbeitnow', 'jobicy']
        });
        const agg = Array.isArray(result?.jobs) ? result.jobs : (Array.isArray(result) ? result : []);
        if (agg.length) {
          // Map to renderer-expected shape and apply additional filters
          let mapped = agg.map(job => ({
            id: job.id || `${(job.company||'company')}-${(job.title||'role')}`.toLowerCase().replace(/[^a-z0-9]+/g,'-'),
            title: job.title,
            company: job.company,
            ...
            postedDate: job.postedDate || job.posted || new Date().toISOString(),
          ...
          if (roleType) {
            ...
            mapped = mapped.filter(j => (j.roleType||'').toLowerCase().includes(q) || (j.title||'').toLowerCase().includes(q));
          }
          if (experienceLevel) {
            const q = experienceLevel.toLowerCase();
            mapped = mapped.filter(j => (j.experienceLevel||'').toLowerCase().includes(q));
          }
          if (salaryMin > 0) {
            mapped = mapped.filter(j => {
              ...
              if (typeof s === 'string') {
                ...
                if (!m) return false;
                const nums = m.map(n => parseInt(n.replace(/,/g,''),10)).filter(Boolean);
                if (!nums.length) return false;
                return Math.max(...nums) >= salaryMin;
              }
              if (typeof s === 'object' && s.min) return Number(s.min) >= salaryMin;
              return false;
            });
          }
      ...
      console.warn('[Jobs] Aggregated provider search failed, falling back to mock:', e.message);
    }
    ...
    let allJobs = [
      {
        ...
        description: 'Join our team building the core systems that power League of Legends and VALORANT. Work on high-performance game servers, data pipelines, and player-facing features.',
        requirements: [
        ...
        tags: ['C++', 'Go', 'Distributed Systems', 'Gaming', 'Server Infrastructure']
      },
      {
        id: 'blizzard_001',
        ...
        description: 'Design and implement compelling gameplay systems for World of Warcraft. Collaborate with engineering, art, and QA teams to create engaging player experiences.',
        requirements: [
        ...
        tags: ['Game Design', 'MMO', 'Scripting', 'Player Systems', 'World Building']
      },
      {
        id: 'epic_games_001',
        ...
        description: 'Design intuitive and engaging user interfaces for Fortnite across multiple platforms. Focus on player experience and accessibility.',
        requirements: [
      ...
      },
      {
        ...
        experienceLevel: 'senior',
        roleType: 'Software Engineer',
        description: 'Build and maintain the backend systems that power Steam, serving millions of gamers worldwide. Work on scalable microservices and data infrastructure.',
        requirements: [
        ...
        tags: ['Backend', 'Python', 'Microservices', 'Steam', 'Distributed Systems']
      }
    ];
    ...
    if (salaryMin > 0) {
      filteredJobs = filteredJobs.filter(job => 
        job.salary && job.salary.min >= salaryMin
      );
    }
    ...
    console.log(`[Jobs] Search returned ${paginatedJobs.length} of ${filteredJobs.length} jobs`);
...
ipcMain.handle('jobs-get-studios', ignoreEvent(async (query = {}) => {
  try {
    // Use live database-backed service for studios
    const { DatabaseStudioService } = require('./services/DatabaseStudioService.cjs');
    ...
    const { studios = [], total = 0, page = 1, pageSize = 50 } =
...
ipcMain.handle('jobs-save', ignoreEvent(async ({ jobId }) => {
  try {
    if (!jobId) {
...
ipcMain.handle('jobs-get-saved', ignoreEvent(async () => {
  try {
...
ipcMain.handle('jobs-apply', ignoreEvent(async ({ jobId, resume, coverLetter }) => {
  try {
    // Track whether a cover letter was provided (prevents unused variable warning)
    const coverLetterIncluded = !!(typeof coverLetter === 'string' && coverLetter.trim().length);
...
ipcMain.handle('jobs-match', ignoreEvent(async ({ profile, jobs }) => {
  try {
...
ipcMain.handle('studio-search', ignoreEvent(async (request) => {
  try {
    const databaseStudioService = new DatabaseStudioService();
    await databaseStudioService.init();
...
ipcMain.handle('studio-get-by-id', ignoreEvent(async ({ id }) => {
  try {
    const databaseStudioService = new DatabaseStudioService();
    await databaseStudioService.init();
...
ipcMain.handle('studio-get-health', ignoreEvent(async () => {
  try {
    const databaseStudioService = new DatabaseStudioService();
    await databaseStudioService.init();
...
ipcMain.handle('studio-import', ignoreEvent(async (data) => {
  try {
    const databaseStudioService = new DatabaseStudioService();
    await databaseStudioService.init();
...
ipcMain.handle('studio-export', ignoreEvent(async () => {
  try {
    const databaseStudioService = new DatabaseStudioService();
    await databaseStudioService.init();
...
ipcMain.handle('studio-get-statistics', ignoreEvent(async () => {
  try {
    const databaseStudioService = new DatabaseStudioService();
    await databaseStudioService.init();
