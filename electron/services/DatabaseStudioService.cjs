...
class DatabaseStudioService {
    ...
    try {
      ...
      try {
        ...
        for (const studio of expandedStudios) {
          try {
            const converted = this.convertExpandedStudio(studio, 'expanded-gaming');
            studiosToImport.push(converted);
          } catch (error) {
            result.errors.push(`Expanded studio ${studio.name}: ${error.message}`);
          }
        }
      } catch (error) {
        result.errors.push(`Failed to load expanded gaming studios: ${error.message}`);
      }
  ...
  buildExpandedDescription(studio) {
    ...
    if (studio.majorGames?.length > 0) {
      const topGames = studio.majorGames.slice(0, 3).map((g) => g.name).join(', ');
  ...
  async fetchSteamStudios() {
    try {
      const { SteamDataSource } = require('../services/ingestion/SteamDataSource.cjs');
