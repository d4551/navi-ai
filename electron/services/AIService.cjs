...
class MainAIService {
  ...
  async *streamText(prompt, { systemInstructions, options = {} } = {}) {
    ...
    try {
      ...
      if (usage) {
        this._metrics.tokensIn +=
          usage.promptTokenCount || usage.inputTokens || 0
        this._metrics.tokensOut +=
  ...
  async listModels() {
    ...
    try {
        ...
        .map(m => {
          ...
          return {
            ...
            capabilities: {
              ...
              supportsVision: (m.supportedGenerationMethods || []).includes(
      ...
      console.log(
        `[AI] Fetched ${mapped.length} models, cached for ${this._cache.models.ttl / 1000}s`
      )
      logger.info(
        `AI fetched ${mapped.length} models, cached for ${this._cache.models.ttl / 1000}s`
      )
      ...
      const fallbackModels = [
        {
          ...
          pricing: { inputCostPer1M: 0.075, outputCostPer1M: 0.3 },
          voiceSupport: {
            tts: false,
            stt: false,
            realtime: false,
        ...
        },
        {
          ...
          pricing: { inputCostPer1M: 1.25, outputCostPer1M: 5.0 },
          voiceSupport: {
            tts: false,
            stt: false,
            realtime: false,
  ...
  _getModelPricing(modelId) {
    const pricing = {
      ...
      'gemini-1.5-pro': { inputCostPer1M: 1.25, outputCostPer1M: 5.0 },
      'gemini-1.0-pro': { inputCostPer1M: 0.5, outputCostPer1M: 1.5 },
    }
  ...
  getMetrics() {
    ...
    return {
      ...
      model: this._modelId,
      averageLatency:
        this._performanceMetrics.requestLatencies.length > 0
          ? Math.round(
              this._performanceMetrics.requestLatencies.reduce(
                (a, b) => a + b,
                0
              ) / this._performanceMetrics.requestLatencies.length
            )
          : 0,
      errorRate:
        totalRequests > 0
  ...
  async generateSpeech({ text, voice, rate }) {
    ...
    logger.info(
      `Generating speech for text: "${text}" with voice: ${voice} at rate: ${rate}`
    )
    return {
      audioContent:
        'UklGRiQAAABXQVZFZm10IBAAAAABAAEARKwAAIhYAQACABAAAABkYXRhAAAAA', // Placeholder for base64 audio
    }
  }
  ...
  processNewResponse(result, startTime) {
    ...
    console.log(
      `[AI] Generated ${text.length} characters in ${latencyMs}ms (avg: ${this.getMetrics().averageLatency}ms)`
    )
    logger.info(
      `AI generated ${text.length} characters in ${latencyMs}ms (avg: ${this.getMetrics().averageLatency}ms)`
    )
  ...
  handleAPIError(error) {
    ...
    if (error.message?.includes('model') && error.status === 404) {
      return new Error(
        `Model ${this._modelId} not found. Please select a different model.`
      )
    }
