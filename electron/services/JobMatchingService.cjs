...
class JobMatchingService {
  constructor() {
    ...
    this.SALARY_WEIGHT = 0.1
    this.CULTURE_WEIGHT = 0.1
  ...
  calculateMatchScore(userProfile, job) {
    ...
    const jobReq = Array.isArray(job.requirements)
      ? job.requirements
      : this.parseList(job.requirements)
    const jobTech = Array.isArray(job.technologies)
      ? job.technologies
      : this.parseList(job.technologies)
    ...
    const { missingSkills, recommendedSkills, strengths } =
      this.analyzeSkillGaps(criteria.skills, jobReq, jobTech)
  ...
  calculateSkillsMatch(userSkills, jobRequirements, jobTechnologies) {
    ...
    const criticalSkills = (jobRequirements || []).slice(
      0,
      Math.ceil((jobRequirements || []).length * 0.7)
    )
    ...
    const criticalMatches = criticalSkills.filter(skill =>
      userSkillsLower.some(userSkill =>
        this.skillsSimilar(userSkill, String(skill || '').toLowerCase())
      )
    )
    ...
    const criticalScore =
      criticalSkills.length > 0
        ? (criticalMatches.length / criticalSkills.length) * 70
        : 70
  ...
  calculateExperienceMatch(userExperience, jobExperience) {
    ...
    const distance =
      userExperience < jobExp.min
        ? jobExp.min - userExperience
        : userExperience - jobExp.max
  ...
  calculateLocationMatch(userLocation, workStyle, jobLocation, jobRemote) {
    ...
    const up = String(userLocation)
      .split(',')
      .map(s => s.trim().toLowerCase())
    const jp = String(jobLocation)
      .split(',')
      .map(s => s.trim().toLowerCase())
  ...
  calculateCultureMatch(userInterests, company, studioType) {
    let score = 60
    const interestsLower = (userInterests || []).map(i =>
      String(i).toLowerCase()
    )
    if (studioType) {
      if (
        String(studioType) === 'AAA' &&
  ...
  calculateTechnologyMatch(userTechnologies, jobTechnologies) {
    const jobTechLower = (jobTechnologies || []).map(t =>
      String(t).toLowerCase()
    )
    const userTechLower = (userTechnologies || []).map(t =>
      String(t).toLowerCase()
    )
  ...
  analyzeSkillGaps(userSkills, jobRequirements, jobTechnologies) {
    ...
    Array.from(allReq).forEach(skill => {
      if (
        !userLower.some(u => this.skillsSimilar(u, String(skill).toLowerCase()))
      )
        strengths.push ? null : null
    })
    Array.from(allReq).forEach(skill => {
      if (
        !userLower.some(u => this.skillsSimilar(u, String(skill).toLowerCase()))
  ...
  skillsSimilar(skill1, skill2) {
    ...
    for (const [main, vars] of Object.entries(variations)) {
      if (
        (skill1 === main && vars.includes(skill2)) ||
