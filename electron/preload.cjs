...
async function runPreloadTasks() {
  const tasks = [
    { key: 'aiModels', origin: 'ai-list-models', loader: () => ipcRenderer.invoke('ai-list-models') },
    { key: 'appSettings', origin: 'app-get-settings', loader: () => ipcRenderer.invoke('app-get-settings') }
  ];
...
ipcRenderer.on('renderer-start-recording', async (_event, { streamId, deviceId }) => {
  try {
...
const AITextRequestSchema = {
  validate: (data) => {
    ...
    if (!data.prompt || typeof data.prompt !== 'string' || data.prompt.length < 1) return false;
    if (data.systemInstructions && typeof data.systemInstructions !== 'string') return false;
    if (data.options) {
      const opts = data.options;
      if (opts.temperature && (typeof opts.temperature !== 'number' || opts.temperature < 0 || opts.temperature > 2)) return false;
      if (opts.topK && (typeof opts.topK !== 'number' || opts.topK <= 0 || !Number.isInteger(opts.topK))) return false;
      if (opts.topP && (typeof opts.topP !== 'number' || opts.topP < 0 || opts.topP > 1)) return false;
      if (opts.maxTokens && (typeof opts.maxTokens !== 'number' || opts.maxTokens <= 0 || !Number.isInteger(opts.maxTokens))) return false;
...
const InterviewConfigSchema = {
  validate: (data) => {
    if (!data || typeof data !== 'object') return false;
    if (!data.role || typeof data.role !== 'string' || data.role.length < 1) return false;
    if (!['entry', 'mid', 'senior', 'lead'].includes(data.difficulty)) return false;
    if (!data.duration || typeof data.duration !== 'number' || data.duration <= 0 || !Number.isInteger(data.duration)) return false;
...
function checkRateLimit(channel) {
  ...
  if (recentRequests.length >= RATE_LIMIT_MAX_REQUESTS) {
    throw new Error(`Rate limit exceeded for ${channel}. Max ${RATE_LIMIT_MAX_REQUESTS} requests per minute.`);
  }
...
const secureAPI = {
  ...
  ai: {
    init: validateAndCall(
      {
        validate: (data) => {
          if (!data || typeof data !== 'object') return false;
          if (!data.apiKey || typeof data.apiKey !== 'string' || data.apiKey.length < 1) return false;
    ...
    generateText: validateAndCall(
      AITextRequestSchema,
      (request) => ipcRenderer.invoke('ai-generate-text', request)
    ),
    ...
    streamGenerate(request, { onChunk, onComplete, onError } = {}) {
      ...
      const completeListener = (_, data) => {
        ...
        if (data.error) {
          ...
          onComplete && onComplete({ latencyMs: data.latencyMs, cancelled: data.cancelled });
        }
    ...
    requestPermissions: validateAndCall(
      {
        validate: (data) => {
          if (!data || typeof data !== 'object') return false;
          if (data.audio !== undefined && typeof data.audio !== 'boolean' && (typeof data.audio !== 'object')) return false;
          if (data.video !== undefined && typeof data.video !== 'boolean' && (typeof data.video !== 'object')) return false;
          return true;
        }
      },
      (constraints) => ipcRenderer.invoke('media-request-permissions', constraints)
    ),
    
    startRecording: (deviceId) => ipcRenderer.invoke('audio-start-recording', { deviceId }),
    stopRecording: (streamId) => ipcRenderer.invoke('audio-stop-recording', { streamId }),
    ...
    sttTranscribe: validateAndCall(
      {
        validate: (data) => {
          if (!data || typeof data !== 'object') return false;
          if (data.provider && !['system', 'gemini'].includes(data.provider)) return false;
    ...
    ttsSpeak: validateAndCall(
      {
        validate: (data) => {
          if (!data || typeof data !== 'object') return false;
          if (data.provider && !['system', 'gemini'].includes(data.provider)) return false;
          if (!data.text || typeof data.text !== 'string' || data.text.length < 1) return false;
          if (data.voice && typeof data.voice !== 'string') return false;
          if (data.rate !== undefined && (typeof data.rate !== 'number' || data.rate < 0 || data.rate > 5)) return false;
          if (data.pitch !== undefined && (typeof data.pitch !== 'number' || data.pitch < 0 || data.pitch > 2)) return false;
          if (data.volume !== undefined && (typeof data.volume !== 'number' || data.volume < 0 || data.volume > 1)) return false;
          if (data.language && typeof data.language !== 'string') return false;
          if (data.outputDeviceId && typeof data.outputDeviceId !== 'string') return false;
          return true;
        }
      },
  ...
  fs: {
    showOpenDialog: validateAndCall(
      FileDialogOptionsSchema,
      (options) => ipcRenderer.invoke('fs-show-open-dialog', options)
    ),
    
    showSaveDialog: validateAndCall(
      FileDialogOptionsSchema, 
      (options) => ipcRenderer.invoke('fs-show-save-dialog', options)
    ),
  ...
  // Interview System
  interview: {
    start: validateAndCall(
      InterviewConfigSchema,
      (config) => ipcRenderer.invoke('interview-start', { config })
    ),
    
    getQuestion: (sessionId) => ipcRenderer.invoke('question-next', { sessionId }),
    ...
    complete: (sessionId) => ipcRenderer.invoke('interview-complete', { sessionId }),
  ...
  getAllStepSuggestions: () => ipcRenderer.invoke('interview-suggestions-get-all'),
    setStepSuggestions: (stepKey, suggestionsOrRecord, meta = {}) => {
      if (suggestionsOrRecord && suggestionsOrRecord.questions) {
        return ipcRenderer.invoke('interview-suggestions-set', { stepKey, record: suggestionsOrRecord, rationale: meta.rationale })
      }
      return ipcRenderer.invoke('interview-suggestions-set', { stepKey, suggestions: suggestionsOrRecord, rationale: meta.rationale })
    },
  },
  ...
  studios: {
    ...
    setStats: (studioId, stats) => ipcRenderer.invoke('studio-stats-set', { studioId, stats }),
    importNormalized: (studios) => ipcRenderer.invoke('studios-import-normalized', { studios }),
    ...
    export: validateAndCall(
      {
        validate: (data) => {
          ...
          if (data.includeFeaturedOnly !== undefined && typeof data.includeFeaturedOnly !== 'boolean') return false;
          if (data.includeAnalytics !== undefined && typeof data.includeAnalytics !== 'boolean') return false;
          return true;
        }
      },
  ...
  jobs: {
    ...
    applyToJob: (jobId, resume) => ipcRenderer.invoke('jobs-apply', { jobId, resume }),
    match: (profile, jobs) => ipcRenderer.invoke('jobs-match', { profile, jobs }),
    recommend: (profile, jobs, limit = 10) => ipcRenderer.invoke('jobs-recommend', { profile, jobs, limit })
  },
...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  ai: {
    ...
    generateContent: (params) => ipcRenderer.invoke('ai-generate-content', params),
    listModels: () => ipcRenderer.invoke('ai-list-models'),
  },
  ...
  interview: {
    ...
    complete: (sessionId) => ipcRenderer.invoke('interview-complete', { sessionId }),
    cancel: (sessionId) => ipcRenderer.invoke('interview-cancel', { sessionId }),
    nextQuestion: (sessionId) => ipcRenderer.invoke('question-next', { sessionId }),
    ...
    generateQuestions: (config) => ipcRenderer.invoke('generate-questions', config),
  },

  // Media operations
  media: {
    getDevices: () => ipcRenderer.invoke('media-get-devices'),
    requestPermissions: (constraints) => ipcRenderer.invoke('media-request-permissions', constraints),
  },
  ...
  // Database utilities
  db: {
    backupNow: (defaultPath) => ipcRenderer.invoke('db-backup-now', { defaultPath }),
