    ...
    try {
      ...
      for (const [id, template] of Object.entries(RESUME_TEMPLATES)) {
        templates[id] = {
          ...
          preview: template.preview || `https://picsum.photos/seed/${encodeURIComponent(id)}/400/300`
        };
      }
      ...
      try {
        ...
        for (const [name, settings] of Object.entries(customTemplates)) {
          templates[name] = {
            ...
            preview: settings.preview || `https://picsum.photos/seed/${encodeURIComponent(name)}/400/300`,
            isCustom: true
          };
        }
      } catch (e) {
