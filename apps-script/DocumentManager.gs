...
function createResumeDocument(content, title = 'New Resume') {
  try {
    ...
    // Add metadata
    doc.setDescription(
      'Resume created with GeminiCVCL on ' + new Date().toLocaleString()
    )
...
function structureResumeContent(plainContent) {
  ...
  const systemPrompt =
    'You are an expert at formatting resumes. Convert plain text into a well-structured resume with markdown formatting.'
...
function createCoverLetterDocument(content, title = 'Cover Letter') {
  try {
    ...
    // Add metadata
    doc.setDescription(
      'Cover letter created with GeminiCVCL on ' + new Date().toLocaleString()
    )
...
function formatCoverLetter(content) {
  const systemPrompt =
    'You are an expert at writing professional cover letters with proper formatting and structure.'
...
function insertFormattedParagraphs(body, content) {
  ...
  for (const para of paragraphs) {
    ...
    if (trimmed.match(/^[\*\-\+]\s+/)) {
      ...
      const jobTitleMatch = trimmed.match(
        /^([A-Za-z\s]+)\s*[\|,]\s*([A-Za-z\s]+)\s*[\|,]\s*(\d{4}.*\d{4}|\d{4}.*present)/i
      )
      if (jobTitleMatch) {
      ...
      const dateMatch = trimmed.match(
        /\b(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec|January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{4}\s*(-|–|to)\s*(Present|Current|\d{4})\b/i
      )
...
function getDocumentContentById(docId) {
  try {
    ...
    return {
      error: true,
      message: `Could not open document: ${error.toString()}`,
    }
  }
}
...
function updateCurrentDocumentSections(aiContent) {
  try {
    ...
    if (updates.length === 0) {
      ...
      return {
        success: true,
        message: 'Added AI suggestions at the end of document',
      }
    }
...
function insertContentAtPosition(body, content, position) {
  ...
  for (let i = paragraphs.length - 1; i >= 0; i--) {
    ...
    if (paragraph.startsWith('* ') || paragraph.startsWith('- ')) {
      ...
      const numberItem = body.insertListItem(
        position,
 ...
 */
function createJobSpecificDocuments(
  resumeContent,
  coverLetterContent,
  jobInfo
) {
  ...
  const jobFolder = parentFolder.createFolder(folderName)
  {
    ...
    if (jobInfo.url) {
      const textFile = DriveApp.createFile(
        `Job Posting Info.txt`,
 ...
 */
function createJobSpecificDocumentsFromData(
  resumeContent,
  jobContent,
  jobInfo
) {
...
function insertContentToDocument(doc, content) {
  ...
  for (let i = 0; i < sections.length; i++) {
    ...
    if (isHeader) {
      ...
      headerParagraph.setHeading(
        DocumentApp.ParagraphHeading['HEADING' + level]
      )
...
function formatAndInsertParagraphs(body, content) {
  ...
  for (let i = 0; i < paragraphs.length; i++) {
    ...
    if (paragraph.startsWith('* ') || paragraph.startsWith('- ')) {
      ...
      // This is a numbered list item
      const numberItem = body.appendListItem(
        paragraph.replace(/^\d+\./, '').trim()
      )
      numberItem.setGlyphType(DocumentApp.GlyphType.NUMBER)
    } else if (paragraph.startsWith('> ')) {
  ...
  for (let i = 0; i < paragraphs.length; i++) {
    ...
    if (para.toLowerCase().includes('dear ')) {
      ...
      (para.toLowerCase().includes('sincerely') ||
        ...
        para.toLowerCase().includes('yours truly'))
    ) {
      inBodySection = false
...
function createDocumentVersion(documentId, reason = 'Auto-saved version') {
  try {
    ...
    versionBody
      .appendParagraph('VERSION METADATA')
    ...
    // Add the content
    versionBody
      .appendParagraph('DOCUMENT CONTENT')
      .setHeading(DocumentApp.ParagraphHeading.HEADING1)
    ...
    const versionHistory = props.versionHistory
      ? JSON.parse(props.versionHistory)
      : []
    versionHistory.push({
      id: versionDoc.getId(),
      timestamp: timestamp,
      ...
      if (element.getType() === DocumentApp.ElementType.PARAGRAPH) {
        ...
        if (heading !== DocumentApp.ParagraphHeading.NORMAL) {
          ...
          if (text.startsWith('• ') || text.startsWith('* ')) {
            currentSection.formattedContent.push({
              type: 'bullet',
 ...
 */
function createBothDocuments(
  resumeContent,
  cvContent,
  baseTitle = 'Application Documents'
) {
  try {
...
class DocumentManager {
   ...
   */
  static async formatCurrentDocumentWithAI(
    styleTemplate = 'modern-professional'
  ) {
    try {
      const doc = DocumentApp.getActiveDocument()
      if (!doc) {
...
function updateDocumentWithAIAnalysis(prompt) {
  try {
    ...
    const systemPrompt =
      "You are an expert document reviewer and editor. Analyze this document and provide specific improvements based on the user's request."
  ...
  Format each section with ## headers and provide actionable advice.`
    ...
    DriveApp.getRootFolder().removeFile(
      DriveApp.getFileById(analysisDoc.getId())
    )
...
function detectDocumentType(structure) {
  ...
  // Resume indicators
  if (
    (headingsStr.includes('experience') &&
...
function analyzeResumeMetrics(structure) {
  ...
  const quantMatches = structure.content.match(
    /\d+%|\$\d+|\d+\s+percent|\d+\s+times/gi
  )
  ...
  if (metrics.bulletPoints < 10) {
    metrics.improvement.suggestions.push(
      'Add more bullet points to highlight achievements'
    )
  }
...
function analyzeCoverLetterMetrics(structure) {
  ...
  if (metrics.paragraphCount < 3) {
    metrics.improvement.suggestions.push(
      'Add more paragraphs to fully develop your letter'
    )
  } else if (metrics.paragraphCount > 5) {
    metrics.improvement.suggestions.push(
      'Consider condensing your letter to be more concise'
    )
  }

  if (metrics.wordCount < 250) {
    metrics.improvement.suggestions.push(
      'Your letter is quite short. Consider adding more relevant details'
    )
  } else if (metrics.wordCount > 400) {
    metrics.improvement.suggestions.push(
      'Your letter is lengthy. Consider making it more concise'
    )
  }
  ...
  if (structure.content.match(/\bcompany\b|\borganization\b|\bposition\b/gi)) {
    metrics.improvement.suggestions.push(
      "Replace generic terms like 'company' or 'position' with specific names"
    )
  }
...
function generateTargetedDocuments(resumeContent, jobContent) {
  const systemPrompt =
    'You are an expert at tailoring resumes and cover letters to match specific job descriptions. Analyze the provided resume and job posting to create a tailored resume and a compelling cover letter. Maintain a professional tone and format, and address the requirements and keywords from the job posting.'
...
function analyzeJobContent(jobContent) {
  const systemPrompt =
    'You are an expert job market analyst. Extract key information from job postings.'
  ...
  if (keywordMatch) {
    ...
    const keywordLines = keywordList
      .split('\n')
      .map(line => line.trim())
...
function extractJobKeywords(jobContent) {
  const systemPrompt =
    'You are an expert job description analyzer. Your task is to extract the key skills, qualifications, and requirements from a job posting and provide a brief analysis.'
  ...
  * [keyword 3]
  ...
  if (keywordsMatch) {
    ...
    const keywordLines = keywordsList
      .split('\n')
      .map(line => line.trim())
      .filter(line => line.startsWith('*'))
...
function applyFormattingToSection(docId, sectionName, formattingOptions) {
  try {
    ...
    if (formattingOptions.fontSize)
      paragraph.setFontSize(formattingOptions.fontSize)
    if (formattingOptions.font) paragraph.setFontFamily(formattingOptions.font)
    if (formattingOptions.color)
      paragraph.setForegroundColor(formattingOptions.color)
    ...
    for (const element of foundElements) {
      if (formattingOptions.contentBold)
        element.setBold(formattingOptions.contentBold)
      if (formattingOptions.contentItalic)
        element.setItalic(formattingOptions.contentItalic)
      if (formattingOptions.contentFontSize)
        element.setFontSize(formattingOptions.contentFontSize)
      if (formattingOptions.contentFont)
        element.setFontFamily(formattingOptions.contentFont)
    }
...
function optimizeForOnePage(documentId) {
  try {
    ...
    return {
      error: true,
      message: `Could not optimize document: ${error.toString()}`,
    }
  }
}
