...
function generateDocumentFromChat() {
  try {
    ...
    if (!session || !session.history || session.history.length === 0) {
      return { error: true, message: "No chat history found to generate document from" };
    }
    
    // Create a new document
    const doc = DocumentApp.create(`Resume from Chat - ${new Date().toLocaleDateString()}`);
    const body = doc.getBody();
    ...
    cleanContent.forEach(line => {
      ...
      if (trimmedLine.match(/^#+\s/)) {
        ...
        // Bold text
        const paragraph = body.appendParagraph(trimmedLine.replace(/\*\*(.*?)\*\*/g, '$1'));
    ...
    return { error: true, message: `Failed to generate document: ${error.toString()}` };
  }
}
