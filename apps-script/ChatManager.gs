...
function generateDocumentFromChat() {
  try {
    // Get current chat session from Code.gs
    const session = getChatSession()
    if (!session || !session.history || session.history.length === 0) {
      return {
        error: true,
        message: 'No chat history found to generate document from',
      }
    }

    // Create a new document
    const doc = DocumentApp.create(
      `Resume from Chat - ${new Date().toLocaleDateString()}`
    )
    const body = doc.getBody()

    // Add title
    const title = body.appendParagraph('Resume Generated from AI Conversation')
    ...
    const modelResponses = session.history
      .filter(msg => msg.role === 'model')
      .map(msg => msg.parts[0].text)

    if (modelResponses.length === 0) {
      return { error: true, message: 'No AI responses found in chat history' }
    }

    // Process and add the most relevant response (usually the last comprehensive one)
    const relevantContent = modelResponses[modelResponses.length - 1]

    // Clean and format the content
    const cleanContent = relevantContent
      .replace(/```[\s\S]*?```/g, '') // Remove code blocks
      .replace(/^I'll help you.*?\n/gm, '') // Remove AI introductions
      .replace(/^Here's.*?\n/gm, '') // Remove AI transition phrases
      .split('\n')
    ...
    // Apply default resume template
    const templateResult = StyleManager.applyTemplate(doc, 'google-xyz')
    if (templateResult.error) {
      console.warn('Could not apply template:', templateResult.message)
    }
    ...
    return {
      ...
      message: 'Document created successfully from chat history',
    }
  } catch (error) {
    console.error('Error generating document from chat:', error)
    return {
      error: true,
      message: `Failed to generate document: ${error.toString()}`,
    }
  }
}
...
class StyleManager {
          ...
          if (
            ...
            try {
              ...
              paragraph.setBorder(
                null,
                null,
                { width: 1, color: template.accentColor || '#1a73e8' },
                null
              )
            } catch (e) {
    ...
    try {
      ...
      for (let i = 0; i < paragraphs.length; i++) {
        ...
        if (
          ...
          if (
            text.match(
              /^(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec|january|february|march|april|may|june|july|august|september|october|november|december)\s+\d{4}\s*(-|–|to)\s*(present|current|\d{4})$/i
            )
          ) {
    ...
    try {
      ...
      for (let i = 0; i < paragraphs.length; i++) {
        ...
        if (greeting && !signature) {
          ...
          if (
            text.match(
              /^(sincerely|regards|truly|thank you|best|respectfully)/i
            )
          ) {
            paragraph.setAttributes({
              [DocumentApp.Attribute.SPACING_BEFORE]: 12,
  ...
  /**
   * Fix markdown parsing artifacts in document
   *
   * @param {Document} doc - Google Document object
   * @return {Object} Result of operation
   */
    ...
    try {
      ...
      for (const paragraph of paragraphs) {
        ...
        if (text.match(/^\s*[\-\*\+]\s/) || text.match(/^\s*\d+\.\s/)) {
          ...
          const listId =
            body.getListId() || body.insertListItem(0, '').getListId()
        ...
        if (indentLevel > 0) {
          ...
          try {
            paragraph.setBorderLeft(
              2,
              '#CCCCCC',
              DocumentApp.BorderWidth.MEDIUM
            )
        ...
        while ((match = linkPattern.exec(rawText)) !== null) {
          ...
          if (linkText && linkText.length > 0 && url && url.length > 0) {
            ...
            textObj.setForegroundColor(
              linkStart,
              linkStart + linkText.length - 1,
              '#1155CC'
            )
      ...
      for (let i = 0; i < paragraphs.length; i++) {
        ...
        if (headerMatch) {
          ...
          switch (level) {
            case 1:
              headingStyle = DocumentApp.ParagraphHeading.HEADING1
              break
            case 2:
              headingStyle = DocumentApp.ParagraphHeading.HEADING2
              break
            case 3:
              headingStyle = DocumentApp.ParagraphHeading.HEADING3
              break
            case 4:
              headingStyle = DocumentApp.ParagraphHeading.HEADING4
              break
            case 5:
              headingStyle = DocumentApp.ParagraphHeading.HEADING5
              break
            case 6:
              headingStyle = DocumentApp.ParagraphHeading.HEADING6
              break
          }
      ...
      return {
        success: true,
        message: 'Markdown artifacts cleaned successfully',
      }
    } catch (error) {
