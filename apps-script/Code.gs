...
async function formatCurrentDocument() {
  ...
  const settings = JSON.parse(
    userProps.getProperty(USER_PROPS_KEYS.SETTINGS) || '{}'
  )
...
function saveUserProperties(properties) {
  ...
  if (properties.apiKey)
    props.setProperty(USER_PROPS_KEYS.API_KEY, properties.apiKey)
  if (properties.model)
    props.setProperty(USER_PROPS_KEYS.MODEL, properties.model)
  if (properties.settings)
    props.setProperty(
      USER_PROPS_KEYS.SETTINGS,
      JSON.stringify(properties.settings)
    )
  return true
}
...
function improveContentWithAI(title, content) {
  const systemPrompt =
    'You are an expert writer helping to improve document sections. Maintain the same information but enhance the writing style, clarity, and professionalism.'
...
function updateDocumentWithAI(prompt) {
  try {
    ...
    if (structure.error) {
      return {
        error: true,
        message: 'Error analyzing document: ' + structure.message,
      }
    }

    const systemPrompt =
      "You are an expert document editor. Generate content that will improve the document based on the user's request."
  ...
  try {
    ...
    if (!apiKey) {
      return createErrorResponse(
        'API key not found. Please set up your Gemini API key.'
      )
    }
...
function processAISuggestionContent(content, options = {}) {
  ...
  processedContent = processedContent.replace(
    /^I'll help you.*?Here's/im,
    "Here's"
  )
  processedContent = processedContent.replace(
    /^Here are some suggestions.*?:/im,
    ''
  )
