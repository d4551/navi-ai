...
const vueFiles = execSync(
  'find src -name "*.vue" -exec grep -l \'class="[^"]*".*class="[^"]*"\' {} \\;',
  { encoding: 'utf-8' }
)
  .split('\n')
...
console.log(
  `Found ${vueFiles.length} Vue files with duplicate class attributes`
)
...
vueFiles.forEach((file, index) => {
  try {
    ...
    if (fixed !== content) {
      ...
      console.log(
        `${index + 1}/${vueFiles.length}: No changes needed in ${file}`
      )
    }
  } catch (error) {
