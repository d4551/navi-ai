...
const isCI = ['CI', 'GITHUB_ACTIONS', 'BUILDkite'].some(
...
if (isCI || skipFlag) {
  log(
    `Skipping electron-builder (CI=${isCI}, NAVI_SKIP_POSTINSTALL=${skipFlag ? '1' : '0'})`
  )
  process.exit(0)
}
...
child.on('error', err => {
  log(
    `electron-builder failed to start: ${err && err.message ? err.message : err}`
  )
