...
function analyzeBundle() {
  ...
  console.log(
    `📦 Production dependencies: ${Object.keys(pkg.dependencies).length}`
  )
  console.log(
    `📦 Development dependencies: ${Object.keys(pkg.devDependencies).length}\n`
  )
  ...
  for (const dep of HEAVY_DEPS) {
    if (deps[dep]) {
      foundHeavy.push(dep)
      console.log(
        `   ⚠️  ${dep} (${deps[dep]}) - Consider lazy loading or alternatives`
      )
    }
  }
  ...
  if (dateLibs.length > 1) {
    console.log(
      `   📅 Multiple date libraries: ${dateLibs.join(', ')} - Consider using just one`
    )
  }
  ...
  if (httpLibs.length > 1) {
    console.log(
      `   🌐 Multiple HTTP clients: ${httpLibs.join(', ')} - Consider using fetch API`
    )
  }

  // UUID libraries
  const uuidLibs = ['uuid', 'nanoid', '@paralleldrive/cuid2'].filter(
    lib => deps[lib]
  )
  if (uuidLibs.length > 1) {
    console.log(
      `   🆔 Multiple ID generators: ${uuidLibs.join(', ')} - Consider using just one`
    )
  }
...
function generateOptimizationPlan(analysis) {
  ...
  if (analysis.alternatives.length > 0) {
    ...
    analysis.alternatives.forEach((dep, i) => {
      console.log(
        `   ${i + 1}. Replace ${dep} with ${ALTERNATIVE_SUGGESTIONS[dep]}`
      )
    })
    console.log('')
  }
