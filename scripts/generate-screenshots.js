...
if (!ffmpegStatic) {
  console.error(
    'ffmpeg-static binary not found. Please ensure ffmpeg-static is installed.'
  )
  process.exit(1)
}
    ...
    try {
      ...
      for (let index = 1; index <= targetNames.length; index += 1) {
        const sourceFilePath = path.join(
          outputDirectoryPath,
          tempPattern.replace('%i', String(index))
        )
      ...
      console.error(
        'Error while renaming screenshots:',
        renameError && renameError.message ? renameError.message : renameError
      )
      process.exit(1)
    }
  })
