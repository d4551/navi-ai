...
function fixPropsInFile(filePath) {
  try {
    ...
    if (
      ...
      content.includes('withDefaults(defineProps')
    ) {
    ...
    const definePropsPattern =
      /const\s+([a-zA-Z_$][a-zA-Z0-9_$]*)\s*=\s*(?:withDefaults\s*\(\s*)?defineProps/g
    ...
    if (!propsVarName) {
      ...
      const setupFunctionPattern =
        /setup\s*\(\s*([a-zA-Z_$][a-zA-Z0-9_$]*)\s*[,\)]/
    ...
    if (computedPropsPattern.test(newContent)) {
      newContent = newContent.replace(
        computedPropsPattern,
        `computed(() => ${propsVarName}.`
      )
      hasChanges = true
    }
    ...
    if (hasChanges) {
      ...
      return {
        success: true,
        reason: `Fixed props references to use ${propsVarName}`,
        propsVar: propsVarName,
      }
    }
  ...
  try {
    ...
    return new Promise(resolve => {
      exec('npm run lint:check 2>&1', (error, stdout) => {
        ...
        const propsErrors = lines.filter(
          line =>
            line.includes("'props' is not defined") || line.includes('no-undef')
        )
...
async function main() {
  ...
  console.log(
    `📋 Found ${errorFiles.length} files with potential props errors\n`
  )
  ...
  const filesToProcess =
    errorFiles.length > 0 ? errorFiles : allFiles.slice(0, 20) // Limit for safety
  ...
  if (remainingErrors.length > 0) {
    console.log(
      `⚠️  ${remainingErrors.length} files still have props errors (may need manual fixing):`
    )
