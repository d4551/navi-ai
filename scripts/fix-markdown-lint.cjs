...
function fixMarkdownFile(filePath) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (isHeading && prevLine && prevLine.trim() !== '' && !prevLine.startsWith('#')) {
            newLines.push('');
            modified = true;
        }
        
        // Add blank line before list if needed (MD032)
        if (isList && prevLine && prevLine.trim() !== '' && !(/^[\s]*[-*+]\s/.test(prevLine) || /^[\s]*\d+\.\s/.test(prevLine))) {
            newLines.push('');
            modified = true;
        }
        ...
        if (isHeading && nextLine && nextLine.trim() !== '' && !nextLine.startsWith('#')) {
            newLines.push('');
            modified = true;
        }
        
        // Add blank line after list if needed (MD032)
        if (isList && nextLine && nextLine.trim() !== '' && !(/^[\s]*[-*+]\s/.test(nextLine) || /^[\s]*\d+\.\s/.test(nextLine))) {
            newLines.push('');
            modified = true;
        }
        
        // Add blank line after code fence if needed (MD031)
        if (isCodeFence && nextLine && nextLine.trim() !== '' && !nextLine.startsWith('```')) {
            newLines.push('');
            modified = true;
        }
    }
    ...
    if (modified) {
        ...
        fixCount++;
        console.log('  ✅ Added blank lines around headings/lists/code blocks (MD022/MD032/MD031)');
    }
    ...
    if (content !== originalContent) {
        ...
        console.log(`✅ Fixed ${fixCount} markdown issues in ${path.basename(filePath)}`);
        return true;
    } else {
