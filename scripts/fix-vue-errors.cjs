...
function fixVueCompositionApiErrors(content) {
  ...
  const needsVueImports =
    /\b(ref|computed|watch|onMounted|reactive|nextTick)\b/.test(content)
  const hasVueImports =
    /import\s+{[^}]*\b(ref|computed|watch|onMounted)\b[^}]*}\s+from\s+['"]vue['"]/.test(
      content
    )
  ...
  if (needsVueImports && !hasVueImports) {
    ...
    if (imports.length > 0) {
      ...
      if (scriptMatch) {
        const insertIndex =
          content.indexOf(scriptMatch[0]) + scriptMatch[0].length
        content =
...
function fixRouterErrors(content) {
  ...
  const hasRouterImport =
    /import\s+{[^}]*useRouter[^}]*}\s+from\s+['"]vue-router['"]/.test(content)
...
function fixSpecificTypeErrors(content, filePath) {
  ...
  if (filePath.includes('JobSearch.vue')) {
    ...
    content = content.replace(
      /import\s+{\s*computed,\s*watch,\s*watch\s*}/g,
  ...
  for (const filePath of targetFiles) {
    if (fs.existsSync(filePath)) {
      ...
      if (fixed) {
        ...
        console.log(
          `ℹ️  No patterns to fix in: ${path.relative(process.cwd(), filePath)}`
        )
      }
    } else {
