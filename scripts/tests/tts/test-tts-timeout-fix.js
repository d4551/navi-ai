...
async function testTTSConfiguration() {
  ...
  testTexts.forEach((text, index) => {
    ...
    const bufferTime = Math.max(
      8000,
      Math.min(30000, estimatedSpeechTimeMs * 2)
    )

    console.log(
      `${index + 1}. Text: "${text.substring(0, 50)}${text.length > 50 ? '...' : ''}"`
    )
    console.log(
      `   Words: ${wordCount}, Timeout: ${bufferTime}ms (${(bufferTime / 1000).toFixed(1)}s)`
    )
  })
  ...
  if (typeof window !== 'undefined') {
    ...
    console.log(
      'ℹ️  Running in Node.js environment (browser APIs not available)'
    )
  }
