...
async function testSteamDataSource() {
  ...
  try {
    ...
    console.log(`   Steam API connection: ${canConnect ? "✅ Connected" : "❌ Failed"}`);
    ...
    if (studios.length > 0) {
      ...
      studios.slice(0, 5).forEach((studio, i) => {
        ...
        if (studio.games && studio.games.length > 0) {
          console.log(`      Top games: ${studio.games.slice(0, 3).map(g => g.name).join(', ')}`);
        }
        console.log('');
      });
      ...
      const mediumConfidenceStudios = studios.filter(s => s.confidence > 0.5 && s.confidence <= 0.7);
      ...
      console.log(`   Medium confidence (50-70%): ${mediumConfidenceStudios.length}`);
      console.log(`   Low confidence (<50%): ${lowConfidenceStudios.length}`);
      
      const gamesPerStudio = studios.map(s => s.games?.length || 0);
      const avgGames = gamesPerStudio.reduce((a, b) => a + b, 0) / gamesPerStudio.length;
