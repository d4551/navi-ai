...
try {
  ...
  keyModels.forEach(model => {
    const caps = Object.keys(model.capabilities || {}).filter(
...
try {
  const { initializeAI, generateContent } = await import(
...
try {
  const LiveMultimediaAIService = await import(
...
try {
  const { resolveGeminiApiKey, requireGeminiApiKey } = await import(
...
try {
  const { GoogleAIStreamingService } = await import(
