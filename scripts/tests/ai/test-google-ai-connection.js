...
async function testGoogleAIConnection() {
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log(`   ✅ Connected to Google AI API - Found ${data.models?.length || 0} models`);
      
      // Show available Gemini models
      const geminiModels = data.models?.filter(m => m.name.includes('gemini')).slice(0, 8);
    ...
    const result = await model.generateContent('Hello! Please confirm that Google AI is connected to the Navi system and ready for multimedia AI tasks.');
    
    if (result?.response?.text()) {
      results.textGeneration = true;
      console.log('   ✅ Text generation working');
      console.log('   AI Response:', result.response.text().substring(0, 100) + '...');
    }
    ...
    try {
      ...
      console.log('   Capabilities response:', multimodalResult.response.text().substring(0, 150) + '...');
    } catch (error) {
