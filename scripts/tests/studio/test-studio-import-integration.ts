...
function convertToSimpleStudio(raw: any): SimpleStudio {
  return {
    ...
    description:
      raw.description ||
      `Gaming studio known for ${raw.games?.[0]?.name || 'game development'}`,
...
async function testStudioImportIntegration() {
  ...
  try {
    ...
    console.log(
      `📊 Target: ${testJob.metadata.maxStudios} studios (fetching ${testJob.metadata.maxStudios * 10} apps)`
    )
    console.log(
      '⏱️  This will take 2-3 minutes due to Steam rate limiting...\n'
    )
    ...
    for (const raw of rawStudios) {
      if (raw.name && raw.confidence > 0.3) {
    ...
    console.log(
      `   Converted ${simpleStudios.length} studios for application use`
    )
    ...
    topStudios.forEach((studio, i) => {
      ...
      console.log(
        `      Games: ${studio.games.length} (${studio.games.slice(0, 3).join(', ')}${studio.games.length > 3 ? '...' : ''})`
      )
    ...
    console.log(
      `   Would skip: ${skippedCount} studios (low confidence/no games)`
    )
    ...
    console.log(
      `🎯 Steam integration is working and would import ${storedCount} gaming studios`
    )

    // Performance metrics
    const appsPerSecond =
      (testJob.metadata.maxStudios * 10) / (fetchDuration / 1000)
    ...
    console.log(
      `   Success rate: ${((rawStudios.length / (testJob.metadata.maxStudios * 10)) * 100).toFixed(1)}%`
    )
  } catch (error) {
