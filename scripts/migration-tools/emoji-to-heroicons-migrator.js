...
const EMOJI_TO_HEROICON_MAPPINGS = {
  ...
  '🎪': { name: 'BuildingStorefrontIcon', type: 'outline', alt: 'entertainment' },
...
class EmojiToHeroiconsMigrator {
  ...
  migrateContent(content) {
    ...
    for (const [emoji, iconConfig] of Object.entries(EMOJI_TO_HEROICON_MAPPINGS)) {
    ...
    for (const [text, iconConfig] of Object.entries(EMOJI_TO_HEROICON_MAPPINGS)) {
  ...
  migrateFile(filePath) {
    ...
    const { modifiedContent, hasChanges, fileIcons } = this.migrateContent(content);
