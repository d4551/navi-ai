...
function verifyPiniaAndFira(projectRoot) {
  ...
  try {
    ...
    if (mainJs.includes('createPinia(') || mainJs.includes('createPinia()'))
      results.pinia = true
  } catch {}
  try {
    const idxCss = fs.readFileSync(
...
async function run() {
  ...
  console.log('-------')
  console.log(
    `Tailwind dark mode: processed ${tRes.processedFiles}, modified ${tRes.modifiedFiles}`
  )
  console.log(
    `Emoji→Heroicons:    processed ${eRes.processedFiles}, modified ${eRes.modifiedFiles}`
  )
  console.log(
    `Legacy→Heroicons:   processed ${lRes.processedFiles}, modified ${lRes.modifiedFiles}`
  )
  console.log('\nChecks')
  ...
  if (!deps.pinia)
    log(
      'Pinia is not fully configured. Please install and register it in src/main.js',
      'warn'
    )
  if (!deps.fira)
    log(
      'Fira Code font not imported in styles. Add @fontsource/fira-code imports to src/styles/index.css',
      'warn'
    )
  if (!deps.heroiconsVue)
    log('Install @heroicons/vue to use Heroicons Vue components', 'warn')
}
