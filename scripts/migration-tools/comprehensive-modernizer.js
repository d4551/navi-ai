...
const MDI_TO_HEROICON = {
  ...
  'mdi-alert-circle-outline': { name: 'ExclamationCircleIcon', type: 'outline' },
  ...
  'mdi-help-circle-outline': { name: 'QuestionMarkCircleIcon', type: 'outline' },
  ...
  'mdi-clipboard-check-outline': { name: 'ClipboardDocumentCheckIcon', type: 'outline' },
...
class ComprehensiveMigrator {
  ...
  migrateFile(filePath) {
    ...
    content = content.replace(/(leading-icon|trailing-icon|icon)="(mdi-[^"]+)"/g, (match, prop, mdiName) => {
      const heroicon = MDI_TO_HEROICON[mdiName];
      if (heroicon) {
        fileIcons.add(heroicon);
        modified = true;
    ...
    for (const [legacyClass, tailwindClass] of Object.entries(LEGACY_CSS_MAPPINGS)) {
    ...
    this.log(`Migrated ${path.relative(this.projectRoot, filePath)}: ${changes.icons.length} icons, ${changes.classes.length} classes`);
    return true;
  }
  ...
  async migrate() {
    ...
    for (const file of files) {
      try {
        ...
        this.log(`Failed to process ${path.relative(this.projectRoot, file)}: ${error.message}`, 'error');
      }
    }
  ...
  generateReport() {
    ...
    for (const change of this.changes) {
      if (change.changes.icons.length > 0 || change.changes.classes.length > 0) {
        console.log(`\n📄 ${change.file}:`);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const options = {
    ...
    backupDir: args.find(a => a.startsWith('--backup-dir='))?.split('=')[1] || 'migration-backups',
  };
