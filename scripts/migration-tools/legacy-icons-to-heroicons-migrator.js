...
function inferTokenFromLegacy(classes, innerText = '') {
  ...
  for (const c of candidates) {
    ...
    if (TOKEN_TO_HEROICON[c.replace(/_/g, '-')])
      return TOKEN_TO_HEROICON[c.replace(/_/g, '-')]
...
class LegacyIconsToHeroiconsMigrator {
  ...
  async findFiles() {
    const patterns = ['src/**/*.vue', 'src/**/*.html']
    const files = []
  ...
  addImports(content, fileIcons) {
    ...
    let imports = ''
    if (outline.length)
      imports += `import { ${Array.from(new Set(outline)).join(', ')} } from '@heroicons/vue/24/outline';\n`
    if (solid.length)
      imports += `import { ${Array.from(new Set(solid)).join(', ')} } from '@heroicons/vue/24/solid';\n`
  ...
  migrateFile(filePath) {
    ...
    content = content.replace(
      ...
      (match, _pre, classes, _post) => {
        ...
        const extra = classes
          ...
          .filter(
            c =>
              !/^(mdi|mdi-[^\s]+|fa|fas|far|fal|fad|fab|fa-[^\s]+|bi|bi-[^\s]+|icon-[^\s]+|material-icons)$/.test(
                c
              )
          )
          .join(' ')
    ...
    content = content.replace(
      /<span([^>]*?)class=["'][^"']*material-icons[^"']*["']([^>]*)>([^<]+)<\/span>/gi,
    ...
    this.changes.push({
      ...
      icons: Array.from(fileIcons).map(i => i.name),
    })
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  const options = {
    ...
    backupDir:
      args.find(a => a.startsWith('--backup-dir='))?.split('=')[1] || 'backups',
  }
