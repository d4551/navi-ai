...
const PATTERN_RECOGNIZERS = [
  {
    ...
    regex: /position:\s*absolute;\s*top:\s*var\(--spacing-neg-10\);\s*left:\s*var\(--spacing-2\);/g,
    replacement: 'position: absolute; /* Use .skip-link class from design system */',
    classToAdd: 'skip-link'
  },
  {
    name: 'Center Absolute Pattern',
    regex: /position:\s*absolute;\s*top:\s*50%;\s*left:\s*50%;\s*transform:\s*translate\(-50%,\s*-50%\);/g,
    replacement: 'position: absolute; /* Use .center-absolute class from design system */',
    classToAdd: 'center-absolute'
  },
  {
    name: 'Center Fixed Pattern', 
    regex: /position:\s*fixed;\s*top:\s*50%;\s*left:\s*50%;\s*transform:\s*translate\(-50%,\s*-50%\);/g,
    replacement: 'position: fixed; /* Use .center-fixed class from design system */',
    classToAdd: 'center-fixed'
  },
  {
    name: 'Full Overlay Pattern',
    regex: /position:\s*absolute;\s*top:\s*0;\s*left:\s*0;\s*width:\s*100%;\s*height:\s*100%;/g,
    replacement: 'position: absolute; /* Use .overlay-full class from design system */',
    classToAdd: 'overlay-full'
  },
  {
    name: 'Fixed Full Overlay Pattern',
    regex: /position:\s*fixed;\s*top:\s*0;\s*left:\s*0;\s*width:\s*100vw;\s*height:\s*100vh;/g,
    replacement: 'position: fixed; /* Use .overlay-fixed-full class from design system */',
    classToAdd: 'overlay-fixed-full'
  },
  {
    name: 'Corner Top Right Pattern',
    regex: /position:\s*absolute;\s*top:\s*var\(--spacing-4\);\s*right:\s*var\(--spacing-4\);/g,
    replacement: 'position: absolute; /* Use .corner-top-right class from design system */',
    classToAdd: 'corner-top-right'
  },
  {
    name: 'Corner Bottom Right Pattern',
    regex: /position:\s*absolute;\s*bottom:\s*var\(--spacing-4\);\s*right:\s*var\(--spacing-4\);/g,
    replacement: 'position: absolute; /* Use .corner-bottom-right class from design system */',
    classToAdd: 'corner-bottom-right'
  },
  {
    name: 'FAB Pattern',
    regex: /position:\s*fixed;\s*bottom:\s*var\(--spacing-6\);\s*right:\s*var\(--spacing-6\);/g,
...
function updateFile() {
  ...
  Object.entries(POSITION_MAPPINGS).forEach(([cssProperty, utilityClass]) => {
    const regex = new RegExp(cssProperty.replace(/[.*+?^${}()|[\\]\\\\]/g, '\\\\$&'), 'g');
  ...
  if (absoluteMatches) {
    console.log(`  ⚠️  Found ${absoluteMatches.length} absolute/fixed positioning instances`);
    changes.push({
      type: 'positioning',
...
function generateRecommendations(allChanges) {
  ...
  if (positioningIssues.length > 0) {
    ...
    positioningIssues.forEach(change => {
      console.log(`   - ${change.count} absolute/fixed positioning instances found`);
    });
    console.log();
  }
  ...
  console.log(`   2. Replace CSS properties with utility classes where appropriate`);
  console.log(`   3. Use semantic classes (.skip-link, .fab, .center-absolute) for common patterns`);
  ...
  console.log(`   - Position utilities: .position-absolute, .position-fixed, .position-sticky`);
  ...
  console.log(`   - Semantic patterns: .skip-link, .fab, .center-absolute, .overlay-full`);
...
function main() {
  ...
  console.log(`📋 Review the recommendations above and manually apply remaining changes.`);
}
