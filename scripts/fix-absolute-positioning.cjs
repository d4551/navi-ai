  ...
  for (const file of vueFiles) {
    ...
    lines.forEach((line, index) => {
      ...
      if (
        ...
        trimmedLine.includes('absolute')
      ) {
        const isAllowed = ALLOWED_ABSOLUTE_PATTERNS.some(
          pattern =>
      ...
      for (const [hardcoded, token] of Object.entries(
        DESIGN_TOKEN_REPLACEMENTS
      )) {
        if (
          trimmedLine.includes(hardcoded) &&
...
function generateReport(issues) {
  ...
  issues.forEach(issue => {
    ...
    report.summary.byFile[issue.file] =
      (report.summary.byFile[issue.file] || 0) + 1
  })
