...
const { getVitePort, getElectronPort, isPortAvailable } = require('./get-random-port.cjs');
...
async function testRandomPorts() {
  ...
  try {
    ...
    if (uniquePorts.length === ports.length) {
      ...
      console.log('   ⚠️  Some ports were duplicated (this can happen with high port usage)\n');
    }
    ...
    console.log(`   Port ${testPort} available: ${isAvailable ? '✅ Yes' : '❌ No'}\n`);
    ...
    console.log(`   Vite port ${vitePort} in range (3000-9999): ${viteInRange ? '✅ Yes' : '❌ No'}`);
    console.log(`   Electron port ${electronPort} in range (3000-9999): ${electronInRange ? '✅ Yes' : '❌ No'}\n`);
    ...
    console.log('   npm run electron-dev   - Start Electron with Vite on random port');
