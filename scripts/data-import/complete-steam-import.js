...
async function completeSteamImport() {
  ...
  console.log(
    '⏱️  This will take 10-30 minutes to complete due to Steam rate limits'
  )
  ...
  try {
    ...
    console.log(
      '  ⏱️  Total duration:',
      (result.duration / 1000 / 60).toFixed(1) + ' minutes'
    )
    ...
    const staticStudios = allStudios.filter(
      s => !s.dataSource.includes('steam')
    )
    ...
    if (steamStudios.length > 0) {
      ...
      steamStudios.slice(0, 10).forEach((studio, i) => {
        console.log(
          `  ${i + 1}. ${studio.name} (${studio.games?.length || 0} games, confidence: ${studio.confidence?.toFixed(2) || 'N/A'})`
        )
      })

      // Show confidence distribution
      const highConfidence = steamStudios.filter(s => s.confidence > 0.7).length
      const mediumConfidence = steamStudios.filter(
    ...
    console.log(
      `🎮 Steam API provided ${steamStudios.length} additional real studios`
    )
  } catch (error) {
