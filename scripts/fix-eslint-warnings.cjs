...
class ESLintWarningFixer {
  ...
  fixMissingImports(content, _filePath) {
    ...
    if (
      content.includes('<AppIcon') &&
      !content.includes('import AppIcon from')
    ) {
      ...
      if (scriptMatch) {
        ...
        const existingImports = content
          .slice(insertPos)
          .match(/^\s*import.*from.*$/m)
        if (existingImports) {
          // Add after existing imports
          const lastImportEnd =
            insertPos + existingImports.index + existingImports[0].length
          content =
  ...
  fixShowPropOptional(content, _filePath) {
    ...
    const showPropRegex =
      /(show:\s*\{\s*type:\s*Boolean,?\s*default:\s*[^}]+)(\s*\})/gi
    const replacement = '$1,\n    required: false$2'
