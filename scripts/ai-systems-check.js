...
async function runNodeSystemsCheck(apiKey) {
  ...
  try {
    ...
    if (apiKey && apiKey.length >= 30) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log(colorize(`  ❌ Models endpoint failed: ${response.status}`, 'red'))
        ...
        console.log(colorize(`  ❌ Connectivity test failed: ${error.message}`, 'red'))
    ...
    console.log(colorize(`Overall Score: ${score}/${maxScore} (${percentage}%) - ${status}`, statusColor))
    ...
    if (!apiKey) {
      console.log('   • Get a free API key from https://aistudio.google.com/apikey')
