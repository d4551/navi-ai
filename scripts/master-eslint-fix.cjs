...
class MasterESLintFixer {
  ...
  async run() {
    ...
    console.log(
      `Initial state: ${initialStats.errors} errors, ${initialStats.warnings} warnings`
    )
    ...
    this.runCommand('npm run lint', 'Step 5: Running ESLint auto-fix')
    ...
    this.runCommand('npm run lint', 'Step 5: Final ESLint validation')
    ...
    console.log(
      `Initial: ${initialStats.errors} errors, ${initialStats.warnings} warnings`
    )
    console.log(
      `Final:   ${finalStats.errors} errors, ${finalStats.warnings} warnings`
    )
    console.log(
      `Errors reduced by:   ${initialStats.errors - finalStats.errors}`
    )
    console.log(
      `Warnings reduced by: ${initialStats.warnings - finalStats.warnings}`
    )
    ...
    const globalVarFixes = [
      {
        ...
        fixes: [
          ...
          {
            search: 'CompressionStream',
            replace: '/* global CompressionStream */',
          },
          {
            search: 'DecompressionStream',
            replace: '/* global DecompressionStream */',
          },
