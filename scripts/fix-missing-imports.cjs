...
function fixVueCompositionImports(content) {
  ...
  const scriptContent = lines
    .slice(scriptStartIndex + 1, scriptEndIndex)
    .join('\n')
  const hasSetupFunction = /export\s+default\s+{[^}]*setup\s*\(/m.test(
    scriptContent
  )
  ...
  if (
    /\b(ref|computed|watch|onMounted|nextTick|reactive)\b/.test(scriptContent)
  ) {
    const vueImports = []
...
function fixUnusedVariables(content) {
  ...
  const unusedPatterns = [
    {
      pattern: /const\s+(\w+)\s*=.*?catch\s*\(\s*(\w+)\s*\)/g,
      replacement: 'const $1 = ... catch (_$2)',
    },
    {
      pattern: /let\s+(\w+)\s*=.*?catch\s*\(\s*(\w+)\s*\)/g,
      replacement: 'let $1 = ... catch (_$2)',
    },
  ]
