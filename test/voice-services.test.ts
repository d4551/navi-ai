...
const mockMediaStream = {
  ...
  getAudioTracks: vi
    .fn()
    .mockReturnValue([{ stop: vi.fn(), kind: 'audio', enabled: true }]),
  getVideoTracks: vi
    .fn()
    .mockReturnValue([{ stop: vi.fn(), kind: 'video', enabled: true }]),
}
  ...
  describe('LiveMultimediaAIService - Comprehensive Tests', () => {
    ...
    describe('Screenshot Analysis', () => {
      ...
      it('should handle screen capture permission errors', async () => {
        ...
        await expect(service.captureAndAnalyzeScreen()).rejects.toThrow(
