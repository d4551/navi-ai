...
import {
  ...
  AIConfig,
  AIResponse,
} from '@/modules/ai/CanonicalAIService'
import {
  LiveMultimediaAIService,
  MultimediaAIConfig,
...
Object.defineProperty(navigator, 'mediaDevices', {
  value: {
    getUserMedia: vi.fn().mockResolvedValue({
      getTracks: vi
        .fn()
        .mockReturnValue([{ stop: vi.fn() }, { stop: vi.fn() }]),
    }),
...
describe('CanonicalAIService', () => {
  ...
  describe('Multimodal Capabilities', () => {
    ...
    it('should fail when multimodal is disabled', async () => {
      ...
      await expect(aiService.generateMultimodal(input)).rejects.toThrow(
        'Multimodal features not enabled'
      )
    })
  })
  ...
  describe('Real-time Sessions', () => {
    ...
    it('should send real-time messages', async () => {
      ...
      const response = await aiService.sendRealTimeMessage(
        sessionId,
        'Hello in real-time'
      )
  ...
  describe('Health Check', () => {
    ...
    it('should report degraded status when some tests fail', async () => {
      ...
      mockGenerateContentStream.mockRejectedValue(
        new Error('Streaming not available')
      )
      ...
      expect(health.details.tests.errors).toContain(
        'Streaming test: Streaming not available'
      )
    })
...
describe('LiveMultimediaAIService', () => {
  ...
  describe('Audio Streaming', () => {
    ...
    it('should throw error when audio not enabled', async () => {
      ...
      await expect(service.startAudioStreaming()).rejects.toThrow(
