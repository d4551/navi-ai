<template>
  <div
    class="navigation-item text-glass-secondary"
    :class="{
  ...
  >
    ...
    >
      <div class="nav-item-icon">
        ...
        <AppIcon :name="normalizeIcon(item.icon)" class="nav-icon" size="inherit" />
      </div>
    ...
    <Teleport to="body">
      ...
      >
        <div class="tooltip-content">
          ...
          <div v-if="item.badge" class="tooltip-badge"><span class="badge badge-compact stats-chip">{{ item.badge }}</span></div>
        </div>
...
.nav-item-link:focus-visible {
  outline: none;
  box-shadow:
    0 0 0 var(--focus-ring-width) color-mix(in srgb, var(--neon-focus-ring) 70%, transparent),
...
.navigation-item.active .nav-item-link {
  ...
  -webkit-backdrop-filter: var(--glass-backdrop-blur) var(--glass-backdrop-saturate);
  box-shadow: 0 8px 24px color-mix(in srgb, var(--color-primary-500) 10%, transparent);
}
...
.nav-item-badge .badge { white-space: nowrap; }
...
.tooltip-badge { display: inline-block; }
...
.dark-theme .navigation-item.active .nav-item-link {
  ...
  background: var(--glass-bg);
  border: 1px solid color-mix(in srgb, var(--color-primary-400) 18%, var(--glass-border));
  color: var(--color-primary-400);
  box-shadow: 0 10px 28px color-mix(in srgb, var(--color-primary-400) 16%, transparent);
}
...
@media (prefers-color-scheme: dark) {
  ...
  :root:not([data-theme="light"]) .navigation-item.active .nav-item-link {
    background: var(--glass-bg);
    border: 1px solid color-mix(in srgb, var(--color-primary-500) 16%, var(--glass-border));
    color: var(--color-primary-500);
    box-shadow: 0 10px 28px color-mix(in srgb, var(--color-primary-500) 14%, transparent);
  }
...
.navigation-item.active .nav-item-link[href*="/demo/realtime"] .nav-icon-glass::after {
  content: '';
  position: absolute;
  inset: -30%;
  background: radial-gradient(60% 60% at 30% 30%, color-mix(in srgb, var(--color-primary-500) 20%, transparent), transparent 70%);
  opacity: 0.35;
...
@keyframes glassAurora {
  0% { transform: translate(-6%, -6%) scale(1); opacity: 0.25; }
  50% { transform: translate(6%, 6%) scale(1.05); opacity: 0.45; }
  100% { transform: translate(-6%, -6%) scale(1); opacity: 0.25; }
}

/* Respect user motion preference */
@media (prefers-reduced-motion: reduce) {
  .navigation-item.active .nav-item-link::after { animation: none; opacity: 0; }
  .navigation-item.active .nav-icon-glass::after { animation: none; opacity: 0; }
}
...
.theme-gaming .navigation-item.active .nav-item-link {
  background: linear-gradient(135deg, rgba(0, 255, 136, 0.1), rgba(0, 217, 255, 0.05)), var(--glass-bg);
...
.theme-gaming .nav-item-link::before {
  background: linear-gradient(180deg, var(--color-gaming-500), var(--color-cyber-500));
}
...
/* Respect user motion preference */
@media (prefers-reduced-motion: reduce) {
  .navigation-item.nav-item-pressed .nav-item-link { transition: none; }
}
