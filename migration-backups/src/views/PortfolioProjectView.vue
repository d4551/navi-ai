          ...
          <aside class="col side">
            <div class="glass p-4 gap-4 rounded-lg unified-card details">
              ...
              <div v-if="(item.tags||item.skills)?.length" class="tags">
                <span v-for="t in (item.tags || item.skills)" :key="t" class="tag">{{ t }}</span>
              </div>
              <div v-if="links.length" class="divider" />
              <div v-if="links.length" class="links">
                <a v-for="l in links" :key="l.url" :href="l.url" target="_blank" rel="noopener" class="link">
...
async function share() {
  try {
    ...
    if (navigator.share) await navigator.share({ title: item.value?.title || 'Project', url })
...
.hero-media img { width: 100%; height: auto; display: block; border-radius: var(--radius-lg); }

.project-body .grid { display: grid; grid-template-columns: 1fr; gap: 14px; }
.project-body .main { display: grid; gap: var(--spacing-3); }
.project-body .side { display: grid; gap: var(--spacing-3); }

@media (min-width: 992px) {
  .project-body .grid { grid-template-columns: 1fr 340px; }
}
