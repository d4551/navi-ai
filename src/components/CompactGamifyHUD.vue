<template>
  <div class="compact-gamify-hud glass-card section-card font-sans" role="region" aria-label="Gamification">
    ...
    <div class="hud-bar" :title="`${levelInfo.xpInLevel}/${levelInfo.xpRequiredForLevel} XP`">
...
function openQuests() { router.push('/dashboard') }
function openDaily() { router.push('/dashboard') }
</script>
...
.hud-streak { display: inline-flex; align-items: center; gap: 6px; opacity: 0.9; }
.hud-bar { height: 8px; background: var(--glass-bg); border: 1px solid var(--glass-border); border-radius: 999px; overflow: hidden; }
.hud-bar .fill { height: 100%; background: var(--gradient-accent-primary); transition: width .3s ease; }
.hud-actions { display: inline-flex; gap: 6px; justify-content: flex-end; }
</style>
