<template>
  <div class="ai-media-workbench">
    <GlassNavTabs v-model:active-tab="activeTab" :tabs="tabs" aria-label="AI Media Tabs" />
    ...
    <div class="toolbar" role="toolbar" aria-label="Mode and session controls">
      <div class="session-meta">
        ...
        <input id="sessionName" v-model.trim="sessionName" class="session-name glass-input" type="text" placeholder="Session name" @change="persistSessionName" />
      </div>
      <div class="modes">
        <button
          v-for="m in modes"
      ...
      <div class="meta">
        <span class="badge" :title="'Total AI responses'">{{ responses.length }} responses</span>
    ...
    <div v-if="split" class="split-grid">
      <div class="panel">
        <AIMediaIntegration ref="liveRef" :mode-prompt="modePrompt" @ai-response="recordResponse" @integration-error="onError" />
      </div>
      <div class="panel glass-surface">
        <div class="gallery">
          <div v-if="!frames.length" class="muted">No snapshots yet. Capture from Live or File tabs.</div>
    ...
    <template v-else>
      ...
      <div v-show="activeTab === 'file'" class="panel">
        <FileAnalyzer ref="fileRef" :mode-prompt="modePrompt" @ai-response="recordResponse" @frame-captured="addFrame" @error="onError" />
      </div>
      ...
      <div v-show="activeTab === 'gallery'" class="panel glass-surface">
        <div class="gallery">
          <div v-if="!frames.length" class="muted">No snapshots yet. Capture from Live or File tabs.</div>
          <div v-else class="grid">
            <div v-for="f in frames" :key="f.id" class="thumb" :class="{ annotating: annotationMode }">
              <img :src="f.url" alt="snapshot" />
              <div class="thumb-meta">
                <span>{{ formatTime(f.timestamp) }}</span>
...
type ResponseType = 'analysis' | 'chat' | 'realtime' | 'file' | 'image' | 'video'
interface SessionResponse { id: string; type: ResponseType; content: string; timestamp: Date; confidence?: number }
interface FrameItem { id: string; url: string; timestamp: Date; source?: string }
...
const sessionId = ref(`media-${Date.now()}-${Math.random().toString(36).slice(2,6)}`)
...
const emit = defineEmits<{ 'session-updated': [payload: { responses: number; frames: number; avgConfidence: number }] }>()
const responses = ref<SessionResponse[]>([])
const frames = ref<FrameItem[]>([])

function announce() {
  emit('session-updated', { responses: responses.value.length, frames: frames.value.length, avgConfidence: averageConfidence.value })
}

function recordResponse(r: SessionResponse) { responses.value.push(r); announce() }
function addFrame(url: string) { frames.value.push({ id: `f-${Date.now()}-${Math.random().toString(36).slice(2,6)}`, url, timestamp: new Date() }); announce() }
function onError(msg: string) { console.warn('[AI Media Workbench] error:', msg) }

const averageConfidence = computed(() => {
  const vals = responses.value.map(r => r.confidence).filter(v => typeof v === 'number') as number[]
  if (!vals.length) return 0
  return Math.round((vals.reduce((a,b)=>a+b, 0) / vals.length) * 100)
})
const realtimeCount = computed(() => responses.value.filter(r => r.type === 'realtime').length)
const chatCount = computed(() => responses.value.filter(r => r.type === 'chat').length)

function formatTime(d: Date): string { return d.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' }) }
...
function downloadBlob(data: string | Blob | ArrayBuffer, filename: string, type = 'application/octet-stream') {
  try {
...
function exportJSON() {
  const bundle = {
    ...
    tab: activeTab.value,
    responses: responses.value.map(r => ({ id: r.id, type: r.type, content: r.content, timestamp: r.timestamp, confidence: r.confidence })),
    frames: frames.value.map(f => ({ id: f.id, timestamp: f.timestamp, url: f.url, source: f.source || '', annotation: annotations.value[f.id] || { labels: [], note: '' } })),
  }
  downloadBlob(JSON.stringify(bundle, null, 2), 'ai-media-session.json', 'application/json')
}
...
function persistSessionName() {
  try { localStorage.setItem(NAME_KEY.value, sessionName.value || '') } catch {}
}

function resetAnnotations() {
  try { annotations.value = {}; annoOpen.value = {}; annoDraft.value = {}; localStorage.removeItem(ANNO_KEY.value) } catch {}
}
...
function exportManifest() {
  const manifest = {
    ...
    frames: frames.value.map(f => ({ id: f.id, timestamp: f.timestamp, source: f.source || '', annotation: annotations.value[f.id] || { labels: [], note: '' } })),
    responses: responses.value.map(r => ({ id: r.id, timestamp: r.timestamp, type: r.type, confidence: r.confidence }))
  }
  downloadBlob(JSON.stringify(manifest, null, 2), 'ai-media-manifest.json', 'application/json')
}
...
defineExpose({
  ...
  async captureSnapshot() {
    const dataUrl = await (liveRef.value?.captureSnapshot?.() as Promise<string | null>)
...
watch([activeTab, activeMode, () => responses.value.length, () => frames.value.length], pushAssistantContext)
...
function saveAnno(id: string) {
  try {
    ...
    // persist
    try { localStorage.setItem(ANNO_KEY.value, JSON.stringify(annotations.value)) } catch {}
  } catch {}
}
...
onMounted(() => {
  ...
  try { sessionName.value = localStorage.getItem(NAME_KEY.value) || '' } catch {}
})
...
function onKeydown(e: KeyboardEvent) {
  ...
  // Switch tabs Alt+1..4
  if (e.altKey) {
    if (e.key === '1') { activeTab.value = 'live'; e.preventDefault() }
    if (e.key === '2') { activeTab.value = 'file'; e.preventDefault() }
    if (e.key === '3') { activeTab.value = 'gallery'; e.preventDefault() }
    if (e.key === '4') { activeTab.value = 'analytics'; e.preventDefault() }
    // Mode shortcuts Alt+Shift+1..4
    if (e.shiftKey) {
      if (e.key === '1') { activeMode.value = 'describe'; e.preventDefault() }
      if (e.key === '2') { activeMode.value = 'ocr'; e.preventDefault() }
      if (e.key === '3') { activeMode.value = 'safety'; e.preventDefault() }
      if (e.key === '4') { activeMode.value = 'uiqa'; e.preventDefault() }
    }
  }
}
