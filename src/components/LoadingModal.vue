    ...
    <div ref="modalEl" class="modal">
      ...
      <div class="modal__body">
        ...
        <div v-if="showProgress" class="progress">
          <div class="progress__bar" :style="{ width: computedProgress + '%' }" />
        </div>
...
const showProgress = computed(() => props.progress != null && !isNaN(props.progress))
...
function trapFocus(_e: Event) {
  ...
  if (!modalEl.value.contains(document.activeElement)) {
    const focusable = modalEl.value.querySelector<HTMLElement>('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])')
    focusable?.focus()
  }
}
...
watch(() => computedProgress.value, (val) => {
