<template>
  <div class="modal-overlay font-sans" role="dialog" aria-modal="true" aria-labelledby="success-modal-title" @click.self="emitClose" @keydown.esc.prevent.stop="emitClose">
...
function trapFocus(_e: Event) {
  ...
  if (!modalEl.value.contains(document.activeElement)) {
    const focusable = modalEl.value.querySelector<HTMLElement>('button, [href], [tabindex]:not([tabindex="-1"])')
    focusable?.focus()
  }
}
...
onMounted(() => {
  ...
  setTimeout(() => modalEl.value?.querySelector<HTMLElement>('button')?.focus(), 0)
