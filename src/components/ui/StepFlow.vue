<template>
  <div class="step-flow" :class="[orientationClass, compact ? 'is-compact' : '']" class="font-sans">
      ...
      <div class="sf-steps" :class="orientationClass">
        ...
        >
          ...
          <span class="sf-step-info">
            ...
            <span v-if="s.subtitle" class="sf-step-subtitle">{{ s.subtitle }}</span>
          </span>
        </button>
      </div>
    </div>
    ...
    <div class="sf-footer">
      ...
      <div class="center">
        <div class="sf-step-text">Step {{ currentIndex + 1 }} of {{ steps.length }}</div>
      </div>
      <div class="right">
        <UnifiedButton
          variant="ghost"
...
const maxIndex = computed(() => Math.max(props.maxReachable, currentIndex.value))
const activeStep = computed(() => steps.value[currentIndex.value])
const isLast = computed(() => currentIndex.value >= steps.value.length - 1)
const progress = computed(() => Math.round((currentIndex.value) / Math.max(1, steps.value.length - 1) * 100))
const canContinue = computed(() => activeStep.value?.canContinue ? !!activeStep.value.canContinue() : true)
const canFinish = computed(() => activeStep.value?.canContinue ? !!activeStep.value.canContinue() : true)
const orientationClass = computed(() => props.orientation === 'vertical' ? 'is-vertical' : 'is-horizontal')

function goTo(idx: number) { currentIndex.value = idx }
async function next() {
...
watch(() => props.steps, () => {
  if (currentIndex.value > steps.value.length - 1) currentIndex.value = steps.value.length - 1
})
</script>

<style scoped>
.step-flow { display: grid; gap: var(--spacing-4); }
.sf-header { display: grid; gap: var(--spacing-2); }
.sf-progress-track { height: 6px; border-radius: 999px; background: var(--glass-border); overflow: hidden; }
.sf-progress-fill { height: 100%; background: var(--gradient-primary); border-radius: inherit; transition: width .25s ease; }
.sf-steps { display: flex; gap: var(--spacing-2); flex-wrap: wrap; }
...
@media (max-width: 640px) {
  ...
  .sf-step-title { font-size: 0.9rem; }
  .sf-step-subtitle { display: none; }
}
</style>
