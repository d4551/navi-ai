            ...
            <div class="achievement-content">
              ...
              <div class="achievement-meta">
                ...
                <span class="status">{{
                  isEarned(a.id)
                    ? 'Unlocked'
                    : Math.round(getAchievementProgress(a)) + '%'
                }}</span>
              </div>
            </div>
          </div>
        ...
        <div class="quests">
          ...
          >
            ...
            <div class="quest-actions">
              <UnifiedButton
                v-if="!q.completed && q.progress >= 100"
                size="sm"
                variant="primary"
                @click="claim(q.id)"
                >Claim +{{ q.xp }} XP</UnifiedButton
              >
              <UnifiedButton
                v-else
                size="sm"
                ...
                >{{ q.completed ? 'Completed' : 'In Progress' }}</UnifiedButton
              >
            </div>
          </div>
        </div>
      ...
      <section class="glass p-glass-md gap-glass-md rounded-lg">
        ...
        <div class="leaderboard">
          <div class="xp-summary">
            ...
            <div class="xp">
              {{ levelInfo.currentXP }} XP ({{ levelInfo.progress }}%)
            </div>
            <div class="bar">
              <div
                class="fill"
          ...
          <div class="milestones">
            <div
              v-for="(m, i) in levelMilestones"
              :key="i"
              class="milestone"
              :class="{ reached: userXP >= m.min }"
            >
...
const getAchievementProgress = a => {
  ...
  const vals = Object.entries(req).map(([k, v]) =>
    Math.min(((stats[k] || 0) / v) * 100, 100)
  )
