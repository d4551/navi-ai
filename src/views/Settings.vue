  ...
  >
    ...
    <div class="settings-layout unified-container">
      ...
      <div
        ...
        style="padding: var(--card-padding); margin-bottom: var(--spacing-6)"
      >
        <ContentLoader
          :height="160"
          width="100%"
                ...
                >
                  <SystemConfigurationSection
                    ...
                    @update-fairy-setting="
                      (value) =>
                        store.updateSettings({ fairyBubbleSize: value })
                    "
                    @save="saveSettings"
                  />
                </div>
      ...
      <div
        v-show="isDirty"
        class="settings-save-bar section-card"
        style="
          position: fixed;
...
const saveSettings = async (settingsData) => {
  await store.saveSettings(settingsData);
};
...
const logoSrc = computed(() => {
  ...
  return theme.isDarkMode.value ? "/logoDark.svg" : "/logoLight.svg";
});
const appVersion = ref("0.0.1-alpha");
...
- **OS:** ${window.navigator.platform}
  ...
  const githubUrl = `https://github.com/your-repo/navi-game/issues/new?title=${issueTitle}&body=${issueBody}`;
