...
class GamingJobsService {
  ...
  private readonly GAMING_KEYWORDS = [
    'game', 'gaming', 'unity', 'unreal', 'gamedev', 'studio',
    'gameplay', 'level design', 'character', 'animation',
    'shader', 'rendering', 'multiplayer', 'mobile game',
    'console', 'pc gaming', 'esports', 'vr', 'ar',
    'game engine', 'game design', 'game art', 'game audio',
    'qa testing', 'game producer', 'technical artist',
    'gameplay programmer', 'game writer', 'narrative',
    'monetization', 'f2p', 'live ops', 'user acquisition'
  ]

  private readonly GAMING_COMPANIES = [
    'Epic Games', 'Riot Games', 'Blizzard', 'Activision',
    'Electronic Arts', 'EA', 'Ubisoft', 'Valve', 'Steam',
    'Nintendo', 'Sony Interactive', 'PlayStation', 'Xbox',
    'Microsoft Gaming', 'Bungie', 'Bethesda', 'CD Projekt',
    'Square Enix', 'Bandai Namco', 'Capcom', 'Konami',
    'Take-Two', 'Rockstar', '2K Games', 'Indie', 'Studio'
  ]

  async searchGamingJobs(filters: GamingJobFilters = {}): Promise<GamingJobSearchResult> {
    ...
    try {
      ...
      const recommendations = await this.generateRecommendations(enhancedJobs, filters)
  ...
  private filterGamingJobs(jobs: Job[], filters: GamingJobFilters): Job[] {
    return jobs.filter(job => {
      ...
      if (filters.gameEngines?.length && !this.matchesGameEngines(job, filters.gameEngines)) {
        return false
      }
      
      if (filters.studioTypes?.length && !this.matchesStudioType(job, filters.studioTypes)) {
        return false
      }
      
      if (filters.gameGenres?.length && !this.matchesGenres(job, filters.gameGenres)) {
        return false
      }
      
      if (filters.platforms?.length && !this.matchesPlatforms(job, filters.platforms)) {
        return false
      }
  ...
  private matchesGameEngines(job: Job, engines: string[]): boolean {
    const searchText = `${job.title} ${job.description || ''} ${job.technologies?.join(' ') || ''}`.toLowerCase()
  ...
  private matchesStudioType(job: Job, types: string[]): boolean {
    ...
    return types.some(type => {
      switch (type) {
        ...
        case 'indie':
          return searchText.includes('indie') || searchText.includes('independent')
        case 'mobile':
          return searchText.includes('mobile') || searchText.includes('ios') || searchText.includes('android')
        case 'startup':
          return searchText.includes('startup') || searchText.includes('early stage')
        default:
          return false
      }
    })
  ...
  private matchesPlatforms(job: Job, platforms: string[]): boolean {
    const searchText = `${job.title} ${job.description || ''}`.toLowerCase()
    return platforms.some(platform => searchText.includes(platform.toLowerCase()))
  }

  private isUrgentHiring(job: Job): boolean {
    const searchText = `${job.title} ${job.description || ''}`.toLowerCase()
    return searchText.includes('urgent') || 
           searchText.includes('immediate') || 
           searchText.includes('asap') ||
           searchText.includes('start immediately')
  }

  private async enhanceJobsWithAI(jobs: Job[], filters: GamingJobFilters): Promise<Job[]> {
    return jobs.map(job => ({
      ...job,
  ...
  private calculateMatchScore(job: Job, filters: GamingJobFilters): number {
    ...
    if (filters.workStyle === 'onsite' && !job.remote && !job.hybrid) score += 10
    
    // Technology match
    if (filters.gameEngines?.length && job.technologies?.length) {
      const matches = filters.gameEngines.filter(engine => 
        job.technologies.some(tech => tech.toLowerCase().includes(engine.toLowerCase()))
      )
  ...
  private detectStudioType(job: Job): string {
    ...
    if (this.GAMING_COMPANIES.slice(0, 10).some(c => company.includes(c.toLowerCase()))) {
      return 'aaa'
    }
    if (company.includes('indie') || company.includes('independent')) {
      return 'indie'
    }
    if (company.includes('mobile') || job.title.toLowerCase().includes('mobile')) {
      return 'mobile'
    }
  ...
  private detectGameGenres(job: Job): string[] {
    ...
    if (searchText.includes('fps') || searchText.includes('shooter')) genres.push('fps')
    if (searchText.includes('rpg') || searchText.includes('role playing')) genres.push('rpg')
    if (searchText.includes('strategy') || searchText.includes('rts')) genres.push('strategy')
    if (searchText.includes('mmo') || searchText.includes('multiplayer')) genres.push('mmo')
    if (searchText.includes('puzzle')) genres.push('puzzle')
    if (searchText.includes('platform') || searchText.includes('platformer')) genres.push('platformer')
  ...
  private detectPlatforms(job: Job): string[] {
    ...
    if (searchText.includes('pc') || searchText.includes('steam')) platforms.push('pc')
    if (searchText.includes('console') || searchText.includes('xbox') || searchText.includes('playstation')) platforms.push('console')
    if (searchText.includes('mobile') || searchText.includes('ios') || searchText.includes('android')) platforms.push('mobile')
    if (searchText.includes('vr') || searchText.includes('virtual reality')) platforms.push('vr')
    if (searchText.includes('web') || searchText.includes('browser')) platforms.push('web')
  ...
  private isFeaturedJob(job: Job): boolean {
    ...
    const isRecentlyPosted = new Date(job.postedDate).getTime() > Date.now() - (24 * 60 * 60 * 1000)
  ...
  private generateAnalytics(jobs: Job[]): GamingJobAnalytics {
    const salaries = jobs
      .map(job => typeof job.salary === 'object' ? (job.salary as any).min || 0 : 0)
      .filter(s => s > 0)
    
    const averageSalary = salaries.length ? salaries.reduce((a, b) => a + b) / salaries.length : 0
    ...
    const topSkills = Array.from(skillCounts.entries())
      ...
      .map(([skill, count]) => {
        ...
        const popularSkills = ['Unity', 'C#', 'Unreal', 'C++', 'JavaScript', 'Python', 'Blender', 'Maya']
        const emergingSkills = ['Rust', 'WebAssembly', 'AR', 'VR', 'Machine Learning', 'AI', 'Blockchain']
        const decliningSkills = ['Flash', 'ActionScript', 'DirectX 9']
        
        let growth = Math.floor(Math.random() * 10) - 2 // Base random growth -2% to +8%
        
        if (popularSkills.some(s => skill.toLowerCase().includes(s.toLowerCase()))) {
          growth += Math.floor(Math.random() * 8) + 2 // +2% to +10% additional
        }
        
        if (emergingSkills.some(s => skill.toLowerCase().includes(s.toLowerCase()))) {
          growth += Math.floor(Math.random() * 15) + 5 // +5% to +20% additional
        }
        
        if (decliningSkills.some(s => skill.toLowerCase().includes(s.toLowerCase()))) {
    ...
    const topCompanies = Array.from(companyCounts.entries())
      ...
      .map(([company, count]) => {
        ...
        const majorCompanies = ['Epic Games', 'Riot Games', 'Blizzard', 'Electronic Arts', 'Ubisoft', 'Valve']
        const growingCompanies = ['Discord', 'Roblox', 'Unity Technologies', 'Supercell', 'King']
        const consolidatingCompanies = ['Activision', 'Bethesda'] // Companies in transition
        
        let trend = 'stable'
        
        if (majorCompanies.some(c => company.toLowerCase().includes(c.toLowerCase()))) {
          trend = Math.random() > 0.3 ? 'growing' : 'stable'
        } else if (growingCompanies.some(c => company.toLowerCase().includes(c.toLowerCase()))) {
          trend = Math.random() > 0.2 ? 'growing' : 'stable'
        } else if (consolidatingCompanies.some(c => company.toLowerCase().includes(c.toLowerCase()))) {
    ...
    jobs.forEach(job => {
      ...
      const existing = locationCounts.get(location) || { count: 0, salaries: [] }
      existing.count++
    ...
    const locationHotspots = Array.from(locationCounts.entries())
      ...
      .map(([location, data]) => ({
        location,
        count: data.count,
        avgSalary: data.salaries.length ? data.salaries.reduce((a, b) => a + b) / data.salaries.length : 0
      }))
    ...
    const experienceDistribution = Array.from(expCounts.entries()).map(([level, count]) => ({
      level,
    ...
    return {
      ...
      salaryTrends: [
        ...
        { period: 'Last Month', value: Math.round(averageSalary * (0.97 + Math.random() * 0.06)) }, // ±3% variation
        { period: '3 Months Ago', value: Math.round(averageSalary * (0.92 + Math.random() * 0.10)) }, // ±5% variation
        { period: '6 Months Ago', value: Math.round(averageSalary * (0.88 + Math.random() * 0.14)) }, // ±7% variation
        { period: '1 Year Ago', value: Math.round(averageSalary * (0.82 + Math.random() * 0.18)) } // ±9% variation
      ],
      topSkills,
      topCompanies,
      locationHotspots,
  ...
  private async generateRecommendations(jobs: Job[], filters: GamingJobFilters): Promise<JobRecommendation[]> {
    return jobs
  ...
  private getSkillGaps(job: Job, filters: GamingJobFilters): string[] {
    ...
    const gameEngines = ['Unity', 'Unreal Engine', 'Unreal', 'Godot', 'CryEngine', 'Frostbite'];
    const jobDescription = (job.description || '').toLowerCase();
    
    for (const engine of gameEngines) {
      const engineLower = engine.toLowerCase();
      if (jobDescription.includes(engineLower) && 
    ...
    const programmingLanguages = ['C++', 'C#', 'Python', 'JavaScript', 'Java', 'Rust', 'Go'];
    for (const lang of programmingLanguages) {
      const langLower = lang.toLowerCase();
      if (jobDescription.includes(langLower) && 
    ...
    const specializations = ['AI/ML', 'Graphics', 'Networking', 'Backend', 'Frontend', 'DevOps', 'QA', 'UI/UX'];
    for (const spec of specializations) {
      if (jobDescription.includes(spec.toLowerCase()) && 
  ...
  private getExperienceLevelNumber(level: string): number {
    ...
    if (normalizedLevel.includes('entry') || normalizedLevel.includes('junior')) return 1;
    if (normalizedLevel.includes('mid') || normalizedLevel.includes('intermediate')) return 2;
    if (normalizedLevel.includes('senior')) return 3;
    if (normalizedLevel.includes('lead') || normalizedLevel.includes('principal')) return 4;
    if (normalizedLevel.includes('staff') || normalizedLevel.includes('architect')) return 5;
  ...
  private assessCareerImpact(job: Job): 'high' | 'medium' | 'low' {
    ...
    const isLeadRole = job.title.toLowerCase().includes('lead') || 
  ...
  private generateMarketTrends(jobs: Job[], analytics: GamingJobAnalytics): MarketTrend[] {
    ...
    trends.push({
      ...
      trend: remotePercentage > 40 ? 'rising' : remotePercentage > 20 ? 'stable' : 'declining',
    ...
    const aiJobs = jobs.filter(job => 
      ...
      (job.title + job.description).toLowerCase().includes('machine learning') ||
      (job.title + job.description).toLowerCase().includes('artificial intelligence')
    ).length
    ...
    // Game engine trends
    const unityJobs = jobs.filter(job => (job.title + job.description).toLowerCase().includes('unity')).length
    const unrealJobs = jobs.filter(job => (job.title + job.description).toLowerCase().includes('unreal')).length
    ...
    // VR/AR trend
    const vrJobs = jobs.filter(job => 
      (job.title + job.description).toLowerCase().includes('vr') ||
      (job.title + job.description).toLowerCase().includes('ar') ||
      (job.title + job.description).toLowerCase().includes('virtual reality') ||
      (job.title + job.description).toLowerCase().includes('augmented reality')
    ).length
    ...
    const highSalaryPercentage = Math.round((highSalaryJobs / jobs.length) * 100)
  ...
  async createJobAlert(name: string, filters: GamingJobFilters): Promise<GamingJobAlert> {
