...
export { getJobSources, testJobSource, getAllRegions, getAllCategories } from './jobService';
...
export async function searchJobsLegacy(params: LegacySearchParams = {}): Promise<JobSearchResult> {
  const res = await legacySearchJobs(params);
  // Normalize error shape for consistency
  const errors = Array.isArray(res.errors) ? res.errors : [];
  return {
    jobs: res.jobs || [],
    totalResults: typeof res.totalResults === 'number' ? res.totalResults : (res.jobs?.length || 0),
    sources: res.sources || [],
    errors,
 ...
 */
export async function searchJobsRefactored(filters: JobFilters): Promise<JobSearchResult> {
 ...
 */
export async function searchJobsUnified(args: { filters?: JobFilters; legacy?: LegacySearchParams } = {}): Promise<JobSearchResult> {
  if (args.filters) {
    try {
...
export function queryToJobFilters(query: Record<string, any> = {}): JobFilters {
  ...
  const title = (query.q || query.query || query.title) ? String(query.q || query.query || query.title) : undefined;
  const location = query.location ? String(query.location) : undefined;
  const remote = getBool(query.remote);
  const jobType = query.type || query.jobType ? String(query.type || query.jobType) as any : undefined;
  const experienceLevel = (query.exp || query.experience || query.experienceLevel)
...
export async function searchJobsByQuery(query: Record<string, any> = {}): Promise<JobSearchResult> {
