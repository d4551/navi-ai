 ...
 * Generic adapter for ATS platforms like Greenhouse, Lever, Recruitee, Workable, Ashby
 */
...
export class CompanyBoardProvider extends BaseJobProvider {
  ...
  constructor(config: CompanyBoardConfig, priority: number = 20) {
    super()
    this.config = config
    ...
    switch (config.type) {
      case 'greenhouse':
        this.baseUrl = `https://boards-api.greenhouse.io/v1/boards/${config.token}/jobs`
        break
      case 'lever':
        this.baseUrl = `https://api.lever.co/v0/postings/${config.token}?mode=json`
        break
      case 'recruitee':
        this.baseUrl = `https://api.recruitee.com/c/${config.token}/offers`
        break
      case 'workable':
        this.baseUrl = `https://apply.workable.com/api/v1/widget/accounts/${config.token}`
        break
      case 'ashby':
      ...
      case 'smartrecruiters':
        ...
        this.baseUrl = `https://api.smartrecruiters.com/v1/companies/${config.token}/postings`
        break
      case 'teamtailor':
        // Requires API key; headers will be added in request
        this.baseUrl = `https://api.teamtailor.com/v1/jobs`
        break
      case 'workday': {
        // Generic Workday CXS endpoint; token can be 'tenant' or 'tenant/site'
        const parts = String(config.token).split('/')
        const tenant = parts[0]
        const site = parts[1] || tenant
        this.baseUrl = `https://${tenant}.wd1.myworkdayjobs.com/wday/cxs/${tenant}/${site}/jobs`
        break
      }
      default:
  ...
  /**
   * Lightweight availability check. Returns true if the board endpoint is reachable
   * and not 404. Used to auto-disable retired or invalid tokens.
   */
  ...
  private parseTeamtailorJobs(data: any): Job[] {
    ...
    return items.map((entry: any) => {
      ...
      const loc = attr?.locations?.[0]?.name || 'Not specified'
      return {
        id: `teamtailor-${entry?.id || Math.random().toString(36)}`,
        title,
  ...
  private parseWorkdayJobs(data: any): Job[] {
    ...
    return postings.map((p: any) => {
      ...
      const loc = p.locations?.[0] || p.location || 'Not specified'
      const id = p.id || p.externalPath || Math.random().toString(36)
      return {
        id: `workday-${id}`,
        title,
        company: this.config.name,
