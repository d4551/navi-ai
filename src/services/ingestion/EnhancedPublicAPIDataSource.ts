...
export class EnhancedPublicAPIDataSource {
  ...
  async fetchData(job: IngestionJob): Promise<RawStudioData[]> {
    ...
    try {
      ...
      const [wikidataData, githubStudios, wikipediaStudios, dbpediaData] =
      ...
      if (wikidataData.status === 'fulfilled') {
        this.mergeStudiosWithGameData(
          wikidataData.value.studios,
          wikidataData.value.gamesData,
          allStudios,
          'wikidata'
        )
        job.progress = 25
      ...
      if (dbpediaData.status === 'fulfilled') {
        this.mergeStudiosWithGameData(
          dbpediaData.value.developers,
          dbpediaData.value.games,
          allStudios,
          'dbpedia'
        )
        job.progress = 100
      ...
      const mergedStudios = this.fuzzyMergeStudios(
        Array.from(allStudios.values())
      )
  ...
  private async fetchAllWikidataStudios(limit: number) {
    ...
    try {
      ...
      const processedPublishers = this.processWikidataPublishers(
        publishers,
        limit
      )
    ...
    for (const query of queries) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          for (const org of data.items.slice(
            0,
            Math.min(20, limit - allStudios.length)
          )) {
            const detailedStudio = await this.getEnhancedGitHubOrgDetails(
              org.login
            )
            if (detailedStudio) {
            ...
            await new Promise(resolve =>
              setTimeout(resolve, this._githubToken ? 100 : 2000)
            )
          }
        }
  ...
  private async fetchAllWikipediaStudios(
    limit: number
  ): Promise<RawStudioData[]> {
  ...
  private processWikidataStudios(
    studios: EnhancedWikidataStudio[],
    limit: number
  ): RawStudioData[] {
    return studios
      .slice(0, limit)
      .map(studio => ({
        ...
        location:
          studio.countryLabel?.value ||
          studio.headquartersLabel?.value ||
          'Unknown',
        websites: studio.officialWebsite?.value
          ? [studio.officialWebsite.value]
          : [],
        metadata: {
  ...
  private processWikidataPublishers(
    publishers: any[],
    limit: number
  ): RawStudioData[] {
    return publishers
  ...
  private processDBpediaStudios(
    developers: EnhancedDBpediaStudio[],
    limit: number
  ): RawStudioData[] {
    return developers
      .slice(0, limit)
      .map(developer => ({
        ...
        description:
          developer.abstract?.value || `Game development studio from DBpedia`,
  ...
  private async getEnhancedGitHubOrgDetails(
    orgName: string
  ): Promise<RawStudioData | null> {
    try {
      const [orgResponse, reposResponse] = await Promise.all([
        fetch(`${this.githubEndpoint}/orgs/${orgName}`, {
          headers: this.getGitHubHeaders(),
        }),
        fetch(
          `${this.githubEndpoint}/orgs/${orgName}/repos?type=all&per_page=100`,
          { headers: this.getGitHubHeaders() }
        ),
      ])
      ...
      const gameProjects = reposData.filter(
        repo =>
          repo.topics?.some(topic =>
            ['game', 'gaming', 'gamedev', 'unity', 'unreal'].includes(
              topic.toLowerCase()
            )
          ) ||
          repo.description?.toLowerCase().includes('game') ||
          ['javascript', 'c#', 'c++', 'unity'].includes(
  ...
  private async getEnhancedWikipediaPageDetails(
    title: string
  ): Promise<RawStudioData | null> {
    try {
      const [summaryResponse, detailsResponse] = await Promise.all([
        ...
        fetch(
          `${this.wikipediaEndpoint}?action=query&prop=extracts|pageprops|categories&exintro=true&explaintext=true&titles=${encodeURIComponent(title)}&format=json&origin=*`
        ),
      ])
      ...
      const pageDetails = detailsData
        ? (Object.values(detailsData.query?.pages || {})[0] as any)
        : null
      ...
      return {
        ...
        description:
          summaryData.extract || `Game development studio from Wikipedia`,
        location: 'Unknown',
        websites: [],
        metadata: {
          ...
          categories:
            pageDetails?.categories?.map((cat: any) => cat.title) || [],
  ...
  private mergeStudiosWithGameData(
    studios: RawStudioData[],
    gamesData: any[],
    allStudios: Map<string, RawStudioData>,
    sourceId: string
  ) {
  ...
  private mergeStudioData(
    existing: RawStudioData,
    newStudio: RawStudioData,
    sourceId: string
  ) {
    // Merge websites
    existing.websites = [
      ...new Set([...(existing.websites || []), ...(newStudio.websites || [])]),
    ]
    ...
    if (
      ...
      newStudio.description.length > (existing.description?.length || 0)
    ) {
  ...
  getSourceInfo() {
    return {
      ...
      description:
        'Comprehensive multi-API aggregation with fuzzy matching and game data correlation',
