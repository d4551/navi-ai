...
export class PublicAPIDataSource {
  ...
  async fetchData(job: IngestionJob): Promise<RawStudioData[]> {
    ...
    try {
      ...
      const wikipediaStudios = await this.fetchFromWikipedia(maxStudiosPerSource);
      ...
      logger.info(`Public API ingestion completed: ${allStudios.size} studios found`);
    ...
    try {
      ...
      for (const binding of data.results?.bindings || []) {
        const studio: RawStudioData = {
          ...
          description: binding.description?.value || `Game development studio from Wikidata`,
  ...
  private async fetchFromGitHub(limit: number): Promise<RawStudioData[]> {
    try {
      ...
      const queries = ['game+studio', 'game+development', 'gamedev', 'indie+games'];
      const studios: RawStudioData[] = [];
      ...
      for (const query of queries) {
        ...
        for (const org of data.items || []) {
          ...
          try {
            ...
            const orgResponse = await fetch(`${this.githubEndpoint}/orgs/${org.login}`);
  ...
  private async fetchFromWikipedia(limit: number): Promise<RawStudioData[]> {
    try {
      const searchQueries = ['game studio', 'video game developer', 'indie game developer'];
      const studios: RawStudioData[] = [];
