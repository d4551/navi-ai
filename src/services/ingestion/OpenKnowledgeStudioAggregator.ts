...
class WikipediaAPI {
  ...
  async getGameDevelopers() {
    const params = new URLSearchParams({
      ...
      cmtitle: 'Category:Video_game_development_companies',
      cmlimit: '300',
      format: 'json',
      origin: '*',
    })
...
class GitHubAPI {
  ...
  async getOrgDetails(login: string) {
    try {
      const res = await fetch(`${this.baseUrl}/orgs/${login}`, {
...
export class OpenKnowledgeStudioAggregator {
  ...
  async fetchWikipedia(): Promise<OpenStudio[]> {
    ...
    for (const item of devs.slice(0, 60)) {
      // cap for UX
    ...
    for (const s of studios) {
      ...
      if (map.has(key)) {
        ...
        map.set(key, {
          ...
          ...s,
          sources: Array.from(
            new Set([...(prev.sources || []), ...(s.sources || [])])
          ),
  ...
  async aggregateAll(
    options: {
      ...
      include?: Array<'wikidata' | 'wikipedia' | 'dbpedia' | 'github'>
    } = {}
  ) {
    ...
    const chunks = settled.flatMap(s =>
