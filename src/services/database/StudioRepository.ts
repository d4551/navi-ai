...
export class StudioRepository {
   ...
   */
  async upsert(studio: Omit<Studio, 'createdAt' | 'updatedAt'>): Promise<Studio> {
   ...
   */
  async bulkUpsert(studios: Omit<Studio, 'createdAt' | 'updatedAt'>[]): Promise<number> {
    const db = databaseManager.getDb();
    const now = new Date().toISOString();
    ...
    const transaction = db.transaction((studios: Omit<Studio, 'createdAt' | 'updatedAt'>[]) => {
      let insertedCount = 0;
  ...
  async getStats(): Promise<StudioStats> {
    ...
    const total = db.prepare('SELECT COUNT(*) as count FROM studios').get() as { count: number };
    
    const byType = db.prepare('SELECT type, COUNT(*) as count FROM studios GROUP BY type').all() as { type: string, count: number }[];
    const byLocation = db.prepare('SELECT location, COUNT(*) as count FROM studios GROUP BY location').all() as { location: string, count: number }[];
    ...
    const lastUpdated = db.prepare('SELECT MAX(updated_at) as last_updated FROM studios').get() as { last_updated: string };
