...
export class DatabaseManager {
  ...
  private async writeBackupMeta(meta: { lastBackup: string; lastBackupPath?: string }): Promise<void> {
    if (!this.isElectron()) {
  ...
  async getBackupInfo(): Promise<{ lastBackup: string; lastBackupPath?: string }> {
    try {
      const fs = await import('fs');
      const metaPath = await this.getMetaPath();
  ...
  private async resolveDbPath(): Promise<string> {
    ...
    try {
      ...
      if (isElectron) {
        ...
        const userDataPath = electronApp?.getPath
          ...
          : './data';
        const pathMod = await import('path');
        return pathMod.default?.join(userDataPath, 'navi.db') || pathMod.join(userDataPath, 'navi.db');
      }
    } catch {
      // Fallthrough
    }
    ...
    const pathMod = await import('path');
    return pathMod.default?.join(base, 'navi.db') || pathMod.join(base, 'navi.db');
  }
  ...
  async init(): Promise<void> {
    try {
      ...
      if (this.isElectron()) {
        ...
        logger.info(`Browser environment detected, skipping directory creation for: ${this.dbPath}`);
      }
  ...
  async getStats(): Promise<{
    ...
    const studioCount = this.db.prepare('SELECT COUNT(*) as count FROM studios').get() as { count: number };
    ...
    const lastStudioUpdate = this.db.prepare('SELECT MAX(updated_at) as last_updated FROM studios').get() as { last_updated: string };
  ...
  async backup(backupPath: string): Promise<void> {
    ...
    if (!this.isElectron()) {
      throw new Error('Database backup is only available in Electron environment');
    }
    ...
    await this.writeBackupMeta({ lastBackup: new Date().toISOString(), lastBackupPath: backupPath });
  }
