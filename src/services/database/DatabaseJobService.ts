...
if (typeof window !== 'undefined') {
  console.warn('DatabaseJobService is not supported in browser environments - operations will be no-ops');
}
...
export class DatabaseJobService {
  ...
  async importAllJobs(): Promise<ImportResult> {
    ...
    try {
      ...
      logger.info(`Fetching jobs from ${searchQueries.length} different search queries...`);
      ...
      for (const searchQuery of searchQueries) {
        try {
          ...
          logger.info(`Found ${jobSearchResult.jobs.length} jobs for query: ${searchQuery.query}`);

          // Convert API jobs to our database format
          for (const apiJob of jobSearchResult.jobs) {
            try {
              const dbJob: Omit<Job, 'createdAt' | 'updatedAt'> = {
                id: apiJob.id || `${apiJob.company.replace(/[^a-z0-9]/gi, '-')}-${apiJob.title.replace(/[^a-z0-9]/gi, '-')}-${Date.now()}`,
                title: apiJob.title,
                ...
                posted_at: apiJob.posted_at || apiJob.datePosted || new Date().toISOString(),
                scraped_at: new Date().toISOString()
              };

              jobsToImport.push(dbJob);
            } catch (conversionError) {
              result.errors.push(`Failed to convert job ${apiJob.title} at ${apiJob.company}: ${conversionError.message}`);
            }
          }
          ...
          result.errors.push(`Search failed for query "${searchQuery.query}": ${searchError.message}`);
          logger.warn(`Search failed for query "${searchQuery.query}":`, searchError);
        }
      }

      logger.info(`Collected ${jobsToImport.length} real job postings from APIs`);
      ...
      logger.info(`After deduplication: ${finalJobs.length} unique jobs to import`);

      if (finalJobs.length > 0) {
        logger.info(`Bulk importing ${finalJobs.length} jobs to SQLite database...`);
      ...
      logger.info(`Database job import completed: ${result.imported}/${result.total} successful in ${result.duration}ms`);
      return result;
