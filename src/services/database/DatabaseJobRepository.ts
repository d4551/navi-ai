...
if (typeof window !== 'undefined') {
  console.warn('JobRepository is not supported in browser environments - operations will be no-ops');
}
...
export class DatabaseJobRepository {
  ...
  async upsert(job: Omit<Job, 'createdAt' | 'updatedAt'>): Promise<Job> {
    ...
    if (existing) {
      ...
      updateStmt.run(
        dbJob.title, dbJob.company, dbJob.location, dbJob.description,
        dbJob.requirements, dbJob.salary_range, dbJob.remote ? 1 : 0, dbJob.type,
        dbJob.source, dbJob.source_url, dbJob.posted_at, dbJob.scraped_at,
        now, job.id
      );
      ...
      insertStmt.run(
        dbJob.id, dbJob.title, dbJob.company, dbJob.location,
        dbJob.description, dbJob.requirements, dbJob.salary_range,
        dbJob.remote ? 1 : 0, dbJob.type, dbJob.source, dbJob.source_url,
        dbJob.posted_at, dbJob.scraped_at, now, now
      );
   ...
   */
  async bulkUpsert(jobs: Omit<Job, 'createdAt' | 'updatedAt'>[]): Promise<number> {
    const db = databaseManager.getDb();
    const now = new Date().toISOString();
    ...
    const transaction = db.transaction((jobs: Omit<Job, 'createdAt' | 'updatedAt'>[]) => {
      let insertedCount = 0;
      ...
      for (const job of jobs) {
        try {
          insertStmt.run(
            ...
            job.description, JSON.stringify(job.requirements), job.salary_range,
            job.remote ? 1 : 0, job.type, job.source, job.source_url,
            job.posted_at, job.scraped_at, now, now
          );
          insertedCount++;
        } catch (error) {
  ...
  async getStats(): Promise<JobStats> {
    ...
    const byType = db.prepare('SELECT type, COUNT(*) as count FROM jobs GROUP BY type').all() as { type: string, count: number }[];
    const byLocation = db.prepare('SELECT location, COUNT(*) as count FROM jobs GROUP BY location').all() as { location: string, count: number }[];
    const bySource = db.prepare('SELECT source, COUNT(*) as count FROM jobs GROUP BY source').all() as { source: string, count: number }[];
    const byCompany = db.prepare('SELECT company, COUNT(*) as count FROM jobs GROUP BY company ORDER BY count DESC LIMIT 20').all() as { company: string, count: number }[];

    const lastUpdated = db.prepare('SELECT MAX(updated_at) as last_updated FROM jobs').get() as { last_updated: string };
    ...
    return {
      ...
      bySource: Object.fromEntries(bySource.map(row => [row.source, row.count])),
      byCompany: Object.fromEntries(byCompany.map(row => [row.company, row.count])),
