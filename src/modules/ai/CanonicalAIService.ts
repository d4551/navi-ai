...
class CanonicalAIService {
  ...
  async initialize(config: Partial<AIConfig>): Promise<void> {
    ...
    try {
      if (this.config.geminiApiKey && (this.config.primaryProvider === 'google')) {
  ...
  async generateText(prompt: string, options: {
    ...
    try {
      ...
      if (options.conversationId && this.config!.enableContextPersistence) {
        ...
        if (history.length > 0) {
          const contextPrompt = history.map(msg => `${msg.role}: ${msg.content}`).join('\n');
  ...
  async generateMultimodal(input: MultimodalInput, options: {
    model?: string;
    conversationId?: string;
  } = {}): Promise<AIResponse> {
  ...
  async startRealTimeSession(sessionId: string, options: {
    ...
    try {
      ...
      const sessionMonitor = setInterval(() => {
        ...
        if (currentTime - sessionData.lastActivity > 300000) { // 5 minutes timeout
      ...
      if (options.enableAudio || options.enableVideo) {
        try {
          ...
          logger.warn(`Media capabilities initialization warning for session ${sessionId}:`, mediaError);
  ...
  private async initializeMediaCapabilities(sessionId: string, options: {
    enableAudio?: boolean;
    enableVideo?: boolean;
  }): Promise<void> {
  ...
  async processAudio(audioData: ArrayBuffer, options: {
    format?: 'wav' | 'mp3' | 'webm';
    ...
    try {
      ...
      const prompt = options.transcribeOnly 
        ...
        : options.systemPrompt || 'Please transcribe this audio and provide a helpful response.';
  ...
  async healthCheck(): Promise<{ status: 'healthy' | 'degraded' | 'unhealthy'; details: any }> {
    ...
    try {
      ...
      const testResult = await this.generateText('Hello', { model: 'gemini-1.5-flash' });
      ...
      if (this.config?.enableContextPersistence) {
        try {
          ...
          await this.generateText('Remember this number: 42', { conversationId: contextId });
          const contextResult = await this.generateText('What number should I remember?', { conversationId: contextId });
          testResults.contextPersistence = contextResult.success && 
            contextResult.content?.toLowerCase().includes('42');
          if (!testResults.contextPersistence) {
            testResults.errors.push('Context persistence: Failed to maintain conversation context');
          }
      ...
      const allPassed = testResults.basicGeneration && testResults.streamingCapability && 
                       testResults.multimodalCapability && testResults.contextPersistence;
