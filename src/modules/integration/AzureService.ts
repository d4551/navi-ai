...
class AzureService {
  ...
  private async validateConfiguration(): Promise<void> {
    ...
    if (this.config.cognitiveServicesKey && this.config.cognitiveServicesEndpoint) {
  ...
  private async testCognitiveServices(): Promise<void> {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Cognitive Services test failed: ${response.statusText}`);
      }
  ...
  async analyzeText(text: string, features: string[] = ['sentiment', 'keyPhrases', 'entities']): Promise<AzureResponse<CognitiveServiceResponse>> {
  ...
  private async analyzeSentiment(text: string): Promise<AzureResponse<any>> {
    ...
    if (!response.ok) {
      return { success: false, error: result.error?.message || 'Sentiment analysis failed' };
    }
  ...
  private async extractKeyPhrases(text: string): Promise<AzureResponse<string[]>> {
    ...
    if (!response.ok) {
      return { success: false, error: result.error?.message || 'Key phrase extraction failed' };
    }
  ...
  private async recognizeEntities(text: string): Promise<AzureResponse<any[]>> {
    ...
    if (!response.ok) {
      return { success: false, error: result.error?.message || 'Entity recognition failed' };
    }
  ...
  async generateText(prompt: string, options: {
    ...
    try {
      ...
      if (!response.ok) {
        return { success: false, error: result.error?.message || 'OpenAI request failed' };
      }
  ...
  // Storage Methods
  async uploadBlob(containerName: string, blobName: string, data: Blob): Promise<AzureResponse<string>> {
  ...
  async downloadBlob(containerName: string, blobName: string): Promise<AzureResponse<Blob>> {
    if (!this.config?.storageAccountName) {
      return { success: false, error: 'Azure Storage not configured' };
    }
  ...
  // Event Hub Methods
  async sendEvent(eventData: any, partitionKey?: string): Promise<AzureResponse<void>> {
  ...
  // IoT Hub Methods
  async sendTelemetry(deviceId: string, telemetryData: any): Promise<AzureResponse<void>> {
  ...
  async invokeDirectMethod(deviceId: string, methodName: string, payload?: any): Promise<AzureResponse<any>> {
    if (!this.config?.iotHubConnectionString) {
      return { success: false, error: 'IoT Hub not configured' };
    }
    ...
    try {
      // This would use IoT Hub SDK in a real implementation
      logger.debug(`Invoking method ${methodName} on device ${deviceId}:`, payload);
  ...
  async setSecret(secretName: string, secretValue: string): Promise<AzureResponse<void>> {
    if (!this.config?.keyVaultUrl) {
      return { success: false, error: 'Key Vault not configured' };
    }
  ...
  async healthCheck(): Promise<{ status: 'healthy' | 'degraded' | 'unhealthy'; services: Record<string, boolean> }> {
    ...
    const totalConfigured = Object.entries(this.config || {}).filter(([, value]) => !!value).length / 2; // Rough estimate
