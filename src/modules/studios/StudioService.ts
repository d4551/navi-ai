...
export type Platform = 'PC' | 'Console' | 'Mobile' | 'VR' | 'AR' | 'Web' | 'Switch' | 'PlayStation' | 'Xbox' | 'Steam';
...
class CanonicalStudioService {
  ...
  async searchStudios(filters: StudioFilters = {}): Promise<StudioSearchResult> {
    ...
    try {
      ...
      if (filters.size) {
        studios = studios.filter(studio => String((studio as any).size || '').toLowerCase() === String(filters.size).toLowerCase());
      }
      ...
      if (filters.remoteWork !== undefined) {
        studios = studios.filter(studio => {
          ...
          return (typeof rw === 'boolean' ? rw : undefined) === filters.remoteWork
            || (typeof remoteFirst === 'boolean' ? remoteFirst : undefined) === filters.remoteWork;
        });
      }
        ...
        studios = studios.filter(studio => 
          filters.technologies!.some(tech => 
            (studio as any).technologies?.some((studioTech: string) => 
              String(studioTech).toLowerCase().includes(String(tech).toLowerCase())
            )
          )
        );
      }
  ...
  async ingestOpenData(openStudios: Array<{ name: string; website?: string; founded?: any; country?: string; headquarters?: string; summary?: string; employees?: any; sources?: string[] }>): Promise<{ created: number; updated: number; total: number }> {
    try {
      ...
      for (const inbound of openStudios) {
        ...
        if (existing) {
          ...
          const updates: any = {}
          if (!existing.description && inbound.summary) updates.description = inbound.summary
          if (!existing.headquarters && inbound.headquarters) updates.headquarters = inbound.headquarters
          if (!existing.location && inbound.country) updates.location = inbound.country
          if (!existing.size && inbound.employees) updates.size = String(inbound.employees)
          if ((!existing.websites || (existing.websites as any).length === 0) && inbound.website) updates.websites = [inbound.website]
          if (!existing.founded && inbound.founded) updates.founded = inbound.founded
          ...
          if (Object.keys(updates).length > 0) {
            try {
              ...
              logger.warn('Studio update failed (ingest):', (existing as any).name, e)
            }
          }
        } else {
          ...
          const record: any = {
            ...
            reputation: {},
            hiringData: {},
            dataSource: Array.isArray(inbound.sources) ? inbound.sources : ['open-knowledge']
          }
          try {
