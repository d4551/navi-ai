...
export class GameStudioRepository {
  ...
  private static async ensureInitialized(): Promise<void> {
    ...
    if (
      !existing ||
      Object.keys(existing).length < Object.keys(GAMING_STUDIOS).length
    ) {
  ...
  static async search(
    criteria: GameStudioSearchCriteria,
  ): Promise<GameStudio[]> {
    ...
    if (criteria.name) {
      ...
      filtered = filtered.filter(
        (studio) =>
          ...
          (studio.description &&
            studio.description.toLowerCase().includes(nameQuery)) ||
          studio.technologies.some((tech) =>
            tech.toLowerCase().includes(nameQuery),
          ),
      );
    }
    ...
    if (criteria.platforms && criteria.platforms.length > 0) {
      ...
      filtered = filtered.filter((studio) => {
        ...
        return criteria.platforms!.some((platform) =>
          studioPlatforms.includes(platform),
        );
      });
    }
  ...
  private static getStudioPlatforms(studio: GameStudio): string[] {
    ...
    if (
      desc.includes("console") ||
      games.includes("console") ||
      games.includes("playstation") ||
      games.includes("xbox") ||
      games.includes("switch")
    )
      platforms.push("Console");
    if (desc.includes("mobile") || games.includes("mobile"))
  ...
  static async getSuggestions(
    ...
    const allStudios = Object.values(await this.getAll());
    const matching = allStudios.filter(
      (studio) =>
  ...
  static async getByCategory(category: string): Promise<GameStudio[]> {
    ...
    return allStudios.filter(
      (studio) => this.getStudioCategory(studio) === category.toLowerCase(),
    );
  }

  private static getStudioCategory(studio: GameStudio): string {
    const size = studio.size.toLowerCase();
    if (size.includes("5000") || size.includes("enterprise"))
      return "enterprise";
  ...
  static async getByRegion(region: string): Promise<GameStudio[]> {
    ...
    return allStudios.filter((studio) => {
      ...
      if (
        (normalizedRegion === "north america" && location.includes("ca")) ||
      ...
      if (
        ...
        (location.includes("japan") || location.includes("china"))
      )
        return true;
      return false;
    });
  ...
  static async getStudiosByAdvancedFilters(filters: {
    ...
    if (filters.minSize !== undefined) {
      studios = studios.filter(
        (studio) => this.extractStudioSize(studio) >= filters.minSize!,
      );
    }

    if (filters.maxSize !== undefined) {
      studios = studios.filter(
        (studio) => this.extractStudioSize(studio) <= filters.maxSize!,
      );
    }

    if (filters.hasRemoteWork !== undefined) {
      studios = studios.filter(
        (studio) => (studio as any).remoteWork === filters.hasRemoteWork,
      );
    }
    ...
    if (filters.technologies && filters.technologies.length > 0) {
      studios = studios.filter((studio) =>
        filters.technologies!.every((tech) =>
          studio.technologies.some(
            (studioTech) => studioTech.toLowerCase() === tech.toLowerCase(),
          ),
        ),
      );
    }
    ...
    if (filters.foundedAfter !== undefined) {
      studios = studios.filter(
        (studio) => studio.founded && studio.founded >= filters.foundedAfter!,
      );
    }

    if (filters.foundedBefore !== undefined) {
      studios = studios.filter(
        (studio) => studio.founded && studio.founded <= filters.foundedBefore!,
      );
    }
