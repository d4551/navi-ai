...
export class AIInterviewPreparationService {
  ...
  async generateQuestions(
    ...
    allQuestionTexts
      .slice(0, Math.ceil(request.duration / 5))
...
Provide feedback in JSON format:
{
  ...
  "responseTime": "appropriate"
    ...
    const averageScore =
      questionFeedbacks.length > 0
        ? questionFeedbacks.reduce((sum, f) => sum + f.score, 0) /
  ...
  private calculateCommunicationScore(
    ...
    const avgConfidence =
      session.userResponses.reduce((sum, r) => sum + r.confidence, 0) /
