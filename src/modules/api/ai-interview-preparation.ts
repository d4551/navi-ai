...
/**
 * Enhanced AI Interview Preparation API
 * Provides comprehensive AI-powered interview preparation tools
 * Integrates with job applications and user profile data
 */
...
export interface UserResponse {
  ...
  timestamp: Date
  audioRecording?: string // base64 encoded audio for practice
  notes?: string
}
...
export interface InterviewFeedback {
  ...
  gameDevSpecific?: {
    industryKnowledge: number // 1-10
    portfolioPresentation: number // 1-10
    gameDesignThinking: number // 1-10
...
export class AIInterviewPreparationService {
  ...
  private readonly QUESTION_BANKS: Record<string, string[]> = {
    ...
    coding: [
      ...
      'Write a function to detect collision between two rectangles',
  ...
  static getInstance(): AIInterviewPreparationService {
    if (!AIInterviewPreparationService.instance) {
      AIInterviewPreparationService.instance = new AIInterviewPreparationService()
    }
  ...
  async createSession(request: InterviewPreparationRequest): Promise<InterviewPreparationSession> {
  ...
  async generateQuestions(request: InterviewPreparationRequest): Promise<InterviewQuestion[]> {
    ...
    const allQuestionTexts = [...bankQuestions, ...contextualQuestions, ...(request.customQuestions || [])]
    
    allQuestionTexts.slice(0, Math.ceil(request.duration / 5)).forEach((questionText, index) => {
      questions.push({
        id: `q_${Date.now()}_${index}`,
        type: request.interviewType,
        category: this.determineQuestionCategory(questionText, request.interviewType),
        question: questionText,
        expectedDuration: 5,
        difficulty: request.difficulty,
        gameDevSpecific: request.includeGameDevQuestions && this.isGameDevSpecific(questionText),
        keyPoints: this.generateKeyPoints(questionText),
        evaluationCriteria: this.generateEvaluationCriteria(questionText, request.interviewType)
      })
    })
  ...
  private async generateContextualQuestions(request: InterviewPreparationRequest): Promise<string[]> {
    try {
      ...
      if (request.applicationId) {
        try {
          const application = await jobApplicationTrackingService.getApplication(request.applicationId)
          if (application) {
...
Return only the questions, one per line.`
  ...
  async submitResponse(sessionId: string, questionId: string, response: string, confidence: number): Promise<void> {
  ...
  async completeSession(sessionId: string): Promise<InterviewPreparationSession> {
    const session = await this.getSession(sessionId)
    if (!session) {
      throw new Error(`Session with ID ${sessionId} not found`)
    }
  ...
  private async generateFeedback(session: InterviewPreparationSession): Promise<InterviewFeedback> {
...
Provide feedback in JSON format:
{
  ...
  "responseTime": "appropriate"
        ...
        questionFeedbacks.push({
          ...
          suggestions: ['Review the question and consider more detailed examples'],
          missedKeyPoints: [],
          responseTime: 'appropriate'
        })
      }
    ...
    const averageScore = questionFeedbacks.length > 0 
      ? questionFeedbacks.reduce((sum, f) => sum + f.score, 0) / questionFeedbacks.length 
      : 0
    ...
    return {
      overall: {
        ...
        recommendations: Array.from(new Set(allSuggestions)).slice(0, 5)
      },
      ...
      technicalCompetency: session.interviewType === 'technical' ? {
        accuracy: this.calculateTechnicalScore(questionFeedbacks, 'accuracy'),
        depth: this.calculateTechnicalScore(questionFeedbacks, 'depth'),
        problemSolving: this.calculateTechnicalScore(questionFeedbacks, 'problemSolving'),
        bestPractices: this.calculateTechnicalScore(questionFeedbacks, 'bestPractices')
      } : undefined,
      gameDevSpecific: session.questions.some(q => q.gameDevSpecific) ? {
        industryKnowledge: this.calculateGameDevScore(questionFeedbacks, 'industryKnowledge'),
        portfolioPresentation: this.calculateGameDevScore(questionFeedbacks, 'portfolioPresentation'),
        gameDesignThinking: this.calculateGameDevScore(questionFeedbacks, 'gameDesignThinking'),
        technicalSkills: this.calculateGameDevScore(questionFeedbacks, 'technicalSkills')
      } : undefined
    }
  }
  ...
  async getUserSessions(limit = 20): Promise<InterviewPreparationSession[]> {
    if (!this.db) await this.initializeDatabase()
  ...
  async getAnalytics(): Promise<InterviewAnalytics> {
    ...
    const analytics: InterviewAnalytics = {
      ...
      averageScore: completedSessions.reduce((sum, s) => sum + s.score, 0) / completedSessions.length,
      ...
      mostDifficultQuestions: this.identifyDifficultQuestions(completedSessions),
      preparationTime: completedSessions.reduce((sum, s) => sum + s.duration, 0),
  ...
  private async updateSession(_id: string, session: InterviewPreparationSession): Promise<void> {
    if (!this.db) await this.initializeDatabase()
  ...
  private async updateAnalytics(session: InterviewPreparationSession): Promise<void> {
    // Update daily analytics
    const dateKey = session.completedAt?.toISOString().split('T')[0] || new Date().toISOString().split('T')[0]
  ...
  private extractStrengths(feedbacks: QuestionFeedback[]): string[] {
    const highScoringFeedbacks = feedbacks.filter(f => f.score >= 80)
    const strengths: string[] = []
    
    highScoringFeedbacks.forEach(f => {
      if (f.feedback.toLowerCase().includes('clear')) strengths.push('Clear communication')
      if (f.feedback.toLowerCase().includes('detailed')) strengths.push('Detailed responses')
      if (f.feedback.toLowerCase().includes('example')) strengths.push('Good use of examples')
    })
  ...
  private extractWeaknesses(feedbacks: QuestionFeedback[]): string[] {
    const lowScoringFeedbacks = feedbacks.filter(f => f.score < 60)
    const weaknesses: string[] = []
    
    lowScoringFeedbacks.forEach(f => {
      if (f.missedKeyPoints.length > 0) weaknesses.push('Missing key technical points')
      if (f.feedback.toLowerCase().includes('vague')) weaknesses.push('Responses lack specificity')
      if (f.responseTime === 'too_fast') weaknesses.push('Responses may be too brief')
    })

    return Array.from(new Set(weaknesses)).slice(0, 3)
  }

  private calculateCommunicationScore(session: InterviewPreparationSession, _skill: string): number {
    // Simplified calculation - in a real implementation, this would use audio analysis
    const avgConfidence = session.userResponses.reduce((sum, r) => sum + r.confidence, 0) / session.userResponses.length
    return Math.round(avgConfidence * 0.8 + Math.random() * 2) // Add some variation
  }

  private calculateTechnicalScore(feedbacks: QuestionFeedback[], _aspect: string): number {
    const avgScore = feedbacks.reduce((sum, f) => sum + f.score, 0) / feedbacks.length
    return Math.round(avgScore / 10) // Convert to 1-10 scale
  }

  private calculateGameDevScore(feedbacks: QuestionFeedback[], _aspect: string): number {
    const gameDevFeedbacks = feedbacks.filter((_, _index) => Math.random() > 0.5) // Simulate game dev specific questions
    if (gameDevFeedbacks.length === 0) return 5
    
    const avgScore = gameDevFeedbacks.reduce((sum, f) => sum + f.score, 0) / gameDevFeedbacks.length
    return Math.round(avgScore / 10)
  }

  private calculateImprovementTrend(sessions: InterviewPreparationSession[]): Array<{ date: Date; score: number }> {
    return sessions
  ...
  private identifyStrengthAreas(sessions: InterviewPreparationSession[]): string[] {
  ...
  private identifyWeaknessAreas(sessions: InterviewPreparationSession[]): string[] {
    const typeScores: Record<string, number[]> = {}
    ...
    return Object.entries(typeScores)
      ...
      .filter(({ avgScore }) => avgScore < 60)
      .sort((a, b) => a.avgScore - b.avgScore)
      .map(({ type }) => type)
      .slice(0, 3)
  }

  private identifyDifficultQuestions(sessions: InterviewPreparationSession[]): InterviewQuestion[] {
    const questionScores: Record<string, { question: InterviewQuestion; scores: number[] }> = {}
    
    sessions.forEach(session => {
      session.feedback.byQuestion.forEach(feedback => {
        const question = session.questions.find(q => q.id === feedback.questionId)
        if (question) {
    ...
    return Object.values(questionScores)
      ...
      .filter(({ avgScore }) => avgScore < 50)
      .sort((a, b) => a.avgScore - b.avgScore)
      .slice(0, 5)
      .map(({ question }) => question)
  }

  private groupSessionsByType(sessions: InterviewPreparationSession[]): Record<InterviewType, number> {
    const groups: Record<InterviewType, number> = {} as Record<InterviewType, number>
    
    sessions.forEach(session => {
      groups[session.interviewType] = (groups[session.interviewType] || 0) + 1
    })

    return groups
  }

  private determineQuestionCategory(question: string, type: InterviewType): string {
    const questionLower = question.toLowerCase()
    
    if (type === 'behavioral') {
      if (questionLower.includes('team') || questionLower.includes('conflict')) return 'teamwork'
      if (questionLower.includes('challenge') || questionLower.includes('difficult')) return 'problem-solving'
      if (questionLower.includes('leadership') || questionLower.includes('lead')) return 'leadership'
      return 'general-behavioral'
    }
    
    if (type === 'technical') {
      if (questionLower.includes('algorithm') || questionLower.includes('data structure')) return 'algorithms'
      if (questionLower.includes('system') || questionLower.includes('architecture')) return 'system-design'
      if (questionLower.includes('performance') || questionLower.includes('optimization')) return 'optimization'
  ...
  private isGameDevSpecific(question: string): boolean {
    const gameDevKeywords = ['game', 'unity', 'unreal', 'shader', 'rendering', 'physics', 'ai', 'gameplay', 'engine', 'studio']
    return gameDevKeywords.some(keyword => question.toLowerCase().includes(keyword))
  }
  ...
  private generateKeyPoints(question: string): string[] {
    ...
    if (questionLower.includes('challenge') || questionLower.includes('difficult')) {
      return ['Describe the situation', 'Explain the challenge', 'Detail your approach', 'Share the outcome']
    }
    
    if (questionLower.includes('team')) {
      return ['Describe the team dynamics', 'Explain your role', 'Detail the collaboration', 'Share results']
    }
    
    return ['Provide context', 'Explain your approach', 'Share specific examples', 'Discuss the outcome']
  }

  private generateEvaluationCriteria(_question: string, type: InterviewType): string[] {
    const criteria: string[] = []
    
    if (type === 'behavioral') {
      criteria.push('Uses STAR method', 'Provides specific examples', 'Shows self-awareness', 'Demonstrates learning')
    } else if (type === 'technical') {
      criteria.push('Technical accuracy', 'Problem-solving approach', 'Code quality', 'Explains reasoning')
    } else {
      criteria.push('Clarity of response', 'Relevant examples', 'Structured thinking', 'Completeness')
    }
...
export const aiInterviewPreparationService = AIInterviewPreparationService.getInstance()
