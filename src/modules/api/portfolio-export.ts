...
export class PortfolioExportService {
   ...
   */
  async exportPortfolio(request: PortfolioExportRequest): Promise<PortfolioExportResponse> {
    try {
      logger.info(`Exporting portfolio to ${request.format} with ${request.projects.length} projects`)
   ...
   */
  private async exportToPDF(projects: PortfolioProject[], options?: PortfolioExportOptions): Promise<PortfolioExportResponse> {
    try {
      const htmlTemplate = this.generatePortfolioHTML(projects, options)
      
      // Use Electron PDF generation if available
      if (typeof window !== 'undefined' && (window as any).electronAPI?.portfolio?.exportPDF) {
   ...
   */
  private async exportToHTML(projects: PortfolioProject[], options?: PortfolioExportOptions): Promise<PortfolioExportResponse> {
    try {
   ...
   */
  private async exportToWebsite(projects: PortfolioProject[], options?: PortfolioExportOptions): Promise<PortfolioExportResponse> {
    try {
      ...
      throw new Error(`Website export failed: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
   ...
   */
  private async exportToJSON(projects: PortfolioProject[], options?: PortfolioExportOptions): Promise<PortfolioExportResponse> {
    try {
      const portfolioData = {
        metadata: {
   ...
   */
  private async exportToZip(projects: PortfolioProject[], options?: PortfolioExportOptions): Promise<PortfolioExportResponse> {
    try {
      ...
      // Add CSS files
      files['assets/styles.css'] = this.generatePortfolioCSS(options?.template, options?.theme)
   ...
   */
  private generatePortfolioHTML(projects: PortfolioProject[], options?: PortfolioExportOptions, standalone = false): string {
    ...
    const jsContent = standalone ? `<script>${this.generatePortfolioJS(options)}</script>` :
...
<body class="portfolio-template-${template} theme-${theme}">
   ...
   */
  private generateProjectHTML(project: PortfolioProject, options?: PortfolioExportOptions): string {
    const template = options?.template || 'modern'
    const theme = options?.theme || 'light'
...
<body class="portfolio-template-${template} theme-${theme}">
   ...
   */
  private generateProjectGrid(projects: PortfolioProject[], _options?: PortfolioExportOptions): string {
        ...
        <div class="project-info">
          ...
          <div class="project-metadata">
            <div class="gaming-badges">
              ${this.extractPlatforms(project.description).map(platform => 
                `<span class="badge platform-badge">${platform}</span>`).join('')}
              ${this.extractGameEngines(project.description).map(engine => 
                `<span class="badge engine-badge">${engine}</span>`).join('')}
            </div>
          </div>
        </div>
      </div>
  ...
  private extractPlatforms(description: string): string[] {
    const platforms = ['PC', 'Console', 'Mobile', 'VR', 'AR', 'Web', 'Switch', 'PlayStation', 'Xbox', 'Steam']
  ...
  private extractGameEngines(description: string): string[] {
    const engines = ['Unity', 'Unreal Engine', 'Godot', 'GameMaker', 'Construct', 'RPG Maker', 'Custom Engine']
  ...
  private extractGenres(description: string): string[] {
    const genres = ['RPG', 'FPS', 'Strategy', 'Puzzle', 'Platformer', 'Racing', 'Sports', 'Horror', 'Adventure']
  ...
  private getProjectCategory(project: PortfolioProject): string {
    ...
    if (description.includes('game') || description.includes('unity') || description.includes('unreal')) {
      return 'games'
    }
    if (description.includes('tool') || description.includes('editor') || description.includes('utility')) {
      return 'tools'
    }
    if (tech.includes('react') || tech.includes('vue') || tech.includes('web') || description.includes('website')) {
      return 'web'
    }
   ...
   */
  private async generateWebsiteFiles(projects: PortfolioProject[], options?: PortfolioExportOptions): Promise<Record<string, string | Blob>> {
   ...
   */
  private async createZipFromFiles(files: Record<string, string | Blob>): Promise<Blob> {
    // Simple ZIP implementation - in production, use JSZip library
    // For now, return a single file archive
    const content = Object.entries(files).map(([filename, content]) => 
   ...
   */
  private generateReadme(projects: PortfolioProject[], options?: PortfolioExportOptions): string {
    const metadata = options?.metadata || {}
...
`
   ...
   */
  private async addProjectAssets(files: Record<string, string | Blob>, projects: PortfolioProject[]): Promise<void> {
   ...
   */
  private async generatePDFInBrowser(htmlContent: string, projects: PortfolioProject[], options?: PortfolioExportOptions): Promise<PortfolioExportResponse> {
   ...
   */
  private generateFilename(baseName: string, extension: string, options?: PortfolioExportOptions): string {
...
export const portfolioExportRoutes = {
  async export(request: PortfolioExportRequest): Promise<PortfolioExportResponse> {
    return await portfolioExportService.exportPortfolio(request)
  },

  async downloadPortfolio(projects: PortfolioProject[], format: 'pdf' | 'zip' | 'html' | 'json' | 'website', options?: PortfolioExportOptions): Promise<void> {
    const result = await portfolioExportService.exportPortfolio({ projects, format, options })
