...
export class StudioDatabaseInitializer {
   ...
   */
  private async loadAdditionalStudioData(result: StudioInitResult): Promise<void> {
    try {
      ...
      if (Array.isArray(studiosData) && studiosData.length > 0) {
        ...
        logger.info(`📚 Loaded additional ${studiosData.length} studios from data file`);
      }
    } catch (error) {
  ...
  async validateStudioData(): Promise<{
    ...
    try {
      ...
      if (studioArray.length === 0) {
        ...
        validation.recommendations.push('Run studio initialization to populate database');
        return validation;
      }
      ...
      if (studiosWithoutGames > 0) {
        validation.issues.push(`${studiosWithoutGames} studios missing game information`);
      }

      if (studiosWithoutLocation > 0) {
        validation.issues.push(`${studiosWithoutLocation} studios missing location information`);
      }

      if (studiosWithoutDescription > 0) {
        validation.issues.push(`${studiosWithoutDescription} studios missing descriptions`);
      }

      // Recommendations
      if (studioArray.length < 50) {
        validation.recommendations.push('Consider expanding studio database for better coverage');
      }

      if (validation.issues.length === 0) {
        validation.recommendations.push('Studio database appears to be in good condition');
      }

      logger.info(`[STATS] Studio validation: ${studioArray.length} studios, ${validation.issues.length} issues found`);
    ...
    try {
      ...
      studioArray.forEach(studio => {
        if (studio.description) {
          ...
          if (desc.includes('rpg')) genreCount['RPG'] = (genreCount['RPG'] || 0) + 1;
          if (desc.includes('action')) genreCount['Action'] = (genreCount['Action'] || 0) + 1;
          if (desc.includes('strategy')) genreCount['Strategy'] = (genreCount['Strategy'] || 0) + 1;
          if (desc.includes('simulation')) genreCount['Simulation'] = (genreCount['Simulation'] || 0) + 1;
          if (desc.includes('mobile')) genreCount['Mobile'] = (genreCount['Mobile'] || 0) + 1;
        }
      });
      ...
      logger.info(`📈 Studio statistics generated: ${stats.total} total studios`);
  ...
  private extractCountry(location: string): string {
    ...
    if (loc.includes('uk') || loc.includes('united kingdom') || loc.includes('london')) return 'United Kingdom';
    ...
    if (loc.includes('china') || loc.includes('beijing') || loc.includes('shanghai')) return 'China';
    if (loc.includes('south korea') || loc.includes('korea') || loc.includes('seoul')) return 'South Korea';
   ...
   */
  async getStudioSuggestions(query: string, limit = 10): Promise<Array<{
    id: string;
    name: string;
    description: string;
    location: string;
    ...
    try {
      ...
      studioArray.forEach(studio => {
        ...
        // Check games match
        else if (studio.games?.some(game => game.toLowerCase().includes(searchQuery))) {
          matchType = 'game';
        }
