...
export const useThemeStore = defineStore('theme', () => {
  ...
  const currentPalette = computed(() => {
    return colorPalettes.value[config.value.primaryColor] || colorPalettes.value.blue;
  });
  ...
  const applyTheme = async () => {
    ...
    if (config.value.glassEffect) {
      root.style.setProperty('--glass-bg', effectiveTheme.value === 'dark' ? 'rgba(0, 0, 0, 0.2)' : 'rgba(255, 255, 255, 0.2)');
      root.style.setProperty('--glass-border', effectiveTheme.value === 'dark' ? 'rgba(255, 255, 255, 0.1)' : 'rgba(0, 0, 0, 0.1)');
      root.style.setProperty('--glass-shadow', effectiveTheme.value === 'dark' ? '0 8px 32px rgba(0, 0, 0, 0.3)' : '0 8px 32px rgba(0, 0, 0, 0.1)');
    }
  ...
  const getFontFamily = () => {
    switch (config.value.fontFamily) {
      ...
      case 'gaming': return 'var(--font-family-gaming)';
