...
interface CacheEntry {
  data: Partial<GameStudio["externalStats"]>;
  ts: number;
}
...
async function fetchJson<T = any>(url: string): Promise<T | null> {
  ...
  while (attempt <= MAX_RETRIES) {
    try {
      if (attempt > 0) {
        const backoff =
          BASE_BACKOFF * Math.pow(2, attempt - 1) + Math.random() * 200;
      ...
      if (attempt > MAX_RETRIES) {
        logger.warn(
          "StudioStatsService fetch failed",
          url,
          (e as Error).message,
        );
        return null;
...
export class StudioStatsService {
  ...
  static async fetchForStudio(
    studio: GameStudio,
  ): Promise<Partial<GameStudio["externalStats"]>> {
    ...
    try {
      if (studio.games?.length) {
        ...
        const q = encodeURIComponent(
          sample.split(/:|-/)[0].split(" ").slice(0, 2).join(" "),
        );
        const cheapUrl = `https:
    ...
    try {
      if (RAWG_API_KEY) {
        ...
        if (rawg?.results?.length) {
          ...
          stats.topPublishers = rawg.results
            .slice(0, 3)
            .map((p: any) => p.name);
...
export async function ensureStudioStats(studio: GameStudio) {
  try {
    if (!studio.externalStats) {
      ...
      Date.now() - new Date(studio.externalStats.lastUpdated || 0).getTime() >
      24 * 60 * 60 * 1000
    ) {
      // stale -> refresh in background (do not await)
      StudioStatsService.fetchForStudio(studio).then((fresh) => {
