...
export class CanonicalAIClientExtensions {
  ...
  private async initializeMultimodal(config: any): Promise<void> {
    try {
      ...
      if (config.audioConfig || config.multimodalConfig?.audioConfig) {
        ...
        audioService.on('audio-data', (audioData: Float32Array, sampleRate: number) => {
          if (multimodalClient) {
   ...
   */
  private parseError(error: any): { message: string; code?: string; details?: any } {
    if (error?.message) {
      ...
      if (error.message.includes('WebSocket')) {
        return { message: 'Connection error - check network', code: 'CONNECTION_ERROR' };
      }
