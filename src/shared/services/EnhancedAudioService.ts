...
class EnhancedAudioService {
   ...
   */
  private async selectBestVoice(preferredVoice?: string): Promise<string | undefined> {
    try {
      ...
      return qualityVoices[0]?.name || voices.find(v => v.lang.startsWith('en'))?.name
    } catch (error) {
  ...
  private handleAudioError(error: Error, context: string): void {
    ...
    if (error.message.includes('permission')) {
      userMessage = 'Microphone permission required. Please allow access and try again.'
    ...
    if (this.retryCount < this.maxRetries && !error.message.includes('permission')) {
      this.retryCount++
      setTimeout(() => {
        this.addFeedback({
          type: 'info',
  ...
  private setupActivityMonitoring(): void {
    ...
    this.visualizationInterval = window.setInterval(() => {
      if (this.activity.value.isListening) {
        ...
        audioService.getVolumeLevel().then(volume => {
