...
import { GAMING_STUDIOS, ROLE_TYPES, TECHNOLOGY_TAGS } from '../constants/gaming-studios';
...
export class SemanticSearchService {
  ...
  private async initialize() {
    ...
    try {
      ...
      logger.info(`SemanticSearchService initialized with ${this.vectors.length} vectors`);
    } catch (error) {
        ...
        queryKeywords.forEach(keyword => {
          if (vector.keywords.includes(keyword)) {
            ...
            if (vector.type === 'studio') {
              ...
              } else if (studio.games.some(game => game.toLowerCase().includes(keyword))) {
                relevanceBoost *= defaultBoosts.gameMatch;
              } else if (studio.technologies.some(tech => tech.toLowerCase().includes(keyword))) {
  ...
  public async findSimilar(
    ...
    const targetVector = this.vectors.find(v => v.id === itemId && v.type === itemType);
    ...
    this.vectors.forEach(vector => {
      ...
      const similarity = this.cosineSimilarity(targetVector.vector, vector.vector);
  ...
  private getRoleCategory(roleName: string): string {
    ...
    if (engineeringRoles.some(term => lowerRole.includes(term))) return 'Engineering';
  ...
  public getSearchAnalytics() {
    return {
      ...
      typeDistribution: this.vectors.reduce((acc, vector) => {
