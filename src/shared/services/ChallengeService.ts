...
class ChallengeService {
  private challengeTemplates: Record<string, Omit<Challenge, 'id' | 'completed'>> = {
    // Profile Challenges
    'complete-profile': {
   ...
   */
  generateDailyChallenges(userProgress: UserProgress, maxChallenges: number = 5): Challenge[] {
    ...
    if ((userProgress.profileCompletion || 0) < 100) {
      challenges.push(this.createChallenge('complete-profile', {
        description: `${100 - (userProgress.profileCompletion || 0)}% remaining to complete`,
        progress: userProgress.profileCompletion || 0,
        xp: Math.max(20, Math.floor((100 - (userProgress.profileCompletion || 0)) / 5))
      }));
    ...
    if (!userProgress.resumeCount || userProgress.resumeCount === 0) {
      ...
      challenges.push(this.createChallenge('enhance-resume', {
    ...
    if ((userProgress.portfolioCount || 0) < 3) {
      challenges.push(this.createChallenge('create-portfolio-item', {
    ...
    // Streak challenge
    if ((userProgress.streakDays || 0) >= 3 && (userProgress.streakDays || 0) < 7) {
      challenges.push(this.createChallenge('daily-streak', {
   ...
   */
  private createChallenge(templateKey: string, overrides: Partial<Challenge> = {}): Challenge {
   ...
   */
  getChallengesByCategory(challenges: Challenge[]): Record<string, Challenge[]> {
