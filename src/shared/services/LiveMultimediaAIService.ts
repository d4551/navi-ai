...
class LiveMultimediaAIService {
  ...
  async captureAndAnalyzeScreen(prompt?: string): Promise<MediaAnalysisResult> {
    ...
    try {
      ...
      const analysisPrompt =
        prompt ||
        'Analyze this screenshot and describe what you see in detail. Focus on any code, interfaces, or important visual elements.'
  ...
  private buildContextualPrompt(message: string): string {
    const recentHistory = this.conversationHistory
      ...
      .map(
        item => `[${item.type.toUpperCase()}] ${item.content.substring(0, 200)}`
      )
      .join('\n')
