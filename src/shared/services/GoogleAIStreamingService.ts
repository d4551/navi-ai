...
export class GoogleAIStreamingService {
   ...
   */
  private addToConversation(message: Omit<ConversationMessage, 'id' | 'timestamp'>): ConversationMessage {
  ...
  private buildChatHistory(): any[] {
    ...
    return this.conversationHistory
      ...
      .map(msg => ({
        role: msg.role,
        parts: msg.type === 'image' && msg.metadata?.frameData
          ? [
              {
                inlineData: {
                  mimeType: 'image/jpeg',
  ...
  async initialize(apiKey: string): Promise<void> {
    ...
    try {
      ...
      const testModels = ['gemini-2.5-flash', 'gemini-2.5-pro', 'gemini-1.5-flash', 'gemini-1.5-pro'];
      ...
      for (const m of testModels) {
        try {
          ...
          if (msg.includes('API_KEY_INVALID') || msg.includes('API key expired') || msg.includes('PERMISSION_DENIED')) {
      ...
      if (rawMessage.includes('API_KEY_INVALID') || rawMessage.toLowerCase().includes('expired')) {
        ...
        helpfulMessage = 'API key lacks required permissions. Check API restrictions in Google AI Studio (HTTP referrers / app restrictions).';
      }
      logger.error('Failed to initialize Google AI service:', { rawMessage, helpfulMessage });
  ...
  async startVideoStreaming(
    ...
    const {
      ...
      systemPrompt = aiSettings?.systemPrompt || 'You are a helpful AI assistant analyzing live video feed. Describe what you see and provide relevant insights.'
    } = options;
  ...
  await videoService.startAIStreaming(async (frame: SafeImageData) => {
        try {
          ...
          const framePrompt = 'Analyze this frame and continue our conversation about what you see.';
          ...
          logger.error('Error processing video frame:', { error: errorMessage, frame: frame.width + 'x' + frame.height });
          
          // Don't stop streaming for individual frame errors, but notify callback
          callbacks.onError?.(new Error(`Frame processing failed: ${errorMessage}`));
        }
      }, fps);
  ...
  async startScreenStreaming(
    ...
    const {
      ...
      maxTokens = aiSettings?.maxTokens || 1000,
      temperature = aiSettings?.temperature || 0.7,
      systemPrompt = aiSettings?.systemPrompt || 'You are analyzing a screen share. Describe what you see on the screen and provide insights about the content and user activity.'
    } = options;
  ...
  await videoService.startAIStreaming(async (frame: SafeImageData) => {
        try {
          ...
          const screenPrompt = 'Analyze this screen frame and continue our conversation about what you see.';
          ...
          logger.error('Error processing screen frame:', { error: errorMessage, frame: frame.width + 'x' + frame.height });
          
          // Don't stop streaming for individual frame errors, but notify callback
          callbacks.onError?.(new Error(`Screen frame processing failed: ${errorMessage}`));
        }
      }, fps);
   ...
   */
  async sendMessage(text: string, callbacks?: StreamingCallbacks): Promise<string> {
    if (!this.currentSession) {
    ...
    try {
      ...
      if (!this.genAI) {
        ...
        const envKey = ((typeof import.meta !== 'undefined' && (import.meta as any)?.env?.VITE_GEMINI_API_KEY) as string | undefined) ||
          ((typeof process !== 'undefined' && (process as any)?.env?.VITE_GEMINI_API_KEY) as string | undefined) ||
          (typeof localStorage !== 'undefined' ? (localStorage.getItem('gemini_api_key') as string | null) : null);
        if (envKey) {
      ...
      const prompt = options.prompt || 'Analyze this frame and describe what you see.';
      const modelId = options.model || 'gemini-2.5-flash';
      const base64 = typeof frame === 'string' ? frame : this.imageDataToBase64(frame);
