  ...
  > {
    ...
    for (const [table, stats] of Object.entries(tableStats)) {
      result[table] = {
        ...
        cacheHitRate:
          Math.round((stats.cacheHits / stats.queries) * 100 * 100) / 100,
      }
    }
  ...
  private generateRecommendations(
    ...
    if (cacheHitRate < 50) {
      recommendations.push(
        'Consider enabling more aggressive caching for frequently accessed data'
      )
    }
    ...
    for (const [table, stats] of Object.entries(tableStats)) {
      if (stats.averageTime > 50 && stats.cacheHitRate < 30) {
        recommendations.push(
          `Consider adding indexes or improving caching for '${table}' queries`
        )
      }
    }

    // Query pattern recommendations
    const searchQueries = metrics.filter(m =>
      m.operation.includes('search')
    ).length
    const totalQueries = metrics.length
    if (searchQueries / totalQueries > 0.5) {
      recommendations.push(
        'High search query volume - consider implementing search indexes'
      )
    }
    ...
    if (memoryUsage > 50 * 1024 * 1024) {
      // 50MB
      recommendations.push(
        'High memory usage detected - consider cache cleanup or optimization'
      )
    }
      ...
      if (stats.totalQueries > 0) {
        ...
        if (stats.slowQueries > 0 || stats.cacheHitRate < 50) {
          stats.recommendations.forEach(rec =>
            logger.warn(`DB Recommendation: ${rec}`)
          )
        }
      }
    }, intervalMs)
