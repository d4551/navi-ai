...
class SharingService {
   ...
   */
  async share(content: ShareContent, options: ShareOptions = {}): Promise<void> {
   ...
   */
  private showSharingModal(content: ShareContent, shareUrl: string, options: ShareOptions): void {
    const platforms = options.platforms || ['clipboard', 'linkedin', 'twitter', 'email'];
  ...
  private generatePlatformButtons(platforms: string[]): string {
    const buttons: Record<string, { label: string; icon: string; class: string }> = {
      clipboard: { label: 'Copy Link', icon: 'mdi-clipboard', class: 'btn-primary w-100 mb-2' },
      linkedin: { label: 'LinkedIn', icon: 'mdi-linkedin', class: 'btn-outline-primary' },
      twitter: { label: 'Twitter', icon: 'mdi-twitter', class: 'btn-outline-info' },
      email: { label: 'Email', icon: 'mdi-email', class: 'btn-outline-success' },
      qr: { label: 'QR Code', icon: 'mdi-qrcode', class: 'btn-outline-secondary' }
    };
   ...
   */
  private attachSharingHandlers(content: ShareContent, shareUrl: string, modal: HTMLElement): void {
   ...
   */
  async shareInterviewResults(score: number, totalQuestions: number): Promise<void> {
    await this.share({
  ...
  async shareResume(name: string, role: string): Promise<void> {
    await this.share({
      ...
      hashtags: ['resume', 'gaming', 'career', role.toLowerCase().replace(/\s+/g, '')]
    }, {
      preview: {
        title: `📄 ${name}'s Resume`,
        subtitle: role,
