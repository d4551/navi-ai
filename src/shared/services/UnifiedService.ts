...
class UnifiedService {
  ...
  private async initializeAI(provider?: string): Promise<void> {
    try {
      ...
      if (isReady) {
        ...
        logger.warn('⚠️ AI service not configured - some features will be limited');
      }
    } catch (error) {
  ...
  private async initializeStudioServices(): Promise<void> {
    try {
      ...
      const stats = await databaseManager.getStats();
      logger.info(`✅ Studio services ready (${stats.studios} studios in database)`);
    } catch (error) {
   ...
   */
  async getJobRecommendations(userProfile: any, limit: number = 10): Promise<UnifiedJob[]> {
    this.ensureInitialized();
   ...
   */
  async getStudioRecommendations(userProfile: any, limit: number = 10): Promise<UnifiedStudio[]> {
    this.ensureInitialized();
  ...
  async getServiceStats(): Promise<{
    database: { connected: boolean; jobs: number; studios: number; size: string };
  ...
  private ensureInitialized(): void {
    if (!this.initialized) {
      throw new Error('UnifiedService not initialized. Call initialize() first.');
    }
  }

  private async addAIJobScoring(jobs: any[], userProfile: any): Promise<UnifiedJob[]> {
    try {
        ...
        jobs.map(async (job) => {
          try {
            ...
            return {
              ...
              aiScore: (aiAnalysis.skillMatch + aiAnalysis.experienceMatch + aiAnalysis.cultureFit) / 3,
              aiAnalysis
            };
          } catch {
  ...
  private async addAIStudioInsights(studios: any[], userProfile: any): Promise<UnifiedStudio[]> {
    try {
  ...
  private convertToUnifiedStudio(studio: any): UnifiedStudio {
    return {
      ...
      games: Array.isArray(studio.games) ? studio.games : 
             typeof studio.games === 'string' ? JSON.parse(studio.games || '[]') : [],
      technologies: Array.isArray(studio.technologies) ? studio.technologies : 
                   typeof studio.technologies === 'string' ? JSON.parse(studio.technologies || '[]') : [],
