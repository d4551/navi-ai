...
export const SETTINGS_SCHEMA = z.object({
  ...
  portfolioFilter: z.enum(['all', 'project', 'open-source', 'writing', 'other']).default('all'),
  ...
  streaming: z.object({
    ...
    aiStreaming: z.object({
      enabled: z.boolean().default(false),
      model: z.string().default('gemini-2.5-flash'),
      fps: z.number().int().min(1).max(30).default(5),
      maxTokens: z.number().int().min(100).max(4000).default(1000),
      temperature: z.number().min(0).max(2).default(0.7),
      systemPrompt: z.string().default('You are a helpful AI assistant analyzing live video feed. Describe what you see and provide relevant insights.'),
      analysisType: z.enum(['continuous', 'on-demand', 'interval']).default('continuous'),
  ...
  jobBoardApis: z.object({
    // Job Board Sources
    enabledSources: z.array(z.string()).default(['indeed', 'linkedin', 'glassdoor']),
    ...
    defaultKeywords: z.array(z.string()).default(['software engineer', 'developer']),
    ...
    preferredGameEngines: z.array(z.string()).default(['Unity', 'Unreal Engine']),
    preferredStudioTypes: z.array(z.string()).default(['aaa', 'indie']),
    
    // Filters and preferences
    salaryMin: z.number().optional(),
    salaryMax: z.number().optional(),
    jobTypes: z.array(z.enum(['full-time', 'part-time', 'contract', 'internship'])).default(['full-time']),
    experienceLevels: z.array(z.enum(['entry', 'mid', 'senior', 'lead'])).default(['mid', 'senior']),
    remoteWorkPreference: z.enum(['any', 'remote-only', 'hybrid', 'onsite']).default('any'),
...
export function mergeSettings(current: Settings, patch: Partial<Settings>): Settings {
  ...
  if (typeof candidate.suggestionsSyncMinutes === 'number') {
    candidate.suggestionsSyncMinutes = Math.min(60, Math.max(1, Math.round(candidate.suggestionsSyncMinutes)))
  }
