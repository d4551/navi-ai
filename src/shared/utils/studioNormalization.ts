...
export function inferRegion(headquarters: string): string {
  ...
  if (/seattle|los angeles|irvine|austin|new york|san francisco|usa|u\.s\.|united states|california|washington|texas/.test(hq)) return 'North America';
  if (/montreal|vancouver|canada/.test(hq)) return 'North America';
  if (/london|paris|berlin|france|germany|poland|sweden|helsinki|copenhagen|europe|amsterdam|netherlands|finland|denmark/.test(hq)) return 'Europe';
  if (/tokyo|kyoto|osaka|japan|seoul|korea|shanghai|beijing|china|singapore|asia/.test(hq)) return 'Asia';
  return 'Other';
}

// Size description -> bucket
export function sizeToBucket(sizeDesc: string): NormalizedStudio['normalized']['sizeBucket'] {
  const s = (sizeDesc||'').toLowerCase();
  if (/^\s*(1-10|11-50|under 50|< ?50)/.test(s) || /indie|startup/.test(s)) return 'Indie';
...
const mapRoleToCategory = (role: string): string => {
  ...
  if (/engineer|programmer|developer|architect|devops|platform|graphics|backend|frontend|full\s*stack/.test(r)) return 'ENGINEERING';
  if (/designer|design|ux|ui|quest|level|systems|narrative/.test(r)) return 'DESIGN';
  if (/artist|art|animator|vfx|concept|technical artist|modeler|environment/.test(r)) return 'ART';
  if (/producer|product|project|scrum|manager|director/.test(r)) return 'PRODUCTION';
  if (/audio|sound|music|voice/.test(r)) return 'AUDIO';
  if (/data|analytics|scientist|analyst|research/.test(r)) return 'DATA_ANALYTICS';
  if (/community|relations|marketing|social|content/.test(r)) return 'COMMUNITY';
  return 'OTHER';
};
Object.values(GAMING_STUDIOS).forEach(s => {
  (s.commonRoles || []).forEach(r => { roleCategoryMap[r.toLowerCase()] = mapRoleToCategory(r); });
});
...
export function normalizeStudio(studio: GameStudio): NormalizedStudio {
  ...
  const techTags = Array.from(new Set((studio.technologies||[]).map(t => techCanonical[t.toLowerCase()] || t))).sort();
  const roleCategories = Array.from(new Set((studio.commonRoles||[]).map(r => roleCategoryMap[r.toLowerCase()]).filter(Boolean))).sort();
  const searchTokens = buildSearchTokens(studio, { region, sizeBucket, categories, techTags, roleCategories });
  return { ...studio, normalized: { region, sizeBucket, categories, techTags, roleCategories, searchTokens } };
}

export function buildSearchTokens(studio: GameStudio, extras: { region: string; sizeBucket: string; categories: string[]; techTags: string[]; roleCategories: string[]; }): string[] {
  const base = [studio.name, studio.description, studio.headquarters].concat(studio.games, studio.technologies, studio.commonRoles);
  base.push(extras.region, extras.sizeBucket, ...extras.categories, ...extras.techTags, ...extras.roleCategories);
  return Array.from(new Set(base.filter(Boolean).map(s => s.toLowerCase())));
}

export function buildNormalizedStudios(source: Record<string, GameStudio> = GAMING_STUDIOS) {
...
export function fuzzyMatchStudios(query: string, index: Record<string, Set<string>>, studios: Record<string, NormalizedStudio>) {
  ...
  return Array.from(candidateIds).map(id => {
    ...
    // Token prefix matches
      ...
      bestDistance = Math.min(...prefixTokens.map(t => levenshteinDistance(q, t.slice(0, q.length))));
    }
    ...
    // Token contains query
      ...
      bestDistance = Math.min(...containingTokens.map(t => {
    ...
    // Fuzzy match based on Levenshtein distance (for typos)
      ...
      const tokenDistances = tokens.slice(0, 5).map(t => levenshteinDistance(q, t));
