...
export function buildInterviewSystemContext(input: InterviewContextInput) {
  ...
  if (input.studio) {
    ...
    if (input.studio.category) lines.push(`Studio Category: ${input.studio.category}`);
    if (input.studio.region) lines.push(`Region: ${input.studio.region}`);
    if (input.studio.commonRoles?.length) lines.push(`Common Roles: ${input.studio.commonRoles.slice(0,6).join(', ')}`);
    if (input.studio.technologies?.length) lines.push(`Tech Stack: ${input.studio.technologies.slice(0,8).join(', ')}`);
  }
  ...
  if (input.focusAreas?.length) lines.push(`Focus Areas: ${input.focusAreas.join(', ')}`);
  if (input.persona) {
    lines.push(`Interviewer Persona: ${input.persona.name} - ${input.persona.role}`);
    if (input.persona.style) lines.push(`Interview Style: ${String(input.persona.style).slice(0,160)}`);
  }
  ...
  if (input.userProfile) {
    ...
    if (Array.isArray(primarySkills) && primarySkills.length) lines.push(`Primary Skills: ${primarySkills.slice(0,8).join(', ')}`);
  }
  return lines.join('\n');
}
...
export function buildInterviewContextObject(input: InterviewContextInput) {
  return {
    studio: input.studio ? { id: input.studio.id, name: input.studio.name, category: input.studio.category } : undefined,
    roleType: input.roleType,
    experienceLevel: input.experienceLevel,
    difficulty: input.difficulty,
    focusAreas: input.focusAreas,
    persona: input.persona ? { name: input.persona.name, role: input.persona.role, style: input.persona.style } : undefined,
  };
}
