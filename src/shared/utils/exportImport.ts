    ...
    searchPreferences: z
      .object({
        ...
        salaryRange: z
          ...
          .optional(),
        jobTypes: z
          .array(z.enum(['full-time', 'part-time', 'contract', 'remote']))
          .optional(),
...
export class NaviExporter {
  ...
  private static countItems(data: Partial<NaviExportData>): number {
    ...
    if (data.resumeData?.experience?.length)
      count += data.resumeData.experience.length
    if (data.resumeData?.education?.length)
      count += data.resumeData.education.length
    if (data.resumeData?.projects?.length)
      count += data.resumeData.projects.length
    if (data.portfolio?.items?.length) count += data.portfolio.items.length
    if (data.chatHistory?.length) count += data.chatHistory.length
    if (data.jobSearchData?.applications?.length)
      count += data.jobSearchData.applications.length
    if (data.jobSearchData?.savedJobs?.length)
      count += data.jobSearchData.savedJobs.length
...
export class NaviImporter {
  static async importFromFile(
    file: File
  ): Promise<{ success: boolean; data?: NaviExportData; error?: string }> {
    try {
    ...
    if (data.version && data.version.startsWith('0.')) {
      ...
      const migrated: Partial<NaviExportData> = {
        ...
        portfolio: data.portfolio
          ? {
              items: Array.isArray(data.portfolio)
                ? data.portfolio
...
export const ExportTypes = {
  ...
  complete: (allData: Partial<NaviExportData>) =>
    NaviExporter.createExport(allData),
}
...
export const ExportHelpers = {
  ...
  getDefaultFilename: (type: string = 'complete') =>
    `navi-cv-${type}-${new Date().toISOString().split('T')[0]}.json`,
