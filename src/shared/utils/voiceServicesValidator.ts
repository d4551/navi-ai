...
export function checkVoiceServicesCapabilities(): VoiceServicesCapabilities {
  const capabilities = {
    ...
    getUserMedia: !!(navigator.mediaDevices?.getUserMedia || (navigator as any).getUserMedia),
  ...
  capabilities.fullSupport = capabilities.webSocket && 
...
export function getVoiceServicesErrors(): VoiceServicesErrors {
  ...
  if (!capabilities.getUserMedia) {
    errors.missing.push('getUserMedia API not available - microphone access not supported');
  }

  if (!capabilities.getDisplayMedia) {
    errors.warnings.push('getDisplayMedia API not available - screen capture not supported');
  }
  ...
  if (!capabilities.mediaRecorder) {
    errors.warnings.push('MediaRecorder API not available - recording features limited');
  }
 ...
 */
export async function testAudioContext(): Promise<{ success: boolean; error?: string; requiresUserGesture?: boolean }> {
  try {
    ...
    return { 
      success: false, 
      error: error instanceof Error ? error.message : 'Unknown audio context error' 
    };
  }
}
 ...
 * Test camera access
 */
export async function testCameraAccess(): Promise<{ success: boolean; error?: string }> {
  try {
    const stream = await navigator.mediaDevices.getUserMedia({ 
 ...
 */
export async function testScreenShareAccess(): Promise<{ success: boolean; error?: string }> {
  try {
    ...
    return { 
      success: false, 
      error: error instanceof Error ? error.message : 'Screen sharing access denied' 
    };
  }
}
 ...
 */
export async function runVoiceServicesDiagnostics(options: { invasive?: boolean } = {}): Promise<{
 ...
 */
export function getVoiceServicesStatusMessage(capabilities: VoiceServicesCapabilities): string {
