...
class ConfigManager {
  ...
  private loadConfiguration(): ServiceConfig {
    const envConfig = environmentConfigs[this.environment] || environmentConfigs.development;
  ...
  private applyEnvironmentOverrides(config: ServiceConfig): ServiceConfig {
    ...
    return {
      ...
      jobs: {
        ...config.jobs,
        enableAIScoring: env.ENABLE_AI_SCORING === 'true' || config.jobs.enableAIScoring,
        enableBackgroundSync: env.ENABLE_JOB_SYNC === 'true' || config.jobs.enableBackgroundSync
      },
      studios: {
        ...config.studios,
        enableAIInsights: env.ENABLE_AI_INSIGHTS === 'true' || config.studios.enableAIInsights,
        enableBackgroundSync: env.ENABLE_STUDIO_SYNC === 'true' || config.studios.enableBackgroundSync
      },
      logging: {
        ...config.logging,
