// useScreenCapture.ts - Vue 3 Composition API version
import { ref, onUnmounted, getCurrentInstance } from 'vue'

export interface UseMediaStreamResult {
  type: 'webcam' | 'screen'
  start: (options?: { audio?: boolean; constraints?: DisplayMediaStreamOptions }) => Promise<MediaStream>
  stop: () => void
...
export function useScreenCapture(): UseMediaStreamResult {
  ...
  const start = async (options?: { audio?: boolean; constraints?: DisplayMediaStreamOptions }): Promise<MediaStream> => {
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.name === 'NotAllowedError') {
          throw new Error('Screen capture permission was denied. Please allow screen sharing and try again.')
        } else if (error.name === 'NotSupportedError') {
          throw new Error('Screen capture is not supported on this device or browser.')
