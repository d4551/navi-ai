/**
 * Unified UI System Composable
 * Combines theme management and responsive design into a single system
 * Updated to work with the new UnifiedTheme system
 */
...
export function useUnifiedUI() {
  ...
  const getSpacing = (config?: {
    ...
    const deviceType = responsive?.deviceType?.value || 'desktop'
    const baseSpacing = config?.[deviceType] || '1rem'
    ...
    if (theme.isDark.value && (responsive?.isMobile?.value || false)) {
      ...
      return `${Math.max(0.5, numValue * 0.9)}${unit}`
    }
  ...
  const getFontSize = (config?: {
    ...
    const deviceType = responsive?.deviceType?.value || 'desktop'
    const baseSize = config?.[deviceType] || '1rem'
...
export function autoInitializeUnifiedUI() {
