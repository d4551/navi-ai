...
export function usePerformantPortfolio() {
  ...
  const layout = computed({
    ...
    set: (val: 'grid' | 'list' | 'timeline') => store.updatePortfolioLayout(val)
  })
  ...
  const filtered = computed(() => {
    ...
    const sortedItems = PortfolioService.sortItems(filteredItems, sortMode.value)
  ...
  async function bulkDelete(ids: string[]) {
    ...
    try {
      ...
      const sortedIds = ids.sort((a, b) => {
        const indexA = store.user.portfolio.findIndex((item: any) => item.id === a)
        const indexB = store.user.portfolio.findIndex((item: any) => item.id === b)
  ...
  async function suggestSkills(project: PortfolioItem): Promise<string[]> {
    try {
      ...
      const skillCategories = {
        programming: ['C++', 'C#', 'JavaScript', 'Python', 'Lua', 'Rust', 'Java'],
        engines: ['Unity', 'Unreal Engine', 'Godot', 'CryEngine', 'GameMaker', 'Construct'],
        graphics: ['3D Modeling', 'Animation', 'Shader Programming', 'VFX', 'UI/UX Design'],
        audio: ['Audio Implementation', 'Sound Design', 'Music Composition', 'FMOD', 'Wwise'],
        tools: ['Git', 'Perforce', 'Jenkins', 'Jira', 'Confluence', 'Maya', 'Blender'],
        platforms: ['PC', 'Console', 'Mobile', 'VR', 'AR', 'Web'],
        specializations: ['Gameplay Programming', 'Engine Programming', 'AI Programming', 'Graphics Programming']
      }
      ...
      Object.values(skillCategories).flat().forEach(skill => {
        if (lowerProjectText.includes(skill.toLowerCase()) && !project.skills?.includes(skill)) {
      ...
      for (const item of items.value) {
        ...
        if (item.technologies?.length) {
          item.technologies.forEach(tech => {
            if (['Unity', 'Unreal', 'Godot'].some(engine => tech.includes(engine))) {
      ...
      for (const item of items.value) {
        if (!item.description || item.description.length < 50) {
          ...
          if (item.responsibilities?.length) {
            parts.push(`Key responsibilities included ${item.responsibilities[0]}`)
          }
  ...
  // Utility functions
  function createPortfolioItem(data: Partial<PortfolioProject | PortfolioItem>): PortfolioItem {
  ...
  watch(
    [searchQuery, filterType, showFeaturedOnly, skillFilters, dateFrom, dateTo, sortMode],
    () => {
      currentPage.value = 1
    },
    { deep: true }
