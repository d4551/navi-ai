...
export function useAIJobSearch(userProfile = {}) {
  ...
  const topMatches = computed(() => 
    searchResults.value
      .filter(job => job.overallScore > 0.8)
      .slice(0, 5)
  )
  ...
  async function performAISearch(searchForm, options = {}) {
    ...
    try {
      ...
      if (analysisResult.success) {
        ...
        logger.warn('Search analysis failed, using fallback:', analysisResult.error)
        searchAnalysis.value = analysisResult.fallback
      }

      // Step 2: Generate personalized insights
      const insightsResult = await aiJobMatchingService.generateJobSearchInsights(
        searchForm.query,
        userProfile,
        marketSummary.value || {}
      )
      ...
      if (matchingResult.success) {
        ...
        if (matchingResult.enrichedJobs) {
          ...
          logger.info(`Jobs enriched with studio data: ${enrichedJobs.value.filter(j => j.studioData).length}/${enrichedJobs.value.length}`)
        }
        
        logger.info(`AI job matching completed: ${matchingResult.matches.length} jobs ranked`)
      } else {
  ...
  async function getSearchSuggestions(partialQuery) {
    ...
    try {
      ...
      const gamingTerms = [
        'game designer', 'unity developer', 'unreal engineer', 'community manager',
        'esports coordinator', 'mobile producer', 'technical artist', 'qa lead',
        'gameplay programmer', 'ui/ux designer', 'devops engineer', 'data analyst'
      ]
      ...
      return [...filtered, ...quickSuggestions.value.slice(0, 8 - filtered.length)]
  ...
  function updateSearchInsights(newInsights) {
    ...
    searchInsights.value = [...searchInsights.value, ...uniqueNewInsights].slice(0, 6)
  }
  ...
  function generateMockJobs(analysis, count = 10) {
    ...
    const mockJobs = []
    const companies = ['Epic Games', 'Riot Games', 'Blizzard', 'Valve', 'CD Projekt', 'Ubisoft', 'EA', 'Unity Technologies']
    const roles = analysis?.searchAnalysis?.extractedRoles || ['Game Developer', 'Game Designer', 'Producer']
    
    for (let i = 0; i < count; i++) {
      mockJobs.push({
        id: `job_${i + 1}`,
        title: roles[i % roles.length] + (i > roles.length - 1 ? ` ${Math.floor(i / roles.length) + 1}` : ''),
        company: companies[i % companies.length],
        location: ['San Francisco, CA', 'Los Angeles, CA', 'Remote', 'Austin, TX', 'Seattle, WA'][i % 5],
        ...
        requirements: analysis?.searchAnalysis?.keySkills?.slice(0, 3) || ['Gaming passion', 'Team collaboration'],
