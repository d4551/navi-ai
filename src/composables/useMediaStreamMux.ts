// useMediaStreamMux.ts - Vue 3 Composition API for managing multiple media streams
...
export interface UseMediaStreamMuxResult {
  ...
  // Unified controls
  startWebcam: (constraints?: { video?: MediaTrackConstraints | boolean; audio?: MediaTrackConstraints | boolean }) => Promise<MediaStream>
  startScreenCapture: (options?: { audio?: boolean; constraints?: DisplayMediaStreamOptions }) => Promise<MediaStream>
  stopAll: () => void
...
export function useMediaStreamMux(): UseMediaStreamMuxResult {
  ...
  const activeStream = computed(() => {
    ...
    if (screenCapture.stream && screenCapture.isStreaming) return screenCapture.stream
    return null
  })
  ...
  const startWebcam = async (constraints?: { video?: MediaTrackConstraints | boolean; audio?: MediaTrackConstraints | boolean }): Promise<MediaStream> => {
  ...
  const startScreenCapture = async (options?: { audio?: boolean; constraints?: DisplayMediaStreamOptions }): Promise<MediaStream> => {
  ...
  const toggleMicrophone = () => {
    ...
    if (stream) {
      ...
      isMicrophoneEnabled.value = audioTracks.length > 0 ? audioTracks[0].enabled : false
    }
  }
