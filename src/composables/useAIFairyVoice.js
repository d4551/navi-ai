...
export function useAIFairyVoice() {
  ...
  const addFairyMagic = text => {
    ...
    if (Math.random() < 0.2) {
      const phrase =
        fairyPhrases[Math.floor(Math.random() * fairyPhrases.length)]
      text = `${phrase}, ${text}`
    }
  ...
  const toggleVoice = async () => {
    ...
    if (isVoiceEnabled.value) {
      await speakAsFairy(
        "Voice mode enabled! I'm ready to help with magical assistance."
      )
    }
...
export function useAIFairyChatVoice() {
  ...
  const handleVoiceCommand = async command => {
    ...
    if (
      ...
      await speakAsFairy(
        "I'm your magical AI assistant! I can help with tasks, answer questions, and provide guidance with a touch of fairy magic!"
      )
    } else if (
