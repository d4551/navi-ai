/**
 * Job-Tailored Resume Composable
 * Vue 3 composable for analyzing job postings and creating tailored resumes/cover letters
 */
...
export function useJobTailoredResume(): UseJobTailoredResumeReturn {
  // State
  ...
  // Get cover letter composable for integrated functionality
  ...
  const matchScore = computed(() => {
    return resumeTailoring.value?.matchAnalysis?.overallMatch || 0
  })

  const keywordCoverage = computed(() => {
    return resumeTailoring.value?.tailoredResume?.keywordCoverage || 0
  })

  const atsScore = computed(() => {
    return resumeTailoring.value?.tailoredResume?.atsCompliance || 0
  })
  ...
  // Methods
  async function analyzeJobFromText(jobText: string) {
    if (!jobText?.trim()) {
  ...
  async function analyzeJobFromUrl(url: string) {
    if (!url?.trim()) {
  ...
  async function tailorResumeToJob(
    ...
    try {
      ...
      logger.info(`Starting resume tailoring for ${jobAnalysis.value.jobTitle}`)
...
`
  ...
  function getMissingKeywords(currentResume: any): string[] {
  ...
  async function exportTailoredDocuments() {
