 ...
 */
export function createCompatibilityErrorMessage(
  missingFeatures: string[]
): string {
  if (missingFeatures.length === 0) return ''

  const browserSuggestion =
    'Please use a modern browser like Chrome 90+, Firefox 88+, Safari 14+, or Edge 90+.'
...
export function logBrowserCompatibility(): void {
  ...
  console.log(
    'AI Compatibility:',
    aiCompat.isCompatible ? '✅ Compatible' : '❌ Issues found'
  )
