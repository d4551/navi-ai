...
export function initStatusPanels(root = document) {
  try {
    ...
    panels.forEach((panel) => {
      ...
      const header = panel.querySelector('[data-toggle="status-panel"], .panel-header, .status-header') || null;
      const content = panel.querySelector('.panel-content, .status-content, #statusPanelContent') || null;
      if (!header || !content) {
...
export function hideExternalStatusPanels() {
  try { document.body.classList.add('hide-external-status-panels'); } catch {}
}
