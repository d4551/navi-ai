...
export function useTailorActions() {
  ...
  const insertKeywordsToSkills = (keywords, skillType = "technical") => {
    ...
    if (!store.resumeData) { store.resumeData = { personalInfo:{}, experience:[], education:[], skills:{ technical:[], soft:[] }, achievements:[], portfolio:[] } }
    if (!store.resumeData.skills) { store.resumeData.skills = { technical: [], soft: [] } }
