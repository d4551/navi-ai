...
export function dedupeStudios(
  ...
  for (const raw of records) {
    ...
    const existingScore =
      (existing.games?.length || 0) * 2 + (existing.description?.length || 0)
    const candidateScore =
      (raw.games?.length || 0) * 2 + (raw.description?.length || 0)
    if (candidateScore > existingScore) {
      ...
      existing.technologies = Array.from(
        new Set([...(existing.technologies || []), ...(raw.technologies || [])])
      )
    }
  }
