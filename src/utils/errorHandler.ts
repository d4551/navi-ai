...
export class ErrorHandler {
  ...
  private errorListeners: Map<string, Array<(error: EnhancedError) => void>> = new Map();
  ...
  wrap(error: unknown, context: ErrorContext = {}): EnhancedError {
    ...
    if (error instanceof Error) {
      ...
      if (message.includes('network') || message.includes('fetch') || message.includes('timeout')) {
        ...
        severity = 'low';
        recoverable = false;
      } else if (message.includes('chunk') || message.includes('module') || message.includes('import')) {
        category = 'resource';
        code = 'RESOURCE_ERROR';
        severity = 'high';
  ...
  async safeExecute<T>(
    ...
    try {
      ...
      if (fallbackOptions.showUserMessage) {
        const message = fallbackOptions.userMessage || 'An error occurred. Please try again.';
  ...
  async reportError(error: EnhancedError, additionalContext: Record<string, any> = {}): Promise<void> {
    try {
      const errorReport = {
        ...error.toJSON(),
        additionalContext,
