...
export class GameStudioAPIDemo {
  ...
  async demonstrateWikipediaQueries() {
    ...
    try {
      ...
      console.log(`   ✅ Found ${wikipediaStudios.length} studios from Wikipedia`);
      
      if (wikipediaStudios.length > 0) {
        const sample = wikipediaStudios[0];
        console.log('   📋 Sample Wikipedia page:', {
          name: sample.name,
          hasDescription: !!sample.description && sample.description.length > 50,
  ...
  async demonstrateDataAggregation() {
    ...
    try {
      ...
      const sourceBreakdown = studios.reduce((acc, studio) => {
      ...
      const multiSourceStudios = studios.filter(s => 
        s.metadata?.sources && s.metadata.sources.length > 1
      );
      ...
      console.log(`      ⭐ High confidence (≥0.8): ${highConfidenceStudios.length}`);
