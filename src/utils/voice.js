...
import {
  resolveGeminiApiKey,
  resolveGoogleCloudApiKey,
} from '@/shared/utils/apiKeys'
...
function cleanupCache() {
  ...
  entries.sort((a, b) => {
    const scoreA =
      (a[1].hits || 0) - (Date.now() - a[1].timestamp) / (1000 * 60) // Decay score over time
    const scoreB =
      (b[1].hits || 0) - (Date.now() - b[1].timestamp) / (1000 * 60)
  ...
  logger.debug(
    `Cache cleanup completed. Kept ${keepCount} entries out of ${entries.length}`
  )
}
...
function getQualityConfig(provider, quality = null) {
  ...
  return (
    qualityConfigs[targetQuality]?.[provider] ||
    qualityConfigs.balanced[provider] ||
    {}
  )
}

function adaptQualityBasedOnPerformance(provider) {
  if (!qualityPreferences.adaptiveQuality)
    return qualityPreferences.preferredQuality
...
async function speakViaGeminiLive(text, options) {
  try {
    logger.debug('speakViaGeminiLive called with:', {
    ...
    const { MultimodalLiveService } = await import(
      '@/shared/services/MultimodalLiveService'
    )
      ...
      const ttsConfig = {
        ...
        systemInstruction: {
          parts: [
            {
              text:
                "You are a text-to-speech system. Read the user's message aloud naturally and clearly. Do not add any additional commentary or responses." +
                (voiceHint
                  ? ` Use the following voice style if possible: ${voiceHint}.`
                  : ''),
            },
          ],
        },
      ...
      while (connectionAttempts < maxConnectionAttempts) {
        try {
          ...
          if (connectionAttempts >= maxConnectionAttempts) {
            throw new Error(
              `Failed to connect to Gemini Live API after ${maxConnectionAttempts} attempts: ${error.message}`
            )
          }
    ...
    return new Promise((resolve, reject) => {
      ...
      const callbacks = {
        onAudio: audioData => {
          logger.debug(
            'Received audio from Gemini Live API, size:',
          ...
          try {
            ...
            audioContext
              .decodeAudioData(audioData.slice())
              .then(audioBuffer => {
      ...
      logger.debug(
        'Setting callbacks and sending message to Gemini Live API...'
      )
      ...
      const estimateAudioDuration = text => {
        ...
        const bufferTime = Math.max(
          3000,
          Math.min(8000, estimatedSpeechTimeMs * 1.5)
        )
        return bufferTime
      }
      ...
      logger.debug(
        `Setting dynamic timeout: ${timeoutDuration}ms for text length: ${text.length} chars`
      )
      ...
      liveService
        ...
        .then(() => {
          ...
          setTimeout(() => {
            if (!audioReceived) {
              logger.warn(
                `No audio response received within ${timeoutDuration}ms timeout`
              )
              reject(
                new Error(
                  `No audio response received within ${timeoutDuration}ms timeout`
...
async function speakViaGoogleCloudTTS(text, options) {
  try {
    logger.debug('speakViaGoogleCloudTTS called with:', {
      text: text.substring(0, 50) + '...',
      hasApiKey: !!options.apiKey,
    })
...
async function speakViaGoogleCloudTTSRest(text, options) {
  try {
    ...
    if (options.apiKey.startsWith('AIzaSyC')) {
      const warning =
        'CONFIGURATION ERROR: You are using a Gemini API key for Google Cloud TTS.\n' +
      ...
      throw new Error(
        'Invalid API key type: Gemini API key cannot be used for Google Cloud TTS. Please configure a Google Cloud API key instead.'
      )
    }
    ...
    if (!response.ok) {
      ...
      if (response.status === 403) {
        let errorMessage =
          'Google Cloud TTS API access denied (403). Possible issues:\n'
        errorMessage += '1. API key is not valid for Google Cloud TTS service\n'
        errorMessage +=
          '2. Text-to-Speech API is not enabled in your Google Cloud project\n'
        errorMessage += '3. API key lacks proper permissions\n'
        errorMessage +=
          '4. You may be using a Gemini API key instead of a Google Cloud API key\n'
        errorMessage +=
...
async function speakViaKokoro(text, options) {
  try {
    logger.debug('speakViaKokoro called with:', {
      text: text.substring(0, 50) + '...',
      options,
    })
    ...
    const voice = availableVoices.includes(requestedVoice)
      ? requestedVoice
      : 'af_heart'

    if (voice !== requestedVoice) {
      logger.warn(
        `Requested voice '${requestedVoice}' not available, using '${voice}' instead`
      )
  ...
  return {
    ...
    async speak(text, options = {}) {
      const requestedProvider =
        options.provider || routing.ttsProvider || 'system'
      const speakOpts = {
      ...
      logger.debug(
        'Voice.js - speak() called with requested provider:',
        requestedProvider
      )
      ...
      const optimizedChain = fallbackChain.filter(provider => {
        ...
        return (
          healthyProviders.includes(provider) || provider === requestedProvider
        )
      })
      ...
      for (let i = 0; i < optimizedChain.length; i++) {
        ...
        try {
          logger.debug(
            `Attempting TTS with provider: ${provider} (${i + 1}/${optimizedChain.length})`
          )
          ...
          const result = await this.speakWithProvider(text, provider, {
            ...options,
            ...speakOpts,
          })
          ...
          if (
            error.message.includes(
              'Gemini API key cannot be used for Google Cloud TTS'
            )
          ) {
            try {
              if (window.toast?.warning) {
                window.toast.warning(
                  '🔑 API Key Configuration Issue: Using Gemini key for Google Cloud TTS. Falling back to browser TTS.',
                  { duration: 5000 }
                )
              }
            } catch {}
          ...
          if (i === optimizedChain.length - 1) {
            ...
            if (
              ...
              error.message.includes('Invalid API key type')
            ) {
    ...
    async speakWithProvider(text, provider, options) {
      ...
      logger.debug(
        `Using ${adaptiveQuality} quality for ${provider}:`,
        qualityConfig
      )
      ...
      const timeoutPromise = new Promise((_, reject) =>
        setTimeout(
          () => reject(new Error(`TTS timeout after ${timeout}ms`)),
          timeout
        )
      )
      ...
      try {
        ...
        const enhancedError = new Error(
          `${provider} TTS failed: ${error.message}`
        )
    ...
    async testProviderHealth(provider, options = {}) {
      const testText =
        options.testText || 'Testing voice synthesis quality and connectivity.'
      ...
      try {
        ...
        return {
          success: false,
          message: `${provider} TTS failed: ${error.message}`,
        }
      }
    },
    ...
    startMonitoring(callback, interval = 30000) {
      ...
      this._monitoringInterval = setInterval(async () => {
        try {
          ...
          const healthStatus = Object.entries(providerHealth).reduce(
