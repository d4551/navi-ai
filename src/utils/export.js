...
export class ExportService {
  ...
  static async exportToPDF(resumeData, options = {}) {
    ...
    try {
      ...
      const defaultBg = css ? (css.getPropertyValue("--bg-primary") || "#ffffff").trim() : "#ffffff";
      const {
        theme = "professional",
        scale = 2,
      ...
      if (typeof window === "undefined" || typeof document === "undefined") {
        const lines = []
        const name = `${resumeData.personalInfo?.firstName || ''} ${resumeData.personalInfo?.lastName || ''}`.trim() || 'Resume'
        lines.push(name)
...
<body>
  ...
  static async generateQRCode(url, options = {}) {
    try {
      const css = getComputedStyle(document.documentElement);
      const dark = (options.darkColor || css.getPropertyValue("--text-primary") || "#000000").trim();
      const light = (options.lightColor || css.getPropertyValue("--bg-primary") || "#ffffff").trim();
      const qrOptions = {
