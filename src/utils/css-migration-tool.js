...
import { DESIGN_TOKENS as _DESIGN_TOKENS, GAMING_COLORS as _GAMING_COLORS } from '@/shared/composables/useUnifiedTheme'
...
export class CSSMigrationTool {
  ...
  async migrateElement(element, config) {
    ...
    const elementId = element.id || `element-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`
  ...
  migrateInlineStyles(element, config) {
    ...
    // Migrate spacing
    for (const property of ['margin', 'padding', 'marginTop', 'marginRight', 'marginBottom', 'marginLeft', 'paddingTop', 'paddingRight', 'paddingBottom', 'paddingLeft']) {
      const value = style.getPropertyValue(property)
    ...
    if (fontFamily) {
      for (const [legacy, modern] of Object.entries(STYLE_MAPPINGS.fontFamily)) {
  ...
  migrateCSSClasses(element, config) {
    ...
    if (element.tagName === 'BUTTON' || classList.contains('btn')) {
      ...
      } else if (classList.contains('secondary') || classList.contains('btn-secondary')) {
  ...
  applyComponentTemplate(element, config) {
    ...
    if (classList.contains('ds-button') || element.tagName === 'BUTTON') {
      if (classList.contains('ds-button--primary') || classList.contains('primary')) {
        this.applyTemplate(element, COMPONENT_TEMPLATES.button.primary, config)
      } else {
        this.applyTemplate(element, COMPONENT_TEMPLATES.button.secondary, config)
      }
    }
    
    // Apply input template
    if (classList.contains('ds-input') || element.tagName === 'INPUT' || element.tagName === 'TEXTAREA') {
  ...
  backupElementStyles(element, elementId) {
    ...
    const keyProperties = [
      'color', 'backgroundColor', 'fontSize', 'fontFamily', 'padding', 'margin',
      'borderRadius', 'border', 'boxShadow', 'transform', 'transition'
    ]
  ...
  rollbackElement(elementId) {
    ...
    if (!element || !backup) {
      console.warn(`Cannot rollback element ${elementId} - element or backup not found`)
      return false
    }
