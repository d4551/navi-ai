...
export async function preloadCriticalModules(): Promise<void> {
  ...
  try {
    const hasProjects = localStorage.getItem('portfolio') || localStorage.getItem('documents');
    if (hasProjects) {
 ...
 */
export function getModuleLoadingStatus(): Record<string, 'loading' | 'loaded' | 'error'> {
  ...
  for (const [moduleId, promise] of moduleCache.entries()) {
    ...
    if (promiseWithStatus.__status) {
      ...
      promise
        .then(() => { promiseWithStatus.__status = 'loaded'; })
        .catch(() => { promiseWithStatus.__status = 'error'; });
    }
  }
