...
export const PAGE_COMPONENTS = {
  ...
  'resume.additional': () => import('@/views/resume/AdditionalExperienceList.vue'),
  ...
  'coverLetter.content': () => import('@/views/coverletter/CoverLetterContentForm.vue'),
  'coverLetter.jobInfo': () => import('@/views/coverletter/JobInfoForm.vue'),
  'coverLetter.preview': () => import('@/views/coverletter/CoverLetterPreview.vue'),
};
...
export const PAGE_METADATA = {
  ...
  portfolioStudio: {
    ...
    description: 'Unified portfolio studio for projects, clips, and achievements',
    muiTheme: true,
    requiresAuth: false,
...
export class PagePointerManager {
  ...
  async loadComponent(pageKey) {
    try {
      ...
      // Validate component
      if (!module.default) {
        throw new Error(`Component '${pageKey}' does not have a default export`);
      }
  ...
  async preloadComponents(componentKeys = []) {
    const essentialComponents = componentKeys.length > 0 ? componentKeys : [
      'dashboard',
      'resumeBuilder',
      'jobSearch',
      'settings'
...
export const getPageComponent = (pageKey) => pagePointerManager.getPageComponent(pageKey);
export const getComponentByRoute = (routePath) => pagePointerManager.getComponentByRoute(routePath);
export const getPageMetadata = (pageKey) => pagePointerManager.getPageMetadata(pageKey);
export const preloadComponents = (components) => pagePointerManager.preloadComponents(components);
export const validateAllPointers = () => pagePointerManager.validateAllPointers();
