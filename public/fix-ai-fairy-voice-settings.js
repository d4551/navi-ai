...
function fixTTSSettings() {
  ...
  try {
    ...
    if (
      ...
      !['system', 'gemini', 'kokoro'].includes(appSettings.ttsProvider)
    ) {
    ...
    if (
      !appSettings.sttProvider ||
      !['system', 'gemini'].includes(appSettings.sttProvider)
    ) {
    ...
    if (
      typeof appSettings.speechRate !== 'number' ||
      appSettings.speechRate < 0.5 ||
      appSettings.speechRate > 2
    ) {
    ...
    if (
      typeof appSettings.speechPitch !== 'number' ||
      appSettings.speechPitch < 0.5 ||
      appSettings.speechPitch > 2
    ) {
    ...
    if (
      typeof appSettings.speechVolume !== 'number' ||
      appSettings.speechVolume < 0 ||
      appSettings.speechVolume > 1
    ) {
    ...
    if (appSettings.ttsProvider === 'gemini' && !appSettings.geminiApiKey) {
      console.log(
        '⚠️  Found Gemini TTS without API key - resetting to system TTS'
      )
      appSettings.ttsProvider = 'system'
    }
...
async function testVoiceRouting() {
  ...
  try {
    ...
    const { setVoiceRoutingPreferences, getVoiceRoutingPreferences } =
      await import('/src/utils/voice.js')
...
async function validateGoogleAIIntegration() {
  ...
  try {
    ...
    if (!appSettings.geminiApiKey) {
      console.log(
        'ℹ️  No Gemini API key configured - Google AI features will be disabled'
      )
...
async function testAllVoiceSettings() {
  ...
  const tests = {
    ...
    voiceLang: {
      ...
      valid:
        typeof appSettings.voiceLang === 'string' &&
        appSettings.voiceLang.length >= 2,
      test: 'Voice Language',
    },
  }
  ...
  for (const [key, test] of Object.entries(tests)) {
    ...
    console.log(
      `${status} ${test.test}: ${JSON.stringify(test.value)} (${test.valid ? 'Valid' : 'Invalid'})`
    )
  }
...
async function fixAllAIFairyVoiceSettings() {
  ...
  console.log(
    'Fairy Settings:',
    results.fairySettings ? '✅ Fixed' : '❌ Failed'
  )
  console.log(
    'Kokoro TTS:',
    results.kokoroInit.success
      ? `✅ ${results.kokoroInit.type} models`
      : '⚠️  Not available'
  )
  console.log(
    'Google AI:',
    results.googleAI.available
      ? '✅ Available'
      : `⚠️  ${results.googleAI.reason}`
  )
  console.log(
    'Voice Routing:',
    results.voiceRouting ? '✅ Configured' : '❌ Failed'
  )
...
if (typeof window !== 'undefined') {
  ...
  console.log(
    'Run window.naviVoiceFix.fixAllAIFairyVoiceSettings() to fix all settings'
  )
}
