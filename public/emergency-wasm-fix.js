...
function applyWasmCSP() {
  ...
  const existingCSPs = document.querySelectorAll(
    'meta[http-equiv="Content-Security-Policy"]'
  )
  existingCSPs.forEach(meta => {
    console.log(
      'Removing existing CSP:',
  ...
  try {
    ...
    mediaElements.forEach(element => {
      if (element.src && element.src.startsWith('blob:')) {
        console.log(
          'Reloading blocked media element:',
...
async function testWasmSupport() {
  try {
    ...
    if (typeof globalThis.WebAssembly === 'undefined') {
      console.warn(
        'WebAssembly not available in this environment; skipping WASM test.'
      )
      return false
    }
    ...
    console.log(
      '⚠️ You may need to refresh the page for CSP changes to take effect'
    )
    return false
  }
}
