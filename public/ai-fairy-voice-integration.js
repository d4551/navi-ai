...
window.AIFairyVoice = {
    ...
    async loadSettings() {
        try {
            ...
            const fairySettings = JSON.parse(localStorage.getItem('ai-fairy-voice-settings') || '{}');
    ...
    validateSettings() {
        ...
        this.config.speechRate = Math.max(0.1, Math.min(3.0, this.config.speechRate || 0.85));
        this.config.speechPitch = Math.max(0.0, Math.min(2.0, this.config.speechPitch || 1.0));
        this.config.speechVolume = Math.max(0.0, Math.min(1.0, this.config.speechVolume || 0.9));
    ...
    async initializeGeminiTTS() {
        ...
        try {
            ...
            const { MultimodalLiveService } = await import('/src/services/MultimodalLiveService.js');
    ...
    async systemSpeak(text, options) {
        return new Promise((resolve, reject) => {
            ...
            if (options.ttsVoice || this.config.ttsVoice) {
                const voice = this.state.supportedVoices.find(v => 
                    v.name === (options.ttsVoice || this.config.ttsVoice)
                );
                if (voice) {
    ...
    findBestSystemVoice(preferredVoice) {
        ...
        let voice = voices.find(v => v.name.toLowerCase().includes(preferredVoice.toLowerCase()));
    ...
    async systemListen(options) {
        return new Promise((resolve, reject) => {
            ...
            recognition.continuous = options.continuousListening || this.config.continuousListening;
    ...
    processVoiceCommand(command) {
        ...
        if (lowerCommand.includes('fairy') || lowerCommand.includes('help')) {
            this.speak("Hello! I'm your AI fairy assistant. How can I help you today?");
    ...
    saveSettings() {
        try {
            ...
            localStorage.setItem('ai-fairy-voice-settings', JSON.stringify(this.config));
...
if (typeof window !== 'undefined' && typeof window.module !== 'undefined' && window.module.exports) {
