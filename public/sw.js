        ...
        return Promise.all(
          cacheNames
            .filter(cacheName => {
              ...
              return cacheName.startsWith('navi-') &&
                     ![CACHE_NAME, RUNTIME_CACHE, API_CACHE, ASSETS_CACHE].includes(cacheName);
            })
            .map(cacheName => {
...
async function handleDynamicRequest(request) {
  ...
  try {
    ...
    if (networkResponse.ok && networkResponse.headers.get('content-length') < 1024 * 1024) { // 1MB limit
      cache.put(request, networkResponse.clone());
    }
