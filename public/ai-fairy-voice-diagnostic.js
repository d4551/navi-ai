    ...
    try {
      ...
      if (provider === 'gemini') {
        ...
        if (appSettings.geminiApiKey) {
          ...
          console.warn('⚠️  Gemini API key not found, will fall back to system TTS');
        }
      }
...
async function testKokoroTTS() {
  ...
  try {
    ...
    try {
      ...
      console.log('✅ Kokoro audio generation successful, buffer size:', testAudio?.length || 0);
      
    } catch (localError) {
      console.warn('⚠️  Local Kokoro models not available, testing online fallback...');
      ...
      try {
        ...
        console.error('❌ Both local and online Kokoro failed:', onlineError.message);
      }
    }
...
async function testFairyChatIntegration() {
  ...
  try {
    ...
    const fairyButton = document.querySelector('[class*="fairy"]') || 
    ...
    if (fairyButton) {
      ...
      console.log('ℹ️  Fairy button not currently visible (may need to navigate to appropriate page)');
    }
...
function generateRecommendations() {
  ...
  if (!appSettings.geminiApiKey) {
    console.log('📝 1. Add Gemini API key in Settings > AI & API to enable Google AI TTS');
  }
  
  if (!appSettings.ttsProvider || appSettings.ttsProvider === 'system') {
    console.log('📝 2. Consider upgrading to Google AI or Kokoro TTS for better voice quality');
  }
  
  if (!appSettings.voiceMode) {
    console.log('📝 3. Enable Voice Mode in Settings > Voice Experience for hands-free interaction');
  }
  
  console.log('📝 4. Test different TTS providers in Settings > Voice Experience to find your preference');
  console.log('📝 5. Use the AI Fairy voice settings button to quickly adjust TTS options');
  console.log('📝 6. Check console for any errors if voice features are not working as expected');
}
...
if (typeof window !== 'undefined') {
  ...
  console.log('Run window.naviVoiceDiagnostic.runFullDiagnostic() to start testing');
}
